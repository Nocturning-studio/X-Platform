...
void CRender::create_hi_z_mip_chain()
{
	...
	RenderBackend.SetShaderElement(RenderTarget->s_hi_z->E[SE_HI_Z_GENERATE_MIP_CHAIN_PASS], 0);
	...
	for(u32 i = 1; i < MipChain->get_levels_count(); i++)
	{
		...
		RenderBackend.SetShaderElement(RenderTarget->s_hi_z->E[SE_HI_Z_GENERATE_MIP_CHAIN_PASS], 1);
		...
		RenderBackend.SetConstant("mip_data", 0.0f, (float)prev_mip, 1.0f / prev_mip_width, 1.0f / prev_mip_height);
