...
void CModelPool::DeleteQueue()
{
	...
	{
		// Визуал, на который сцена ещё держит ссылку, удалять нельзя:
...
#ifdef _EDITOR
IC bool _IsBoxVisible(IRender_Visual* visual, const fmat4x4& transform)
{
	Fbox bb;
...
void CModelPool::Render(IRender_Visual* m_pVisual, const fmat4x4& mTransform, int priority, bool strictB2F,
						float m_fLOD)
{
	// render visual
	...
	case MT_SKELETON_RIGID:
	{
		if(_IsBoxVisible(m_pVisual, mTransform))
		{
			...
			if(fis_zero(m_fLOD, EPS) && pV->m_lod)
			{
				...
				{
					RenderBackend.SetShader(pV->m_lod->shader ? pV->m_lod->shader : Device.m_WireShader);
					RenderBackend.SetTransformWorld(mTransform);
					pV->m_lod->Render(1.f);
				}
			}
			else
			{
				...
				for(; I != E; I++)
				{
					...
					{
						RenderBackend.SetShader((*I)->shader ? (*I)->shader : Device.m_WireShader);
						RenderBackend.SetTransformWorld(mTransform);
						(*I)->Render(m_fLOD);
					}
				}
	...
	case MT_PARTICLE_EFFECT:
	{
		//		if (_IsBoxVisible(m_pVisual,mTransform))
		{
			...
			{
				RenderBackend.SetShader(m_pVisual->shader ? m_pVisual->shader : Device.m_WireShader);
				RenderBackend.SetTransformWorld(mTransform);
...
void CModelPool::RenderSingle(IRender_Visual* m_pVisual, const fmat4x4& mTransform, float m_fLOD)
{
	for(int p = 0; p < 4; p++)
	{
