...
void CUIFrameWindow::FrameClip(const Frect parentAbsR)
{
	...
	// fmRT
	r.x1 = max(m_UIWndFrame.frame[CUIFrameRect::fmRT].GetPosX(), parentAbsR.left) -
		   m_UIWndFrame.frame[CUIFrameRect::fmRT].GetPosX();
	r.y1 = max(m_UIWndFrame.frame[CUIFrameRect::fmRT].GetPosY(), parentAbsR.top) -
		   m_UIWndFrame.frame[CUIFrameRect::fmRT].GetPosY();
	...
	// fmR
	RenderBackend.SetShader(m_UIWndFrame.frame[CUIFrameRect::fmR].GetShader());
	T = RenderBackend.GetActiveTexture(0);
	...
	size_x =
		min(ourAbsR.right, parentAbsR.right) - max(m_UIWndFrame.frame[CUIFrameRect::fmR].GetPosX(), parentAbsR.left);
	...
	// fmRB
	r.x1 = max(m_UIWndFrame.frame[CUIFrameRect::fmRB].GetPosX(), parentAbsR.left) -
		   m_UIWndFrame.frame[CUIFrameRect::fmRB].GetPosX();
	r.y1 = max(m_UIWndFrame.frame[CUIFrameRect::fmRB].GetPosY(), parentAbsR.top) -
		   m_UIWndFrame.frame[CUIFrameRect::fmRB].GetPosY();
	r.x2 = min(ourAbsR.right, parentAbsR.right) -
	...
	// fmB
	RenderBackend.SetShader(m_UIWndFrame.frame[CUIFrameRect::fmB].GetShader());
	T = RenderBackend.GetActiveTexture(0);
	ts.set((int)T->get_Width(), (int)T->get_Height());
	...
	size_y =
		min(ourAbsR.bottom, parentAbsR.bottom) - max(m_UIWndFrame.frame[CUIFrameRect::fmRB].GetPosY(), parentAbsR.top);
	rem_x = fmodf(size_x, ts.x);
	rem_y = fmodf(size_y, ts.y);
	...
	// fmLB
	r.x1 = max(m_UIWndFrame.frame[CUIFrameRect::fmLB].GetPosX(), parentAbsR.left) -
		   m_UIWndFrame.frame[CUIFrameRect::fmLB].GetPosX();
	r.y1 = max(m_UIWndFrame.frame[CUIFrameRect::fmLB].GetPosY(), parentAbsR.top) -
		   m_UIWndFrame.frame[CUIFrameRect::fmLB].GetPosY();
	...
	// fmL
	RenderBackend.SetShader(m_UIWndFrame.frame[CUIFrameRect::fmL].GetShader());
	T = RenderBackend.GetActiveTexture(0);
	ts.set((int)T->get_Width(), (int)T->get_Height());
	...
	// fmLT
	r.x1 = max(m_UIWndFrame.frame[CUIFrameRect::fmLT].GetPosX(), parentAbsR.left) -
		   m_UIWndFrame.frame[CUIFrameRect::fmLT].GetPosX();
	r.y1 = max(m_UIWndFrame.frame[CUIFrameRect::fmLT].GetPosY(), parentAbsR.top) -
		   m_UIWndFrame.frame[CUIFrameRect::fmLT].GetPosY();
	...
	// fmT
	RenderBackend.SetShader(m_UIWndFrame.frame[CUIFrameRect::fmT].GetShader());
	T = RenderBackend.GetActiveTexture(0);
	ts.set((int)T->get_Width(), (int)T->get_Height());
	...
	// back
	RenderBackend.SetShader(m_UIWndFrame.frame[CUIFrameRect::fmBK].GetShader());
	T = RenderBackend.GetActiveTexture(0);
	ts.set((int)T->get_Width(), (int)T->get_Height());
	...
	else
	{
		...
		m_UIWndFrame.frame[CUIFrameRect::fmBK].SetPos(
			max(m_UIWndFrame.frame[CUIFrameRect::fmBK].GetPosX(), parentAbsR.left),
			max(m_UIWndFrame.frame[CUIFrameRect::fmBK].GetPosY(), parentAbsR.top));
	}
}
