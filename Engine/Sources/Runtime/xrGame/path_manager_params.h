...
struct SBaseParameters
{
	...
	IC SBaseParameters(_dist_type max_range = type_max(_dist_type),
		...
		: max_range(max_range), max_iteration_count(max_iteration_count), max_visited_node_count(max_visited_node_count)
	{
	}
