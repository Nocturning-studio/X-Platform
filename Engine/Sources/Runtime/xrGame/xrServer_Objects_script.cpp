...
struct CWrapperBaseObjectsScript : public T, public luabind::wrap_base
{
	...
	IC CWrapperBaseObjectsScript(LPCSTR section) : T(section)
	{
	}
...
{
	module(L)[class_<IPureLоadableObject<IReader>>("ipure_alife_load_object"),
			  class_<IPureSavableObject<IWriter>>("ipure_alife_save_object"),
			  class_<IPureSerializeObject<IReader, IWriter>,
					 bases<IPureLоadableObject<IReader>, IPureSavableObject<IWriter>>>("ipure_alife_load_save_object"),
...
{
	typedef CWrapperBaseObjectsScript<CSE_Abstract> WrapType;
...
{
	module(L)[luabind_class_abstract1(CSE_Spectator, "cse_spectator", CSE_Abstract)];
}

void CSE_Temporary::script_register(lua_State* L)
{
	module(L)[luabind_class_abstract1(CSE_Temporary, "cse_temporary", CSE_Abstract)];
}
