...
struct CRegistryHelperLoad
{
	...
	{
		do_load<object_type_traits::is_base_and_derived<IPureLоadableObject<T2>, Head>::value>(self, p1);
	}
};
...
void CALifeRegistryContainer::load(IReader& file_stream)
{
	...
	CRegistryHelperProcess<CRegistryHelperLoad, CALifeRegistryContainer, IReader&, TYPE_LIST>::process(this,
																									   file_stream);
}
...
void CALifeRegistryContainer::save(IWriter& memory_stream)
{
	...
	CRegistryHelperProcess<CRegistryHelperSave, CALifeRegistryContainer, IWriter&, TYPE_LIST>::process(this,
																									   memory_stream);
