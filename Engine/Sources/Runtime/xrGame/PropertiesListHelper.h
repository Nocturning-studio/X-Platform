...
class CPropHelper : public IPropHelper
{
	...
	PropValue* AppendValue(PropItemVec& items, const shared_str& key, PropValue* val, EPropType type,
						   u32 item_flags = 0);
  ...
  public:
	...
	virtual bool __stdcall FvectorRDOnAfterEdit(PropValue* sender, fvec3& edit_val);
	virtual void __stdcall FvectorRDOnBeforeEdit(PropValue* sender, fvec3& edit_val);
  ...
  public:
	...
	virtual VectorValue* __stdcall CreateVector(PropItemVec& items, shared_str key, fvec3* val, float mn = 0.f,
												float mx = 1.f, float inc = 0.01f, int decim = 2);
	...
	virtual Flag16Value* __stdcall CreateFlag16(PropItemVec& items, shared_str key, Flags16* val, u16 mask,
												LPCSTR c0 = 0, LPCSTR c1 = 0, u32 flags = 0);
	...
	virtual VectorValue* __stdcall CreateVColor(PropItemVec& items, shared_str key, fvec3* val);
	...
	virtual VectorValue* __stdcall CreateAngle3(PropItemVec& items, shared_str key, fvec3* val, float mn = flt_min,
												float mx = flt_max, float inc = 0.01f, int decim = 2);
	...
	virtual RTextValue* __stdcall CreateNameCB(PropItemVec& items, shared_str key, shared_str* val,
											   ...
											   RTextValue::TOnAfterEditEvent = 0);
	...
	virtual TokenValueSH* __stdcall CreateTokenSH(PropItemVec& items, shared_str key, u32* val,
												  const TokenValueSH::Item* lst, u32 cnt);
