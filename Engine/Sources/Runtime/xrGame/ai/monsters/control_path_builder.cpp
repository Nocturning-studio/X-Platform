...
#include "xrGame/game_location_selector.h"
...
// Special Build Path
//////////////////////////////////////////////////////////////////////////
bool CControlPathBuilder::build_special(const fvec3& target, u32 node, u32 vel_mask)
{
	...
	if(!path_completed() && (detail().time_path_built() >= Engine.TimeManager.GetGlobalTimeMs()))
		return true;
...
bool CControlPathBuilder::valid_destination(const fvec3& pos, u32 node)
{
	return (ai().level_graph().valid_vertex_id(node) && ai().level_graph().valid_vertex_position(pos) &&
			ai().level_graph().inside(node, pos));
}

bool CControlPathBuilder::valid_and_accessible(fvec3& pos, u32 node)
{
	if(!valid_destination(pos, node) || !accessible(node))
		return false;

	fix_position(fvec3().set(pos), node, pos);
	return true;
}

void CControlPathBuilder::fix_position(const fvec3& pos, u32 node, fvec3& res_pos)
{
	...
	{
		u32 level_vertex_id = restrictions().accessible_nearest(fvec3().set(res_pos), res_pos);
...
bool CControlPathBuilder::get_node_in_radius(u32 src_node, float min_radius, float max_radius, u32 attempts,
											 u32& dest_node)
{
	fvec3 vertex_position = ai().level_graph().vertex_position(src_node);
...
u32 CControlPathBuilder::find_nearest_vertex(const u32& level_vertex_id, const fvec3& target_position,
											 const float& range)
{
	xr_vector<u32> temp;
