...
ObjectFactory::CLIENT_BASE_CLASS* CObjectItemScript::client_object() const
{
	...
	try
	{
		object = luabind::object_cast<ObjectFactory::CLIENT_SCRIPT_BASE_CLASS*>(m_client_creator(),
...
ObjectFactory::SERVER_BASE_CLASS* CObjectItemScript::server_object(LPCSTR section) const
{
	...
	try
	{
		...
		object =
			luabind::object_cast<ObjectFactory::SERVER_SCRIPT_BASE_CLASS*>(*instance, luabind::adopt(luabind::result));
		xr_delete(instance);
	}
	catch(std::exception& e)
	{
