...
void R_transforms::set_World(const fmat4x4& m)
{
	...
	if(c_World)
		RenderBackend.SetConstant(c_World, m_World);
	if(c_WorldView)
		RenderBackend.SetConstant(c_WorldView, m_WorldView);
	if(c_WorldViewProject)
		RenderBackend.SetConstant(c_WorldViewProject, m_WorldViewProject);
...
void R_transforms::set_View(const fmat4x4& m)
{
	...
	m_WorldViewProject.mul(m_Project, m_WorldView);
	if(c_View)
		RenderBackend.SetConstant(c_View, m_View);
	if(c_ViewProject)
		RenderBackend.SetConstant(c_ViewProject, m_ViewProject);
	if(c_WorldView)
		RenderBackend.SetConstant(c_WorldView, m_WorldView);
	if(c_WorldViewProject)
		RenderBackend.SetConstant(c_WorldViewProject, m_WorldViewProject);
...
void R_transforms::set_Project(const fmat4x4& m)
{
	...
	m_WorldViewProject.mul(m_Project, m_WorldView);
	if(c_Project)
		RenderBackend.SetConstant(c_Project, m_Project);
	if(c_ViewProject)
		RenderBackend.SetConstant(c_ViewProject, m_ViewProject);
	if(c_WorldViewProject)
		RenderBackend.SetConstant(c_WorldViewProject, m_WorldViewProject);
...
void R_transforms::apply_InvWorld()
{
	...
	RenderBackend.SetConstant(c_InvWorld, m_InvWorld);
}
