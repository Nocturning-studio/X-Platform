...
struct PARTICLES_API pDomain
{
	...
	// transformation
	void transform(const pDomain& domain, const float4x4& m);
	void transform_dir(const pDomain& domain, const float4x4& m);
	...
	pDomain(PDomainEnum dtype, float a0 = 0.0f, float a1 = 0.0f, float a2 = 0.0f, float a3 = 0.0f, float a4 = 0.0f,
