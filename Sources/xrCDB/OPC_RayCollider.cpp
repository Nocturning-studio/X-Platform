 ...
 *where you want to know whether the path to the light is _free or not (a boolean answer is enough).
 ...
 *queries start by colliding the ray against the cached triangle. If they still collide, we return immediately.
 *
 *	CLOSEST HIT:
 *
...
BOOL RayCollider::InitQuery(const Ray& world_ray, const Matrix4x4* world, udword* faceid)
{
	...
	if (IR(mMaxDist) != IEEE_MAX_FLOAT)
	{
		...
		// Precompute mFDir;
		mFDir.x = std::abs(mData.x);
		mFDir.y = std::abs(mData.y);
		mFDir.z = std::abs(mData.z);
	}
	else
	{
		...
		// Precompute mFDir;
		mFDir.x = std::abs(mDir.x);
		mFDir.y = std::abs(mDir.y);
		mFDir.z = std::abs(mDir.z);
	}
