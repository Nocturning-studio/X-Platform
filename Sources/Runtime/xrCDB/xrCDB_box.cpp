...
template <bool bClass3, bool bFirst> class box_collider
{
  public:
	...
	IC void _init(COLLIDER* CL, fvec3* V, TRI* T, const fvec3& C, const fvec3& E)
	{
		dest = CL;
		verts = V;
		tris = T;
	...
	{
		TRI& T = tris[prim];
		fvec3& v0 = verts[T.verts[0]];
		mLeafVerts[0].x = v0.x;
		mLeafVerts[0].y = v0.y;
		mLeafVerts[0].z = v0.z;
		fvec3& v1 = verts[T.verts[1]];
		mLeafVerts[1].x = v1.x;
		mLeafVerts[1].y = v1.y;
		mLeafVerts[1].z = v1.z;
		fvec3& v2 = verts[T.verts[2]];
		mLeafVerts[2].x = v2.x;
		mLeafVerts[2].y = v2.y;
		mLeafVerts[2].z = v2.z;
		if (!_tri())
...
void COLLIDER::box_query(const MODEL* m_def, const fvec3& b_center, const fvec3& b_dim)
{
	m_def->syncronize();
