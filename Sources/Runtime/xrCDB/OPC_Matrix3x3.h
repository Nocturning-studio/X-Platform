	...
	/*
			//! Compute a cofactor. Used for matrix inversion.
					float			CoFactor(ubyte row, ubyte column)	const
					{
						...
						return	(m[gIndex[row+1]][gIndex[column+1]]*m[gIndex[row+2]][gIndex[column+2]] -
	   m[gIndex[row+2]][gIndex[column+1]]*m[gIndex[row+1]][gIndex[column+2]]);
					}
	*/
	...
	{
		return Matrix3x3(m[0][0] + mat.m[0][0], m[0][1] + mat.m[0][1], m[0][2] + mat.m[0][2], m[1][0] + mat.m[1][0],
	...
	inline_ Matrix3x3 operator*(const Matrix3x3& mat) const
	{
		return Matrix3x3(m[0][0] * mat.m[0][0] + m[0][1] * mat.m[1][0] + m[0][2] * mat.m[2][0],
	...
	{
		return Point(ROW[0] | v, ROW[1] | v, ROW[2] | v);
	}
	...
	{
		return Matrix3x3(s * mat.m[0][0], s * mat.m[0][1], s * mat.m[0][2], s * mat.m[1][0], s * mat.m[1][1],
						 s * mat.m[1][2], s * mat.m[2][0], s * mat.m[2][1], s * mat.m[2][2]);
	}
	...
	inline_ Point* operator[](int nRow)
	{
		return (Point*)&m[nRow][0];
	}
