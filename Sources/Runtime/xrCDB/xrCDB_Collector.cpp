...
namespace CDB
{
u32 Collector::VPack(const fvec3& V, float eps)
{
...
void Collector::add_face(const fvec3& v0, const fvec3& v1, const fvec3& v2, u16 material, u16 sector)
{
	TRI T;
	T.verts[0] = verts.size();
	T.verts[1] = verts.size() + 1;
...
void Collector::add_face_packed_D(const fvec3& v0, const fvec3& v1, const fvec3& v2, // vertices
								  u32 dummy, float eps)
{
	TRI T;
	T.verts[0] = VPack(v0, eps);
...
void Collector::calc_adjacency(xr_vector<u32>& dest)
{
...

	{
		...
		for (; Edges_Iterator != Edges_End; ++Edges_Iterator)
		{
			...
			dest[(*J).face_id * 3 + (*J).edge_id] = (*Edges_Iterator).face_id;
		}
	}
#if 0
...
u32 CollectorPacked::VPack(const fvec3& V)
{
	u32 P = 0xffffffff;
