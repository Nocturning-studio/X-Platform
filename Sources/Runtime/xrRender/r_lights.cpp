...
void CRender::render_lights(light_Package& LP)
{
    ...
    LP.v_shadowed.erase(std::remove_if(LP.v_shadowed.begin(), LP.v_shadowed.end(),
        [&](light* L) { return !is_valid_light(L); }),
        LP.v_shadowed.end());
    ...
    // Упаковка shadow map
    {
        ...
        // SMAP-пул теперь принадлежит Scene
        SMAP_Allocator& smap_pool = Scene.GetSmapPool();

        for (u16 smap_ID = 0; !source.empty(); smap_ID++)
        {
            smap_pool.initialize(o.smapsize);

            for (auto it = source.begin(); it != source.end();)
            {
                light* L = *it;
                SMAP_Rect R;
                if (smap_pool.push(R, L->TransformContext.ShadowContext.size))
                {
    ...
    // ------------------------------------------------------------------------
    // Рендер теней
    xr_vector<light*>& lights_last_frame = Scene.GetLightsLastFrame();
    ...
    while (!LP.v_shadowed.empty())
    {
        ...
        lights_last_frame.insert(lights_last_frame.end(),
        ...
        for (light* L : current_batch)
        {
            ...
            SceneGraphPacket& packet = m_spot_shadow_vis.packet;

            bool bNormal = packet.queue_static[0].size() || packet.queue_dynamic[0].size();
            bool bSpecial = packet.queue_static[1].size() || packet.queue_dynamic[1].size() || packet.queue_transparent.size();
