...
class XRNETSERVER_API IPureClient : private MultipacketReciever, private MultipacketSender
{
  ...
  public:
	...
	BOOL net_isCompleted_Sync()
	{
		return net_Syncronised;
	}
	BOOL net_isDisconnected()
	{
		return net_Disconnected;
	}
	LPCSTR net_SessionName()
	{
		return *(net_Hosts.front().dpSessionName);
	}

	// receive
	IC virtual NET_Packet* net_msg_Retreive()
	{
		return net_Queue.Retreive();
	}
	IC void net_msg_Release()
	{
		net_Queue.Release();
	}
