...
#define LZO_NOTHROW
	...
	typedef int(__LZO_CDECL* lzo_compress_asm_t)(const lzo_byte* src, lzo_uint src_len,
												 lzo_byte* dst, lzo_uintp dst_len,
												 lzo_voidp wrkmem);

	typedef int(__LZO_CDECL* lzo_decompress_asm_t)(const lzo_byte* src, lzo_uint src_len,
												   lzo_byte* dst, lzo_uintp dst_len,
												   lzo_voidp wrkmem);
...
#define lzo_init() __lzo_init2(LZO_VERSION, (int)sizeof(short), (int)sizeof(int),                  \
	...
	LZO_EXTERN(int)
	__lzo_init2(unsigned, int, int, int, int, int, int, int, int, int);
	...
	LZO_EXTERN(unsigned)
	__lzo_align_gap(const lzo_voidp _ptr, lzo_uint _size);
