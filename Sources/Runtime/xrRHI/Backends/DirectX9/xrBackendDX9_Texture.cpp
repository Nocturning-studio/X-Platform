...
TextureHandle CRenderBackendDX9::CreateTexture(const TextureDesc& desc, const void* initialData)
{
    ...
    DWORD usage = 0;
    if (desc.isRenderTarget)  usage |= D3DUSAGE_RENDERTARGET;
    if (desc.isDepthStencil)  usage |= D3DUSAGE_DEPTHSTENCIL;
    ...
    if (desc.isCubeMap)
    {
        ...
        hr = m_pDevice->CreateCubeTexture(
            desc.width, desc.mipLevels, usage, d3dFmt,
            D3DPOOL_DEFAULT, &cubeTex, nullptr);
    ...
    else
    {
        ...
        hr = m_pDevice->CreateTexture(
            desc.width, desc.height, desc.mipLevels, usage, d3dFmt,
            D3DPOOL_DEFAULT, &tex2D, nullptr);
        ...
        if (initialData && !desc.isRenderTarget && !desc.isDepthStencil)
        {
            ...
            if (pixelSize > 0)
            {
                ...
                hr = m_pDevice->CreateTexture(
                    desc.width, desc.height, 1, 0, d3dFmt,
                    D3DPOOL_SYSTEMMEM, &sysTex, nullptr);
...
bool CRenderBackendDX9::CheckFormatSupport(RHI_Format fmt, bool isRenderTarget, bool isDepthStencil, bool isCube)
{
    ...
    DWORD usage = 0;
    if (isRenderTarget)  usage |= D3DUSAGE_RENDERTARGET;
    if (isDepthStencil)  usage |= D3DUSAGE_DEPTHSTENCIL;
...
bool CRenderBackendDX9::GetCubeMapFaceNative(TextureHandle handle, u32 face, u32 level, void** outSurface)
{
    ...
    *outSurface = nullptr;
    DX9Texture* tex = GetTexture(handle);
    if (!tex || !tex->texCube) return false;
