...
ShaderHandle CRenderBackendDX9::CreateShader(ShaderType type, const void* bytecode, size_t bytecodeSize) {
    if (!m_pDevice) return ShaderHandle{};
    ...
    impl->type = type;
    impl->bytecode.assign(static_cast<const u8*>(bytecode),
        static_cast<const u8*>(bytecode) + bytecodeSize);

    HRESULT hr = S_OK;
    if (type == ShaderType::Vertex) {
        hr = m_pDevice->CreateVertexShader(
            reinterpret_cast<const DWORD*>(impl->bytecode.data()),
            &impl->vs
        );
    }
...
void CRenderBackendDX9::UpdateConstantBuffer(ConstantBufferHandle handle, u32 offset, const void* data, u32 size) {
    ...
    if (offset + size > cb->data.size() * sizeof(float)) {
        Print("! [DX9] UpdateConstantBuffer: offset+size (%u) exceeds buffer capacity (%zu bytes).",
            offset + size, cb->data.size() * sizeof(float));
        return;
    }
...
void CRenderBackendDX9::SetShaderConstantBuffer(ShaderType type, u32 startRegister, ConstantBufferHandle handle) {
    ...
    case ShaderType::Pixel:
        m_pDevice->SetPixelShaderConstantF(
            startRegister,
            cb->data.data(),
            static_cast<UINT>(cb->data.size() / 4)
        );
        break;
    default:
...
ShaderConstantLayout CRenderBackendDX9::ReflectConstantLayout(ShaderHandle handle)
{
    ...
    UINT dwSize = 0;
    HRESULT hr = D3DXFindShaderComment(
        reinterpret_cast<const DWORD*>(shader->bytecode.data()),
        MAKEFOURCC('C', 'T', 'A', 'B'),
        &pData,
        &dwSize);
    ...
    const D3DXSHADER_CONSTANTINFO* info = reinterpret_cast<const D3DXSHADER_CONSTANTINFO*>(
        reinterpret_cast<const BYTE*>(pTable) + pTable->ConstantInfo);
    ...
    for (u32 i = 0; i < pTable->Constants; ++i, ++info)
    {
        ...
        {
        case D3DXRS_BOOL:  type = ConstantType::Bool; break;
        case D3DXRS_INT4:  type = ConstantType::Int; break;
        case D3DXRS_FLOAT4: type = ConstantType::Float; break;
        case D3DXRS_SAMPLER: type = ConstantType::Sampler; break;
        default: continue;
        }
        ...
        switch (T->Class)
        {
        case D3DXPC_SCALAR: cls = ConstantClass::Scalar; break;
        case D3DXPC_VECTOR: cls = ConstantClass::Vector; break;
        case D3DXPC_MATRIX_ROWS:
            if (T->Columns == 4 && T->Rows == 4) cls = ConstantClass::MatrixRows_4x4;
            else if (T->Columns == 4 && T->Rows == 3)
                cls = (info->RegisterCount == 2) ? ConstantClass::MatrixRows_2x4 : ConstantClass::MatrixRows_3x4;
            break;
        case D3DXPC_MATRIX_COLUMNS:
            if (T->Columns == 4 && T->Rows == 4) cls = ConstantClass::MatrixColumns_4x4;
            else if (T->Columns == 4 && T->Rows == 3)
                cls = (info->RegisterCount == 2) ? ConstantClass::MatrixColumns_2x4 : ConstantClass::MatrixColumns_3x4;
            break;
        case D3DXPC_STRUCT: cls = ConstantClass::Struct; break;
        case D3DXPC_OBJECT:
        ...
        if (type == ConstantType::Sampler)
        {
            ...
            layout.fields.push_back({ name,
                                    static_cast<u32>(info->RegisterIndex * 16),
                                    0u,
                                    type,
                                    ConstantClass::Object,
                                    static_cast<u16>(info->RegisterIndex),
        ...
        layout.fields.push_back({ name, offset, size, type, cls,
                                  static_cast<u16>(info->RegisterIndex),
                                  static_cast<u16>(info->RegisterCount) });

        u32 endReg = static_cast<u32>(info->RegisterIndex + info->RegisterCount);
        if (endReg > maxReg) maxReg = endReg;
    }
