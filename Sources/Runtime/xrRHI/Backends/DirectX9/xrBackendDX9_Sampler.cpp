...
void CRenderBackendDX9::ApplySampler(u32 slot, const SamplerDesc& desc)
{
    ...
    D3DTEXTUREFILTERTYPE mipFilter = (desc.mipFilter == RHI_Filter::None)
        ? D3DTEXF_NONE
        : RHIFilterToD3D(desc.mipFilter);
    ...
    if (desc.addressU == RHI_TextureAddress::Border ||
        desc.addressV == RHI_TextureAddress::Border ||
        desc.addressW == RHI_TextureAddress::Border)
    {
        D3DCOLOR borderColor = D3DCOLOR_COLORVALUE(
            desc.borderColor.x, desc.borderColor.y, desc.borderColor.z, desc.borderColor.w);
