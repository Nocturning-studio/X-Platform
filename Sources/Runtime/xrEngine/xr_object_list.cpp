...
void CObjectList::SingleUpdate(CObject* O)
{
	...
	if (O->processing_enabled() && (Engine.TimeManager.GetFrameCount() != O->dwFrame_UpdateCL))
	{
		...
		Engine.Statistic->UpdateClient_updated++;
		...
		if (O->H_Parent() && (O->H_Parent()->getDestroy() || O->H_Root()->getDestroy()))
		{
			...
			Msg("! ERROR: incorrect destroy sequence for object[%d:%s], section[%s], parent[%d:%s]", O->ID(),
				*O->cName(), *O->cNameSect(), O->H_Parent()->ID(), *O->H_Parent()->cName());
...
void CObjectList::Update(bool bForce)
{
	...
	if (!(Device.Paused() && !bForce))
	{
		...
		{
			// Select Crow-Mode
			Engine.Statistic->UpdateClient_updated = 0;
			Engine.Statistic->UpdateClient_crows = crows->size();
			...
			Engine.Statistic->UpdateClient.Begin();
			Engine.Statistic->UpdateClient_active = objects_active.size();
			Engine.Statistic->UpdateClient_total = objects_active.size() + objects_sleeping.size();
...
u32 CObjectList::net_Export(NET_Packet* _Packet, u32 start, u32 max_object_size)
{
	...
	for (; start < objects_active.size() + objects_sleeping.size(); start++)
	{
		...
		if (P->net_Relevant() && !P->getDestroy())
		{
			...
			if (g_Dump_Export_Obj)
			{
				u32 sz = u32(Packet.w_tell() - position) - sizeof(u8);
				Msg("* %s : %d", *(P->cNameSect()), sz);
			}
