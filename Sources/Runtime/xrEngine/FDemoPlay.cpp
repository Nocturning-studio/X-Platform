...
void CDemoPlay::MoveCameraSpline(float t, int i0, int i1, int i2, int i3)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		spline1(t, v, (fvec3*)&m_Camera.m[i][0]);
	}
}

void linearInterpolate(float t, fvec3* p0, fvec3* p1, fvec3* ret)
{
...
void CDemoPlay::MoveCameraLinear(float t, int i1, int i2)
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		linearInterpolate(t, &p0, &p1, (fvec3*)&m_Camera.m[i][0]);
	}
}
