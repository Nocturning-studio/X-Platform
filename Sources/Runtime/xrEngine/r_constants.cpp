	...
	for(u32 i = 0; i < desc->Constants; i++, it++)
	{
		...
		case D3DXPC_MATRIX_ROWS:
			...
			case 4:
				...
				{
				case 3:
					if(it->RegisterCount == 2)
						r_class = RC_2x4;
					else if(it->RegisterCount == 3)
						r_class = RC_3x4;
					else
						FATAL("MATRIX_ROWS: unsupported RegisterCount");
					break;
				case 4:
					r_class = RC_4x4;
					...
					break;
				default:
					FATAL("MATRIX_ROWS: unsupported Rows");
				}
				break;
			default:
				FATAL("MATRIX_ROWS: unsupported Columns");
			}
			break;
		...
		case D3DXPC_OBJECT:
			...
			{
				// Это сэмплер – обрабатываем отдельно
				ParamDesc sampParam;
				sampParam.name = name;
		...
		// Ищем, есть ли уже такая (могла быть объявлена в другом шейдере)
		bool exists = false;
		for(auto& p : m_params)
		{
			if(p.name == name)
...
{
	auto it = std::lower_bound(m_params.begin(), m_params.end(), name, [](const ParamDesc& p, const char* s)
							   { return xr_strcmp(p.name.c_str(), s) < 0; });
...
void CShaderConstantLayout::Merge(const CShaderConstantLayout& other)
{
	for(const auto& src : other.m_params)
	{
		...
		else
		{
			...
			if(src.destination & RC_dest_pixel)
				it->ps = src.ps;
			if(src.destination & RC_dest_vertex)
				it->vs = src.vs;
			if(src.destination & RC_dest_sampler)
				it->samp = src.samp;
		}
	}
	std::sort(m_params.begin(), m_params.end(), [](const ParamDesc& a, const ParamDesc& b)
