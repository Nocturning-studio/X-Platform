...
void game_cl_mp::shedule_Update(u32 dt)
{
	...
	if (m_pVoteRespondWindow && m_pVoteRespondWindow->IsShown() &&
		(cur_game_state != GAME_PHASE_INPROGRESS || !IsVotingActive()))
	{
...
void game_cl_mp::OnPlayerVoted(game_PlayerState* ps)
{
	...
	CStringTable st;
	string1024 resStr;
	sprintf_s(resStr, "%s\"%s\" %s%s %s\"%s\"", Color_Teams[ps->team], ps->getName(), Color_Main,
...
void game_cl_mp::OnPlayerChangeName(NET_Packet& P)
{
	...
	string1024 resStr;
	sprintf_s(resStr, "%s\"%s\" %s%s %s\"%s\"", Color_Teams[Team], OldName, Color_Main, *st.translate("mp_is_now"),
...
void game_cl_mp::OnRankChanged(u8 OldRank)
{
	...
	sprintf_s(RankStr, "%s : %s", *st.translate("mp_your_rank"),
