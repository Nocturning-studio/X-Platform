...
int CLevel::get_RPID(LPCSTR /**name/**/)
{
	...
	svector<fvec4,maxRP>	&rp = Level().get_team(team).RespawnPoints;
...
void CLevel::OnFrame()
{
	...
	{
		Engine.Statistic->netClient1.Begin();

		ClientReceive();

		Engine.Statistic->netClient1.End();
	}
		...
		if(!psNET_direct_connect)
		{
			...
			if(IsClient())
			{
				...
				F->OutNext("P(%d), BPS(%2.1fK), MRR(%2d), MSR(%2d), Retried(%2d), Blocked(%2d), Sended(%2d), SPS(%2d)",
						   ...
						   net_Statistic.getMPS_Receive(), net_Statistic.getMPS_Send(), net_Statistic.getRetriedCount(),
						   net_Statistic.dwTimesBlocked, net_Statistic.dwBytesSended, net_Statistic.dwBytesPerSec);
			}
		}
	}
	...
	if(!g_dedicated_server)
	{
		...
		Engine.ThreadManager.AddParallelTask([this] { m_level_sound_manager->Update(); }, CThreadManager::TaskPriority::Normal, CThreadManager::TaskType::AI);
	}
	// deffer LUA-GC-STEP
	if(!g_dedicated_server)
	{
		Engine.ThreadManager.AddParallelTask([this] { script_gc(); }, CThreadManager::TaskPriority::Normal, CThreadManager::TaskType::AI);
	}
	//-----------------------------------------------------
	if(pStatGraphR)
	{
