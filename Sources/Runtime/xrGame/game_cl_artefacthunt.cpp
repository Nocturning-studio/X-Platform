			...
			for (int id = 0; O->find_chunk(id); ++id)
			{
				...
				case rptTeamBaseParticle: {
					...
					if (pSettings->line_exist("artefacthunt_gamedata", ParticleStr))
					{
						...
						CParticlesObject* pStaticParticles = CParticlesObject::Create(
							pSettings->r_string("artefacthunt_gamedata", ParticleStr), FALSE, false);
...
void game_cl_ArtefactHunt::TranslateGameMessage(u32 msg, NET_Packet& P)
{
	...
	case GAME_EVENT_ARTEFACT_DROPPED: // ahunt
	{
		...
		//			pMessageSounds[0].play_at_pos(NULL, fvec3().set(0,0,0), sm_2D, 0);
	...
	{
		sprintf_s(Text, "%s%s", Color_Main, *st.translate("mp_art_spowned"));
		CommonMessageOut(Text);
	...
	{
		sprintf_s(Text, "%s%s", Color_Main, *st.translate("mp_art_destroyed"));
