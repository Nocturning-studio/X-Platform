...
#pragma todo("Andy->Andy how to check infoportion existence in XML ?")
	...
	for (u32 i = 0; i < m_DontHasInfo.size(); i++)
	{
		/*		INFO_INDEX	result = CInfoPortion::IdToIndex(m_DontHasInfo[i],NO_INFO_INDEX,true);
				if (result == NO_INFO_INDEX) {
					ai().script_engine().script_log(eLuaMessageTypeError,"XML item not found :
		   \"%s\"",*m_DontHasInfo[i]); break;
				}
		*/
...
bool CPhraseScript::Precondition(const CGameObject* pSpeakerGO1, const CGameObject* pSpeakerGO2, LPCSTR dialog_id,
...
void CPhraseScript::Action(const CGameObject* pSpeakerGO1, const CGameObject* pSpeakerGO2, LPCSTR dialog_id,
