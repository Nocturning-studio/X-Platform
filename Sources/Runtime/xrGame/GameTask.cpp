...
{
	int res =
		(object_cast<int>(luabind::object(luabind::get_globals(ai().script_engine().lua())["story_ids"])[story_id]));
...
SGameTaskObjective::SGameTaskObjective(CGameTask* parent, int _idx)
	: description(NULL), article_id(NULL), map_location(NULL), object_id(u16(-1)), task_state(eTaskStateInProgress),
