...
class CScriptGameObject
{
  ...
  public:
	...
	fvec3 Center();
	_DECLARE_FUNCTION10(Position, fvec3);
	_DECLARE_FUNCTION10(Direction, fvec3);
	...
	// Actor only
	void SetActorPosition(fvec3 pos);
	...
	void DropItemAndTeleport(CScriptGameObject* pItem, fvec3 position);
	...
	void set_const_force(const fvec3& dir, float value, u32 time_interval);
	//////////////////////////////////////////////////////////////////////////
	...
	fvec3 memory_position(const CScriptGameObject& lua_game_object);
	...
	void set_sight(SightManager::ESightType sight_type, const fvec3* vector3d, u32 dwLookOverDelay);
	void set_sight(SightManager::ESightType sight_type, bool torso_look, bool path);
	void set_sight(SightManager::ESightType sight_type, const fvec3& vector3d, bool torso_look);
	void set_sight(SightManager::ESightType sight_type, const fvec3* vector3d);
	...
	void set_desired_position(const fvec3* desired_position);
	void set_desired_direction();
	void set_desired_direction(const fvec3* desired_direction);
	...
	const CCoverPoint* best_cover(const fvec3& position, const fvec3& enemy_position, float radius,
								  float min_enemy_distance, float max_enemy_distance);
	const CCoverPoint* safe_cover(const fvec3& position, float radius, float min_distance);
	...
	bool accessible_position(const fvec3& position);
	...
	fvec3 bone_position(LPCSTR bone_name) const;
	...
	bool inside(const fvec3& position, float epsilon) const;
	bool inside(const fvec3& position) const;

	fvec3 head_orientation() const;
	u32 vertex_in_direction(u32 level_vertex_id, fvec3 direction, float max_distance) const;

	void info_add(LPCSTR text);
	void info_clear();

	// Monster Jumper
	void jump(const fvec3& position, float factor);
	...
	u32 location_on_path(float distance, fvec3* location);
