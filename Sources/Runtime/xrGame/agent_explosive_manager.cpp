...
void CAgentExplosiveManager::register_explosive(const CExplosive* explosive, const CGameObject* game_object)
{
	...
	m_explosives.push_back(CDangerExplosive(explosive, game_object, 0, Engine.TimeManager.GetGlobalTimeMs()));
	...
	object().location().add(xr_new<CDangerObjectLocation>(game_object, Engine.TimeManager.GetGlobalTimeMs(), interval, GRENADE_RADIUS));
}
...
void CAgentExplosiveManager::react_on_explosives()
{
...

	{
		...
		m_explosives.erase(std::remove_if(m_explosives.begin(), m_explosives.end(), CRemoveExplosivesPredicate()),
						   m_explosives.end());
	}
}
