...
struct SFillPropData
{
	...
	void load()
	{
		...
		// spawn story names
		{
			...
			spawn_story_names.insert(spawn_story_names.begin(),
									 xr_rtoken("NO SPAWN STORY ID", ALife::_SPAWN_STORY_ID(-1)));
		}
		...
		concurrency::parallel_sort(character_profiles.begin(), character_profiles.end(), SortStringsByAlphabetPred);
#endif
...
void CSE_ALifeTraderAbstract::FillProps(LPCSTR pref, PropItemVec& items)
{
	...
	PHelper().CreateFlag32(items, PrepareKey(pref, *base()->s_name, "Trader\\Infinite ammo"), &m_trader_flags,
						   eTraderFlagInfiniteAmmo);
#ifdef XRSE_FACTORY_EXPORTS
	RListValue* value =
		PHelper().CreateRList(items, PrepareKey(pref, *base()->s_name, "npc profile"), &m_sCharacterProfile,
...
{
#ifdef XRSE_FACTORY_EXPORTS
	PHelper().CreateRToken8(items, PrepareKey(pref, *s_name, "Location\\1"), &m_tLocations[0],
							&*fp_data.locations[0].begin(), fp_data.locations[0].size());
	PHelper().CreateRToken8(items, PrepareKey(pref, *s_name, "Location\\2"), &m_tLocations[1],
							&*fp_data.locations[1].begin(), fp_data.locations[1].size());
	PHelper().CreateRToken8(items, PrepareKey(pref, *s_name, "Location\\3"), &m_tLocations[2],
...
#ifdef XRSE_FACTORY_EXPORTS
	PHelper().CreateRToken32(items, PrepareKey(pref, *s_name, "ALife\\Story ID"), &m_story_id,
...
{
	inherited1::FillProps(pref, items);
	PHelper().CreateToken8(items, PrepareKey(pref, *s_name, "restrictor type"), &m_space_restrictor_type,
						   defaul_retrictor_types);
...
void CSE_ALifeObjectHangingLamp::FillProps(LPCSTR pref, PropItemVec& values)
{
	...
	PHelper().CreateChoose(values, PrepareKey(pref, *s_name, "Light\\Main\\Texture"), &light_texture, smTexture,
						   "lights");
	...
	if (flags.is(flPointAmbient))
	{
		...
		PHelper().CreateChoose(values, PrepareKey(pref, *s_name, "Light\\Ambient\\Texture"), &m_ambient_texture,
							   smTexture, "lights");
