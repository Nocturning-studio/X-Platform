...
struct animation_stats
{
	...
	IC animation_stats(const shared_str& visual_id, const u32& frame_count, const u32& start_count)
		: //		m_visual_id		(visual_id),
		  m_frame_count(frame_count), m_start_count(start_count)
	{
	}
};
...
struct blend_id_predicate
{
	...
	template <typename T> IC bool less(const std::pair<T, T>& _1, const std::pair<T, T>& _2) const
	{
...
void add_animation(const shared_str& animation_id, const shared_str& animation_set_id, const shared_str& visual_id,
				   bool just_started)
{
...
void CStalkerAnimationManager::add_animation_stats(const ANIMATION_ID& animation_id, const BLEND_ID* blend_id,
												   bool just_started)
{
	::add_animation_stats(animation_id.first, animation_id.second, *object().Visual()->dbg_name, blend_id,
						  just_started);
}
