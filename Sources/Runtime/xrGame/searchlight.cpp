...
void CProjector::UpdateCL()
{
	...
	// Update searchlight
	angle_lerp(_current.yaw, _target.yaw, bone_x.velocity, Engine.TimeManager.GetDeltaTime());
	angle_lerp(_current.pitch, _target.pitch, bone_y.velocity, Engine.TimeManager.GetDeltaTime());
}
...
void CProjector::SetTarget(const fvec3& target_pos)
{
	float th, tp;
	fvec3().sub(target_pos, Position()).getHP(th, tp);
...
fvec3 CProjector::GetCurrentDirection()
{
	return (fvec3().setHP(_current.yaw, _current.pitch));
}
