...
enum EPropType
{
	...
	PROP_BUTTON,
	PROP_CHOOSE,
	PROP_NUMERIC, // {u8,u16,u32,s8,s16,s32,float}
	PROP_BOOLEAN,
	PROP_FLAG,
	PROP_VECTOR,
	PROP_TOKEN,
...
template <class T> IC void set_value(T& val, const T& _val)
{
	val = _val;
};

template <class T> class CustomValue : public PropValue
{
  public:
	typedef T TYPE;
...
class PropItem
{
  ...
  public:
	...
	template <class T1, class T2> IC void BeforeEdit(T2& val)
	{
	...
	template <class T1, class T2> IC bool AfterEdit(T2& val)
	{
		T1* CV = smart_cast<T1*>(values.front());
		VERIFY(CV);
	...
	template <class T1, class T2> IC bool ApplyValue(const T2& val)
	{
		bool bChanged = false;
		m_Flags.set(flMixed, FALSE);
		for (PropValueIt it = values.begin(); values.end() != it; ++it)
	...
	IC void Enable(BOOL val)
	{
		m_Flags.set(flDisabled, !val);
	}
	IC BOOL Enabled()
	{
		return !m_Flags.is(flDisabled);
	}
	IC void OnChange()
	{
		for (PropValueIt it = values.begin(); values.end() != it; ++it)
...
class CaptionValue : public PropValue
{
  ...
  public:
	CaptionValue(const shared_str& val)
	{
		value = val;
	}
...
IC bool operator==(const xr_shortcut& A, const xr_shortcut& B)
{
	return !!A.similar(B);
}
...
//------------------------------------------------------------------------------
template <class T> IC xr_string draw_sprintf(xr_string& s, const T& V, int tag)
{
	string256 tmp;
	sprintf_s(tmp, "%d", V);
	s = tmp;
...
//------------------------------------------------------------------------------
IC bool operator==(const fvec3& A, const fvec3& B)
{
	return !!A.similar(B);
}
IC void clamp(fvec3& V, const fvec3& mn, const fvec3& mx)
{
	clamp(V.x, mn.x, mx.x);
	clamp(V.y, mn.y, mx.y);
	clamp(V.z, mn.z, mx.z);
}
IC xr_string draw_sprintf(xr_string& s, const fvec3& V, int dec)
{
	string128 fmt;
	sprintf_s(fmt, "{%%.%df, %%.%df, %%.%df}", dec, dec, dec);
	string256 tmp;
	sprintf_s(tmp, fmt, V.x, V.y, V.z);
	s = tmp;
	return s;
}
//------------------------------------------------------------------------------
...
class VectorValue : public NumericValue<fvec3>
{
  public:
	VectorValue(fvec3* val, float mn, float mx, float increment, int decimal) : NumericValue<fvec3>(val)
	{
		lim_mn.set(mn, mn, mn);
		lim_mx.set(mx, mx, mx);
...
template <class T> class FlagValue : public CustomValue<T>, public FlagValueCustom
{
  public:
	typedef T TYPE;
  ...
  public:
	...
	virtual void ResetValue()
	{
		value->set(mask, init_value.is(mask));
	}
	virtual bool GetValueEx()
	{
		return !!value->is(mask);
	}
	bool ApplyValue(const T& val)
	{
...
//------------------------------------------------------------------------------
template <class T> bool operator==(template_flags<T> const& A, template_flags<T> const& B)
{
...
template <class T> class TokenValue : public CustomValue<T>, public TokenValueCustom
{
  public:
...
template <class T> class RTokenValue : public CustomValue<T>, public RTokenValueCustom
{
  public:
