...
typedef struct tagSoundElement
{
	...
	fvec3 position; // позиция звука, не объекта, издавшего звук
	float power;
	...
	IC void SetConvert(const CObject* who, int eType, const fvec3& position, float power, TTime time)
	{
		this->who = who;
	...
	void CalcValue(TTime cur_time, fvec3 cur_pos)
	{
...
class CMonsterSoundMemory
{
  ...
  public:
	...
	void HearSound(const CObject* who, int eType, const fvec3& Position, float power, TTime time);
