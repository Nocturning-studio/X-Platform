...
void CChimera::reinit()
{
	...
	movement().detail().add_velocity(MonsterMovement::eChimeraVelocityParameterUpperWalkFwd,
									 CDetailPathManager::STravelParams(m_fsVelocityWalkUpper.velocity.linear,
																	   m_fsVelocityWalkUpper.velocity.angular_path,
																	   m_fsVelocityWalkUpper.velocity.angular_real));
...
void CChimera::CheckSpecParams(u32 spec_params)
{
	...
	if (b_upper_state)
	{
		...
		case eAnimWalkFwd:
			...
			break;
		case eAnimStandTurnLeft:
			anim().SetCurAnim(eAnimUpperStandTurnLeft);
			break;
		case eAnimStandTurnRight:
		...
		case eAnimThreaten:
			...
			break;
		case eAnimStandIdle:
			anim().SetCurAnim(eAnimUpperStandIdle);
			break;
		}
	}
}
