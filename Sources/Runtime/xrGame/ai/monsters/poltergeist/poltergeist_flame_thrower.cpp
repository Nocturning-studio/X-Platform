...
#include "xrGame/../xrNetServer/net_utils.h"
...
void CPolterFlame::create_flame(const CObject* target_object)
{
	...
	fvec3 target_point = get_head_position(const_cast<CObject*>(target_object));
			...
			if(Actor()->Position().distance_to(m_object->Position()) < m_scan_radius)
			{
				...
				if(m_scan_next_time < time())
				{
					...
					Actor()->Cameras().AddPPEffector(
						xr_new<CMonsterEffector>(m_scan_effector_info, m_scan_effector_time,
												 m_scan_effector_time_attack, m_scan_effector_time_release));
				}
			}
		}
				...
				if(elem->time_last_hit + m_hit_delay < time())
				{
												   ...
												   NULL))
					{
						if((rq.O == elem->target_object) && (rq.range < m_length))
						{
							...
							HS.p_in_bone_space = (fvec3().set(
								0.f, 0.f, 0.f)); //					P.w_vec3		(fvec3().set(0.f,0.f,0.f));
...
bool CPolterFlame::get_valid_flame_position(const CObject* target_object, fvec3& res_pos)
{
	...
	float angle =
		ai().level_graph().vertex_cover_angle(Obj->ai_location().level_vertex_id(), PI_DIV_6, std::less<float>());
