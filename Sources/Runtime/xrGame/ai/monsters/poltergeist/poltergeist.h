...
class CPoltergeist : public CBaseMonster, public CTelekinesis, public CEnergyHolder
{
  ...
  public:
	...
	// Poltergeist ability
	void PhysicalImpulse(const fvec3& position);
	void StrangeSounds(const fvec3& position);
...
class CPolterFlame : public CPolterSpecialAbility
{
  ...
  private:
	...
	bool get_valid_flame_position(const CObject* target_object, fvec3& res_pos);
...
class CPolterTele : public CPolterSpecialAbility
{
  ...
  private:
	void tele_find_objects(xr_vector<CObject*>& objects, const fvec3& pos);
	bool tele_raise_objects();
	void tele_fire_objects();

	bool trace_object(CObject* obj, const fvec3& target);
};
