...
#include "xrGame/stalker_sound_data.h"
#include "xrGame/stalker_sound_data_visitor.h"
...
#include "xrGame/alife_human_brain.h"
...
#include "xrGame/location_manager.h"
#include "xrGame/torch.h" // [IMPROVEMENT] Needed for flashlight logic
...
void CAI_Stalker::LoadSounds(LPCSTR section)
{
	...
	sound().add(pSettings->r_string(section, "sound_running_in_danger"), 100, SOUND_TYPE_MONSTER_TALKING, 6,
				...
				xr_new<CStalkerSoundData>(this));
	sound().add(pSettings->r_string(section, "sound_walking_in_danger"), 100, SOUND_TYPE_MONSTER_TALKING, 6,
				u32(eStalkerSoundMaskMovingInDanger), eStalkerSoundWalkingInDanger, head_bone_name,
				xr_new<CStalkerSoundData>(this));
...
BOOL CAI_Stalker::net_Spawn(CSE_Abstract* DC)
{
	...
	m_fRankDisperison =
		expirienced_rank_dispersion + (expirienced_rank_dispersion - novice_rank_dispersion) * (1 - rank_k);
