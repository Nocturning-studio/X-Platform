...
#include "xrGame/sound_memory_manager.h"
#include "xrGame/hit_memory_manager.h"
#include "xrGame/enemy_manager.h"
#include "xrGame/danger_manager.h"
...
#include "xrGame/object_handler_planner.h"
...
#include "xrGame/game_path_manager.h"
...
#include "xrGame/agent_manager.h"
#include "xrGame/agent_member_manager.h"
#include "xrGame/agent_enemy_manager.h"
#include "xrGame/agent_corpse_manager.h"
#include "xrGame/agent_location_manager.h"
#include "xrGame/cover_point.h"
#include "xrEngine/camerabase.h"
#include "xrGame/clsid_game.h"
#include "xrGame/mt_config.h"
...
void draw_planner(const planner_type& brain, LPCSTR start_indent, LPCSTR indent, LPCSTR planner_id)
{
	...
	HUD().Font().pFontStat->OutNext("%s%sselected    : %s", start_indent, indent,
...
void CAI_Stalker::OnHUDDraw(CCustomHUD* hud)
{
	...
	if (inventory().ActiveItem())
	{
		...
		HUD().Font().pFontStat->OutNext("%s%sactor is enemy    : %c", indent, indent,
										is_relation_enemy(actor) ? '+' : '-');
		HUD().Font().pFontStat->OutNext("%s%sis enemy to actor : %c", indent, indent,
										actor->is_relation_enemy(this) ? '+' : '-');
	}
	...
	if (memory().enemy().selected())
	{
		...
		if (g_Alive())
		{
			...
			HUD().Font().pFontStat->OutNext("%s%s%svictory   : [%5.2f%%,%5.2f%%] -> %s", indent, indent, indent,
											100.f * right, 100.f * left, description);
		}
	}
	// danger
	...
	if (g_Alive())
	{
		...
		HUD().Font().pFontStat->OutNext("%sdanger locations  : %d", indent,
	...
	HUD().Font().pFontStat->OutNext("%s%sactive item         : %s", indent, indent,
									inventory().ActiveItem() ? *inventory().ActiveItem()->object().cName() : "");
	HUD().Font().pFontStat->OutNext("%s%sbest weapon         : %s", indent, indent,
									best_weapon() ? *best_weapon()->object().cName() : "");
	HUD().Font().pFontStat->OutNext("%s%sitem to kill        : %s", indent, indent,
									item_to_kill() ? *m_best_item_to_kill->object().cName() : "");
	HUD().Font().pFontStat->OutNext("%s%sitem can kill       : %s", indent, indent, item_can_kill() ? "+" : "-");
	HUD().Font().pFontStat->OutNext("%s%smemory item to kill : %s", indent, indent,
									remember_item_to_kill() ? *m_best_found_item_to_kill->object().cName() : "");
	HUD().Font().pFontStat->OutNext("%s%smemory ammo         : %s", indent, indent,
									remember_ammo() ? *m_best_found_ammo->object().cName() : "");
	HUD().Font().pFontStat->OutNext("%s%sinfinite ammo       : %s", indent, indent, m_infinite_ammo ? "+" : "-");
	HUD().Font().pFontStat->OutNext("%s%sitem to spawn       : %s", indent, indent,
									item_to_spawn().size() ? *item_to_spawn() : "no item to spawn");
	HUD().Font().pFontStat->OutNext("%s%sammo in box to spawn: %d", indent, indent,
	...
	if (weapon)
	{
		...
		HUD().Font().pFontStat->OutNext(
			"%s%squeue interval      : %d", indent, indent,
	...
	if (inventory().ActiveItem())
	{
		...
		if (weapon)
		{
			...
			HUD().Font().pFontStat->OutNext("%s%s%sunstrapped     : %s", indent, indent, indent,
											weapon_unstrapped(weapon) ? "+" : "-");
			HUD().Font().pFontStat->OutNext("%s%s%sammo           : %d", indent, indent, indent,
											weapon->GetAmmoElapsed());
			HUD().Font().pFontStat->OutNext("%s%s%smagazine       : %d", indent, indent, indent,
											weapon->GetAmmoMagSize());
			HUD().Font().pFontStat->OutNext("%s%s%stotal ammo     : %d", indent, indent, indent,
	...
	HUD().Font().pFontStat->OutNext("%s%smovement type   : %s (current)", indent, indent,
									movement_type(movement().movement_type()));
	HUD().Font().pFontStat->OutNext("%s%smovement type   : %s (target)", indent, indent,
	...
	HUD().Font().pFontStat->OutNext("%s%shead current    : [%f][%f]", indent, indent,
									movement().head_orientation().current.yaw,
									movement().head_orientation().current.pitch);
	HUD().Font().pFontStat->OutNext("%s%shead target     : [%f][%f]", indent, indent,
									movement().head_orientation().target.yaw,
									movement().head_orientation().target.pitch);
	HUD().Font().pFontStat->OutNext("%s%sbody current    : [%f][%f]", indent, indent,
									movement().body_orientation().current.yaw,
									movement().body_orientation().current.pitch);
	HUD().Font().pFontStat->OutNext("%s%sbody target     : [%f][%f]", indent, indent,
	...
	if (movement().path_type() == MovementManager::ePathTypeGamePath)
	{
		...
		HUD().Font().pFontStat->OutNext("%s%s%scompleted     : %s", indent, indent, indent,
										movement().game_path().completed() ? "+" : "-");
		HUD().Font().pFontStat->OutNext("%s%s%spath size     : %d", indent, indent, indent,
										movement().game_path().path().size());
		HUD().Font().pFontStat->OutNext("%s%s%scurrent point : %d", indent, indent, indent,
										movement().game_path().intermediate_index());
	}

	HUD().Font().pFontStat->OutNext("%s%slevel", indent, indent);
	HUD().Font().pFontStat->OutNext("%s%s%spath size     : %d", indent, indent, indent,
									movement().level_path().path().size());
	HUD().Font().pFontStat->OutNext("%s%s%sstart vertex  : %d", indent, indent, indent,
									movement().level_path().path().empty() ? -1
																		   : movement().level_path().path().front());
	HUD().Font().pFontStat->OutNext("%s%s%sdest vertex   : %d", indent, indent, indent,
									movement().level_path().path().empty() ? -1
																		   : movement().level_path().path().back());

	HUD().Font().pFontStat->OutNext("%s%sdetail", indent, indent);
	HUD().Font().pFontStat->OutNext("%s%s%svelocities    : %d", indent, indent, indent,
									movement().detail().velocities().size());
	HUD().Font().pFontStat->OutNext("%s%s%sextrapolate   : %f", indent, indent, indent,
	...
	if (!movement().detail().path().empty())
	{
		...
		HUD().Font().pFontStat->OutNext("%s%s%sdest point    : [%f][%f][%f]", indent, indent, indent,
										VPUSH(movement().detail().path().back().position));
		HUD().Font().pFontStat->OutNext("%s%s%scurrent point", indent, indent, indent);
		HUD().Font().pFontStat->OutNext("%s%s%s%sindex     : %d", indent, indent, indent, indent,
										movement().detail().curr_travel_point_index());
		HUD().Font().pFontStat->OutNext(
			"%s%s%s%sposition  : [%f][%f][%f]", indent, indent, indent, indent,
		...
		HUD().Font().pFontStat->OutNext("%s%s%s%slinear    : %f", indent, indent, indent, indent,
										current_velocity.linear_velocity);
		HUD().Font().pFontStat->OutNext("%s%s%s%sangular   : %f deg", indent, indent, indent, indent,
										rad2deg(current_velocity.angular_velocity));
		HUD().Font().pFontStat->OutNext("%s%s%s%sangular(R): %f deg", indent, indent, indent, indent,
		...
		HUD().Font().pFontStat->OutNext("%s%s%sspeed(physics): %f", indent, indent, indent,
...
void CAI_Stalker::OnRender()
{
	...
	{
		fvec3 c0 = Position(), c1, t0 = Position(), t1;
		c0.y += 2.f;
...
typedef xr_vector<fvec3> COLLIDE_POINTS;
...
class ray_query_param
{
  public:
	...
	fvec3 m_start_position;
	fvec3 m_direction;
	COLLIDE_POINTS* m_points;

  public:
	IC ray_query_param(CCustomMonster* holder, float power_threshold, float distance, const fvec3& start_position,
					   const fvec3& direction, COLLIDE_POINTS& points)
	{
		m_holder = holder;
		m_power = 1.f;
...
BOOL _ray_query_callback(collide::rq_result& result, LPVOID params)
{
	...
	param->m_points->push_back(fvec3().mad(param->m_start_position, param->m_direction, result.range));
...
void fill_points(CCustomMonster* self, const fvec3& position, const fvec3& direction, float distance,
				 collide::rq_results& rq_storage, COLLIDE_POINTS& points, float& pick_distance)
{
	...
	ray_query_param params(self, self->memory().visual().transparency_threshold(), distance, position, direction,
						   points);
...
void draw_visiblity_rays(CCustomMonster* self, const CObject* object, collide::rq_results& rq_storage)
{
	...
	fvec3 dest_position = item->cp_LAST;
	fvec3 direction = fvec3().sub(dest_position, start_position);
	...
	fvec3 size = fvec3().set(.05f, .05f, .05f);
