...
template <typename src> static void r_vec_q8(src& P, fvec3& vec, const fvec3& min, const fvec3& max)
{
...
/////////////////////////////////16////////////////////////////////////////////////////////////////
static void w_vec_q16(NET_Packet& P, const fvec3& vec, const fvec3& min, const fvec3& max)
{
	P.w_float_q16(vec.x, min.x, max.x);
	P.w_float_q16(vec.y, min.y, max.y);
	P.w_float_q16(vec.z, min.z, max.z);
}
static void r_vec_q16(NET_Packet& P, fvec3& vec, const fvec3& min, const fvec3& max)
{
...
void SPHNetState::net_Export(NET_Packet& P)
{
	...
	// P.w_vec3(torque);
	P.w_vec3(position);
	P.w_vec4(*((fvec4*)&quaternion));
	// P.w_vec4(*((fvec4*)&previous_quaternion));
	P.w_u8((u8)enabled);
}
template <typename src> void SPHNetState::read(src& P)
{
	...
	position = P.r_vec3();
	*((fvec4*)&quaternion) = P.r_vec4();
	previous_quaternion.set(quaternion); // P.r_vec4(*((fvec4*)&previous_quaternion));
	enabled = !!P.r_u8();
}
...
void SPHNetState::net_Save(NET_Packet& P, const fvec3& min, const fvec3& max)
{
	// P.w_vec3(linear_vel);
	// P.w_vec3(angular_vel);
	// P.w_vec3(force);
...
template <typename src> void SPHNetState::read(src& P, const fvec3& min, const fvec3& max)
{
...
void SPHNetState::net_Load(NET_Packet& P, const fvec3& min, const fvec3& max)
{
	VERIFY(!(fsimilar(min.x, max.x) && fsimilar(min.y, max.y) && fsimilar(min.z, max.z)));
	read(P, min, max);
}
void SPHNetState::net_Load(IReader& P, const fvec3& min, const fvec3& max)
{
	VERIFY(!(fsimilar(min.x, max.x) && fsimilar(min.y, max.y) && fsimilar(min.z, max.z)));
	read(P, min, max);
}
...
void SPHBonesData::set_min_max(const fvec3& _min, const fvec3& _max)
{
