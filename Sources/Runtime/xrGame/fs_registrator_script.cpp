...
struct FS_item
{
	...
	LPCSTR ModifDigitOnly()
	{
		...
		time_t t = modif;
		newtime = localtime(&t);
		sprintf_s(buff, "%02d:%02d:%4d %02d:%02d", newtime->tm_mday, newtime->tm_mon + 1, newtime->tm_year + 1900,
...
class FS_file_list_ex
{
  ...
  public:
	...
	u32 Size()
	{
		return m_file_items.size();
	}
	FS_item GetAt(u32 idx)
	{
		return m_file_items[idx];
	}
	void Sort(u32 flags);
};
