...
{
	fvec3 eye_position = fvec3().set(0.f, 0.f, 0.f), eye_direction;
	fmat4x4 eye_matrix;
...
float CVisualMemoryManager::get_visible_value(float distance, float object_distance, float time_delta,
...
bool CVisualMemoryManager::visible(const CGameObject* game_object, float time_delta)
{
	...
	if (distance < object_distance)
	{
		if (object)
		{
			...
			if (object->m_value < 0.f)
				...
				object->m_update_time = Engine.TimeManager.GetGlobalTimeMs();
	...
	if (!object)
	{
		...
		new_object.m_value =
			get_visible_value(distance, object_distance, time_delta, get_object_velocity(game_object, new_object),
							  object_luminocity(game_object));
		clamp(new_object.m_value, 0.f, current_state().m_visibility_threshold + EPS_L);
		new_object.m_update_time = Engine.TimeManager.GetGlobalTimeMs();
...
void CVisualMemoryManager::add_visible_object(const CObject* object, float time_delta, bool fictitious)
{
	...
	if (m_objects->end() == J)
	{
		...
		visible_object.m_first_level_time = Engine.TimeManager.GetGlobalTimeMs();
#endif
...
{
#ifndef MASTER_GOLD
	if (visible_object.m_object && (visible_object.m_object->CLS_ID == CLSID_OBJECT_ACTOR) &&
		psAI_Flags.test(aiIgnoreActor))
		return;
#endif // MASTER_GOLD
...
void CVisualMemoryManager::update(float time_delta)
{
	...
	START_PROFILE("Memory Manager/visuals/update/make_invisible")
	{
		...
		xr_vector<CVisibleObject>::iterator E = m_objects->end();
		for (; I != E; ++I)
			if ((*I).m_level_time + current_state().m_still_visible_time < Engine.TimeManager.GetGlobalTimeMs())
	...
	START_PROFILE("Memory Manager/visuals/update/make_not_yet_visible")
	{
		...
		for (; I != E; ++I)
			if ((*I).m_update_time < Engine.TimeManager.GetGlobalTimeMs())
				(*I).m_value = 0.f;
	}
	STOP_PROFILE
	...
	{
		m_objects->erase(std::remove_if(m_objects->begin(), m_objects->end(), SRemoveOfflinePredicate()),
						 m_objects->end());
	}
