...
void CStalkerAnimationPair::play_global_animation(CKinematicsAnimated* skeleton_animated, PlayCallback callback,
												  CAI_Stalker* object, const bool& use_animation_movement_control)
#else
void CStalkerAnimationPair::play_global_animation(CKinematicsAnimated* skeleton_animated, PlayCallback callback,
												  CAI_Stalker* object, const u32& bone_part,
...
void CStalkerAnimationPair::play(CKinematicsAnimated* skeleton_animated, PlayCallback callback, CAI_Stalker* object,
								 const bool& use_animation_movement_control, bool continue_interrupted_animation)
#else
void CStalkerAnimationPair::play(CKinematicsAnimated* skeleton_animated, PlayCallback callback, CAI_Stalker* object,
								 const bool& use_animation_movement_control, bool continue_interrupted_animation,
								 const u32& bone_part)
#endif
{
	...
	if (actual())
	{
		...
		if (psAI_Flags.is(aiAnimation) && blend())
			Msg				("%6d [%s][%s][%s][%f]",Engine.TimeManager.GetGlobalTimeMs(),m_object_name,m_animation_type_name,*animation()->name(),blend()->timeCurrent);
#endif
#endif
	...
	if (psAI_Flags.is(aiAnimation))
	{
		...
		Msg("%6d [%s][%s][%s][%d] - LOOPED: %d", Engine.TimeManager.GetGlobalTimeMs(), m_object_name, m_animation_type_name, name,
			motion->bone_or_part, !(motion->flags & esmStopAtEnd));
	}
#endif
}
