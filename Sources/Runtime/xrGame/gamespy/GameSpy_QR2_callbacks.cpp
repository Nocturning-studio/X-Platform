...
void __cdecl callback_serverkey(int keyid, void* outbuf, void* userdata)
{
	...
	case MAXPLAYERS_KEY:
		...
		break;
	case GAMETYPE_KEY:
		ADD_KEY_VAL(pServer->game, pQR2, BufferAdd, outbuf, type_name());
		break; //		pQR2->BufferAdd(outbuf, pServer->game->type_name()); break;
	case GAMEMODE_KEY:
	...
	case DEDICATED_KEY:
		pQR2->BufferAdd_Int(outbuf, pServer->IsDedicated());
		break;
	case GAMETYPE_NAME_KEY:
		ADD_KEY_VAL(pServer->game, pQR2, BufferAdd_Int, outbuf, Type());
		break; // pQR2->BufferAdd_Int(outbuf, pServer->game->Type()); break;
	case NUMTEAMS_KEY:
...
void __cdecl callback_playerkey(int keyid, int index, void* outbuf, void* userdata)
{
	...
	case PLAYER__KEY:
		...
		break;
	case PING__KEY:
		pQR2->BufferAdd_Int(outbuf, pCD->ps->ping);
		break;
	case SCORE__KEY:
