...
typedef enum
{
	...
	e_qrnochallengeerror, // no challenge was received from the master - either the master is down, or a firewall is
						  // blocking UDP
	qr2_error_t_count
} qr2_error_t;
...
typedef void(__cdecl* ptPatchCallback)(PTBool available, PTBool mandatory, const char* versionName, int fileID,
									   const char* downloadURL, void* param);

typedef void(__cdecl* ghttpProgressCallback)(GHTTPRequest request, GHTTPState state, const char* buffer,
											 GHTTPByteCount bufferLen, GHTTPByteCount bytesReceived,
											 GHTTPByteCount totalSize, void* param);

typedef GHTTPBool(__cdecl* ghttpCompletedCallback)(GHTTPRequest request, GHTTPResult result, char* buffer,
