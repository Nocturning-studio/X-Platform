...
bool r_eof_IReader(IReader* self)
{
	return (!!self->eof());
}

LPCSTR r_stringZ_IReader(IReader* self)
{
	shared_str temp;
	self->r_stringZ(temp);
	return (*temp);
...
bool r_bool_IReader(IReader* self)
{
	return (!!self->r_u8());
}
