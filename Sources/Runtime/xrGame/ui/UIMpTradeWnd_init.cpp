...
void CUIMpTradeWnd::Init(const shared_str& sectionName, const shared_str& sectionPrice)
{
	...
	m_root_tab_control = xr_new<CUITabControl>();
	...
	m_btn_shop_back = xr_new<CUI3tButton>();
	AttachChild(m_btn_shop_back);
	m_btn_shop_back->SetAutoDelete(true);
	m_btns_preset[0] = xr_new<CUI3tButton>();
	AttachChild(m_btns_preset[0]);
	m_btns_preset[0]->SetAutoDelete(true);
	m_btns_preset[1] = xr_new<CUI3tButton>();
	AttachChild(m_btns_preset[1]);
	m_btns_preset[1]->SetAutoDelete(true);
	m_btns_preset[2] = xr_new<CUI3tButton>();
	AttachChild(m_btns_preset[2]);
	m_btns_preset[2]->SetAutoDelete(true);
	m_btns_preset[3] = xr_new<CUI3tButton>();
	AttachChild(m_btns_preset[3]);
	m_btns_preset[3]->SetAutoDelete(true);
	m_btns_preset[4] = xr_new<CUI3tButton>();
	AttachChild(m_btns_preset[4]);
	m_btns_preset[4]->SetAutoDelete(true);
	m_btns_save_preset[0] = xr_new<CUI3tButton>();
	AttachChild(m_btns_save_preset[0]);
	m_btns_save_preset[0]->SetAutoDelete(true);
	m_btns_save_preset[1] = xr_new<CUI3tButton>();
	AttachChild(m_btns_save_preset[1]);
	m_btns_save_preset[1]->SetAutoDelete(true);
	m_btns_save_preset[2] = xr_new<CUI3tButton>();
	AttachChild(m_btns_save_preset[2]);
	m_btns_save_preset[2]->SetAutoDelete(true);
	m_btn_reset = xr_new<CUI3tButton>();
	AttachChild(m_btn_reset);
	m_btn_reset->SetAutoDelete(true);
	m_btn_sell = xr_new<CUI3tButton>();
	AttachChild(m_btn_sell);
	m_btn_sell->SetAutoDelete(true);

	m_btn_pistol_ammo = xr_new<CUI3tButton>();
	AttachChild(m_btn_pistol_ammo);
	m_btn_pistol_ammo->SetAutoDelete(true);
	m_btn_pistol_silencer = xr_new<CUI3tButton>();
	AttachChild(m_btn_pistol_silencer);
	m_btn_pistol_silencer->SetAutoDelete(true);
	m_btn_rifle_ammo = xr_new<CUI3tButton>();
	AttachChild(m_btn_rifle_ammo);
	m_btn_rifle_ammo->SetAutoDelete(true);
	m_btn_rifle_silencer = xr_new<CUI3tButton>();
	AttachChild(m_btn_rifle_silencer);
	m_btn_rifle_silencer->SetAutoDelete(true);
	m_btn_rifle_scope = xr_new<CUI3tButton>();
	AttachChild(m_btn_rifle_scope);
	m_btn_rifle_scope->SetAutoDelete(true);
	m_btn_rifle_glauncher = xr_new<CUI3tButton>();
	AttachChild(m_btn_rifle_glauncher);
	m_btn_rifle_glauncher->SetAutoDelete(true);
	m_btn_rifle_ammo2 = xr_new<CUI3tButton>();
	AttachChild(m_btn_rifle_ammo2);
	m_btn_rifle_ammo2->SetAutoDelete(true);
	...
	AddCallback("btn_shop_back", BUTTON_CLICKED,
	...
	AddCallback("btn_pistol_ammo", BUTTON_CLICKED,
				CUIWndCallback::void_function(this, &CUIMpTradeWnd::OnBtnPistolAmmoClicked));
	AddCallback("btn_pistol_silencer", BUTTON_CLICKED,
				CUIWndCallback::void_function(this, &CUIMpTradeWnd::OnBtnPistolSilencerClicked));
	AddCallback("btn_rifle_ammo", BUTTON_CLICKED,
				CUIWndCallback::void_function(this, &CUIMpTradeWnd::OnBtnRifleAmmoClicked));
	AddCallback("btn_rifle_silencer", BUTTON_CLICKED,
				CUIWndCallback::void_function(this, &CUIMpTradeWnd::OnBtnRifleSilencerClicked));
	AddCallback("btn_rifle_scope", BUTTON_CLICKED,
				CUIWndCallback::void_function(this, &CUIMpTradeWnd::OnBtnRifleScopeClicked));
	AddCallback("btn_rifle_glauncher", BUTTON_CLICKED,
	...
	m_static_player_money = xr_new<CUIStatic>();
	AttachChild(m_static_player_money);
	m_static_player_money->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xml_doc, "static_player_money", 0, m_static_player_money);

	m_static_curr_items_money = xr_new<CUIStatic>();
	AttachChild(m_static_curr_items_money);
	m_static_curr_items_money->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xml_doc, "static_curr_items_money", 0, m_static_curr_items_money);

	// preset money indicators
	m_static_preset_money[0] = xr_new<CUIStatic>();
	AttachChild(m_static_preset_money[0]);
	m_static_preset_money[0]->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xml_doc, "static_preset_money_last", 0, m_static_preset_money[0]);
	m_static_preset_money[1] = xr_new<CUIStatic>();
	AttachChild(m_static_preset_money[1]);
	m_static_preset_money[1]->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xml_doc, "static_preset_money_1", 0, m_static_preset_money[1]);
	m_static_preset_money[2] = xr_new<CUIStatic>();
	AttachChild(m_static_preset_money[2]);
	m_static_preset_money[2]->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xml_doc, "static_preset_money_2", 0, m_static_preset_money[2]);
	m_static_preset_money[3] = xr_new<CUIStatic>();
	AttachChild(m_static_preset_money[3]);
	m_static_preset_money[3]->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xml_doc, "static_preset_money_3", 0, m_static_preset_money[3]);
	m_static_preset_money[4] = xr_new<CUIStatic>();
	AttachChild(m_static_preset_money[4]);
	m_static_preset_money[4]->SetAutoDelete(true);
	...
	m_static_player_rank = xr_new<CUIStatic>();
	AttachChild(m_static_player_rank);
	m_static_player_rank->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xml_doc, "static_player_rank", 0, m_static_player_rank);

	m_static_item_rank = xr_new<CUIStatic>();
	AttachChild(m_static_item_rank);
	m_static_item_rank->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xml_doc, "static_item_rank", 0, m_static_item_rank);

	m_static_information = xr_new<CUIStatic>();
	AttachChild(m_static_information);
	m_static_information->SetAutoDelete(true);
	CUIXmlInit::InitStatic(xml_doc, "static_info", 0, m_static_information);

	m_static_money_change = xr_new<CUIStatic>();
	AttachChild(m_static_money_change);
	m_static_money_change->SetAutoDelete(true);
