...
void CUIVideoPlayerWnd::Init(CUIXml* doc, LPCSTR start_from)
{
	...
	m_surface = xr_new<CUIStatic>();
	m_surface->SetAutoDelete(true);
	AttachChild(m_surface);
	xml_init.InitStatic(*doc, str, 0, m_surface);

	strconcat(sizeof(str), str, start_from, ":buttons_tab");
	m_tabControl = xr_new<CUITabControl>();
	m_tabControl->SetAutoDelete(true);
	...
	//.    AddCallback
	//("buttons_tab",TAB_CHANGED,boost::bind(&CUIVideoPlayerWnd::OnTabChanged,this,_1,_2));
