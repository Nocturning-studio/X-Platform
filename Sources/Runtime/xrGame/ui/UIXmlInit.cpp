...
bool CUIXmlInit::InitStatic(CUIXml& xml_doc, LPCSTR path, int index, CUIStatic* pWnd)
{
	...
	pWnd->SetClrLightAnim(str_flag, (flag_cyclic) ? true : false, (flag_alpha) ? true : false,
						  (flag_text) ? true : false, (flag_texture) ? true : false);

	str_flag = xml_doc.ReadAttrib(path, index, "transform_anim", "");
	flag_cyclic = xml_doc.ReadAttribInt(path, index, "transform_anim_cyclic", 1);

	pWnd->SetTransformLightAnim(str_flag, (flag_cyclic) ? true : false);
...
bool CUIXmlInit::Init3tButton(CUIXml& xml_doc, const char* path, int index, CUI3tButton* pWnd)
{
	...
	pWnd->SetShadowOffset(fvec2().set(shadowOffsetX, shadowOffsetY));
...
bool CUIXmlInit::InitButton(CUIXml& xml_doc, LPCSTR path, int index, CUIButton* pWnd)
{
	R_ASSERT3(xml_doc.NavigateToNode(path, index), "XML node not found", path);
	...
	pWnd->SetShadowOffset(fvec2().set(shadowOffsetX, shadowOffsetY));
	pWnd->SetPushOffset(fvec2().set(pushOffsetX, pushOffsetY));
