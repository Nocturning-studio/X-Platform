...
MP_BUY_SLOT CUIBuyWnd::GetLocalSlot(u32 slot)
{
	...
	{
	case PISTOL_SLOT:
		return MP_SLOT_PISTOL;
	case RIFLE_SLOT:
		return MP_SLOT_RIFLE;
	case OUTFIT_SLOT:
		return MP_SLOT_OUTFIT;
	default:
		return MP_SLOT_BELT;
	}
}
...
void CUIBuyWnd::ActivatePropertiesBox()
{
	...
	if ((pWeapon || pOutfit) && m_bag.IsInBag(CurrentItem()) &&
		m_bag.CanBuy(CurrentItem()) /*&& m_list[GetLocalSlot(CurrentIItem()->GetSlot())]->ItemsCount()*/)
	{
