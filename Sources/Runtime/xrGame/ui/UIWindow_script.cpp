...
void CUIWindow::script_register(lua_State* L)
{
	module(L)
		[def("GetARGB", &GetARGB), def("GetFontSmall", &GetFontSmall), def("GetFontMedium", &GetFontMedium),
		 ...
		 def("GetTextureName", &get_texture_name), def("GetTextureRect", &get_texture_rect),
		 ...
		 class_<enum_exporter<EUIMessages>>("ui_events")
			 .enum_("events")[
				 ...
				 value("WINDOW_LBUTTON_UP", int(WINDOW_LBUTTON_UP)), value("WINDOW_RBUTTON_UP", int(WINDOW_RBUTTON_UP)),
				 ...
				 // CUITabControl
				 value("TAB_CHANGED", int(TAB_CHANGED)), value("EDIT_TEXT_COMMIT", int(EDIT_TEXT_COMMIT)),
				 ...
				 // CUISleepWnd
				 //				value("SLEEP_WND_PERFORM_BUTTON_CLICKED",
				 // int(SLEEP_WND_PERFORM_BUTTON_CLICKED)),

				 // CUIOutfitSlot
				 value("UNDRESS_OUTFIT", int(UNDRESS_OUTFIT)), value("OUTFIT_RETURNED_BACK", int(OUTFIT_RETURNED_BACK)),
