...
void SBullet::Init(const fvec3& position, const fvec3& direction, float starting_speed, float power, float impulse,
...
void CBulletManager::AddBullet(const fvec3& position, const fvec3& direction, float starting_speed, float power,
...
float SqrDistancePointToSegment(const fvec3& pt, const fvec3& orig, const fvec3& dir)
{
	fvec3 diff;
	diff.sub(pt, orig);
...
void CBulletManager::Render()
{
	...
	for(BulletVecIt it = m_BulletsRendered.begin(); it != m_BulletsRendered.end(); it++)
	{
		...
		float dist2segSqr =
			SqrDistancePointToSegment(Engine.RenderView.Position, bullet->pos, fvec3().mul(bullet->dir, length));
		//---------------------------------------------
...
void CBulletManager::CommitRenderSet() // @ the end of frame
{
	...
	Engine.ThreadManager.AddParallelTask([this]() { UpdateWorkload(); });
}
...
void CBulletManager::RegisterEvent(EventType Type, BOOL _dynamic, SBullet* bullet, const fvec3& end_point,
