...
static fvec3 vFootCenter;
static fvec3 vFootExt;
...
CActor::CActor() : CEntityAlive()
{
...
#ifdef DEBUG
	Engine.Events.Render.Add(this, REG_PRIORITY_LOW);
#endif
	...
	hFriendlyIndicator.create(FVF::F_LIT, RenderBackendLegacy.Vertex.Buffer(), RenderBackendLegacy.QuadIB);
...
#pragma todo("NSDeathman to NSDeathman : Разобраться с небоходимостью перезапуска игры после изменения состояния cam_psp")
	...
	//-----------------------------------------
	m_AutoPickUp_AABB =
		READ_IF_EXISTS(pSettings, r_fvector3, section, "AutoPickUp_AABB", fvec3().set(0.02f, 0.02f, 0.02f));
	m_AutoPickUp_AABB_Offset =
		READ_IF_EXISTS(pSettings, r_fvector3, section, "AutoPickUp_AABB_offs", fvec3().set(0, 0, 0));
...
void CActor::PHHit(float P, fvec3& dir, CObject* who, s16 element, fvec3 p_in_object_space, float impulse,
...
void CActor::Hit(SHit* pHDS)
{
	...
	if (ph_dbg_draw_mask.test(phDbgCharacterControl))
	{
		DBG_OpenCashedDraw();
		fvec3 to;
		to.add(Position(), fvec3().mul(HDS.dir, HDS.phys_impulse()));
...
void CActor::HitMark(float P, fvec3 dir, CObject* who, s16 element, fvec3 position_in_bone_space, float impulse,
...
void CActor::HitSignal(float perc, fvec3& vLocalDir, CObject* who, s16 element)
{
	if (g_Alive())
	{
		...
		// Если разница углов <= 90 (импульс сонаправлен взгляду), значит удар в спину -> берем анимацию 1.
		// Иначе удар спереди -> берем анимацию 0.
		MotionID motion_ID =
			m_anims->m_normal
...
void CActor::g_Physics(fvec3& _accel, float jump, float dt)
{
	// Correct accel
	fvec3 accel;
	accel.set(_accel);
	...
	if (Local() && g_Alive())
	{
		...
		if (!fis_zero(character_physics_support()->movement()->gcontact_HealthLost))
		{
			...
			fvec3 hdir;
			di->HitDir(hdir);
			SetHitInfo(this, NULL, 0, fvec3().set(0, 0, 0), hdir);
			//				Hit
...
void CActor::RenderIndicator(fvec3 dpos, float r1, float r2, ref_shader IndShader)
{
	...
	FVF::LIT* pv_start = (FVF::LIT*)RenderBackendLegacy.Vertex.Lock(4, hFriendlyIndicator->vb_stride, dwOffset);
	FVF::LIT* pv = pv_start;
	// base rect
	...
	c.invert(a);
	d.invert(b);
	pv->set(d.x + pos.x, d.y + pos.y, d.z + pos.z, 0xffffffff, 0.f, 1.f);
	pv++;
	pv->set(a.x + pos.x, a.y + pos.y, a.z + pos.z, 0xffffffff, 0.f, 0.f);
	pv++;
	pv->set(c.x + pos.x, c.y + pos.y, c.z + pos.z, 0xffffffff, 1.f, 1.f);
	pv++;
	pv->set(b.x + pos.x, b.y + pos.y, b.z + pos.z, 0xffffffff, 1.f, 0.f);
	pv++;
	// render
	dwCount = u32(pv - pv_start);
	RenderBackendLegacy.Vertex.Unlock(dwCount, hFriendlyIndicator->vb_stride);

	RenderBackendLegacy.set_transform_world(Fidentity);
	RenderBackendLegacy.set_Shader(IndShader);
	RenderBackendLegacy.set_Geometry(hFriendlyIndicator);
	RenderBackendLegacy.Render(D3DPT_TRIANGLESTRIP, dwOffset, 0, dwCount, 0, 2);
};
...
void CActor::MoveArtefactBelt(const CArtefact* artefact, bool on_belt)
{
	...
	{
		xr_vector<const CArtefact*>::iterator it =
			std::remove(m_ArtefactsOnBelt.begin(), m_ArtefactsOnBelt.end(), artefact);
...
fvec3 CActor::GetMissileOffset() const
{
	return m_vMissileOffset;
}

void CActor::SetMissileOffset(const fvec3& vNewOffset)
{
...
float CActor::GetMass()
{
	return g_Alive()		 ? character_physics_support()->movement()->GetMass()
		   : m_pPhysicsShell ? m_pPhysicsShell->getMass()
							 : 0;
}
