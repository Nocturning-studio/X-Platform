...
void CSightManager::SetPointLookAngles(const fvec3& tPosition, float& yaw, float& pitch, const CGameObject* object)
{
	fvec3 tTemp;
...
void CSightManager::SetFirePointLookAngles(const fvec3& tPosition, float& yaw, float& pitch,
										   const CGameObject* object)
{
	...
	if (object && object->use_center_to_aim())
	{
		...
		m_object->Transform().transform_tiny(tTemp, fvec3().set(.2f, tTemp.y - m_object->Position().y, 0.f));
#else
...
#endif
		tTemp.sub(tPosition, fvec3(tTemp));
...
bool CSightManager::bfIf_I_SeePosition(fvec3 tPosition) const
{
	float yaw, pitch;
	fvec3 tVector;
