											...
											const float3& position)
{
	...
	//	Msg
	//("[%6d][%s][%f][%f][%f]",Engine.TimeManager.GetGlobalTimeMs(),object().name_replace(),VPUSH(m_destination.m_position));
}
...
{
	//	Msg
	//("[%6d][%s][%s]",Engine.TimeManager.GetGlobalTimeMs(),object().name_replace(),*task->patrol_path_name());
	target(*task);
}
...
void CALifeMonsterDetailPathManager::update()
{
	...
	//		Msg							("[detail::update][%6d][%s]",Engine.TimeManager.GetGlobalTimeMs(),object().name_replace());
...
void CALifeMonsterDetailPathManager::follow_path(const ALife::_TIME_ID& time_delta)
{
	...
	for (; m_path.size() > 1;)
	{
		...
		//%d",Engine.TimeManager.GetGlobalTimeMs(),object().name_replace(),object().m_tGraphID,(GameGraph::_GRAPH_ID)m_path.back());
...
float3 CALifeMonsterDetailPathManager::draw_level_position() const
{
	if (path().empty())
	...
	float3 current_vertex = current->level_point();
	float3 next_vertex = next->level_point();
	float3 direction = float3().sub(next_vertex, current_vertex);
	direction.normalize();
