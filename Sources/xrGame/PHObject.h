...
class CPHObject : public ISpatial
{
	...
	u8 m_check_count;
	Flags<CLClassBits> m_collide_class_bits;
  ...
  protected:
	...
	void SetRayMotions()
	{
		m_flags.set(fl_ray_motions, TRUE);
	}
	void UnsetRayMotions()
	{
		m_flags.set(fl_ray_motions, FALSE);
	}

	void SetPrefereExactIntegration()
	{
		m_island.SetPrefereExactIntegration();
	}

	CPHObject* SelfPointer()
	{
		return this;
	}
  ...
  public:
	...
	void IslandStep(dReal step)
	{
		m_island.Step(step);
	}
	void MergeIsland(CPHObject* obj)
	{
		m_island.Merge(&obj->m_island);
	}
	CPHIsland& Island()
	{
		return m_island;
	}
	dWorldID DActiveWorld()
	{
		return m_island.DActiveWorld();
	}
	CPHIsland* DActiveIsland()
	{
		return m_island.DActiveIsland();
	}
	dWorldID DWorld()
	{
		return m_island.DWorld();
	}
	...
	void NetInterpolationON()
	{
		m_flags.set(st_net_interpolation, TRUE);
	}
	void NetInterpolationOFF()
	{
		m_flags.set(st_net_interpolation, TRUE);
	}
	bool NetInterpolation()
	{
	...
	IC Flags<CLClassBits>& collide_class_bits()
	{
	...
	IC const Flags<CLClassBits>& collide_class_bits() const
	{
		return m_collide_class_bits;
	}
