...
void CAttachableItem::reload(LPCSTR section)
{
	...
	float3 angle_offset = pSettings->r_fvector3(section, "attach_angle_offset");
	float3 position_offset = pSettings->r_fvector3(section, "attach_position_offset");
	...
	//!!(READ_IF_EXISTS(pSettings,r_bool,section,"auto_attach",TRUE)));
	enable(false);
#ifdef DEBUG
	m_valid = true;
#endif
...
{
	//	VERIFY							(m_valid);
	::Render->set_Transform(&object().Transform());
