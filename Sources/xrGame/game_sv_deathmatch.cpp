	...
	case KR_RIVAL: {
		...
		case KT_HIT: {
			...
			case SKT_HEADSHOT: {
				...
				Player_AddBonusMoney(pKiller, READ_IF_EXISTS(pSettings, r_s32, "mp_bonus_money", "headshot", 0),
									 SKT_HEADSHOT);
			}
			break;
			case SKT_BACKSTAB: {
				Player_AddBonusMoney(pKiller, READ_IF_EXISTS(pSettings, r_s32, "mp_bonus_money", "backstab", 0),
									 SKT_BACKSTAB);
			}
			break;
			default: {
...
void game_sv_Deathmatch::Update()
{
	...
	case GAME_PHASE_INPROGRESS: {
		...
		if (m_bSpectatorMode)
		{
			if (!m_pSM_CurViewEntity || m_pSM_CurViewEntity->CLS_ID != CLSID_OBJECT_ACTOR ||
...
void game_sv_Deathmatch::OnRender()
{
	...
	{
		fvec3 v0 = rpoints[0][i0].P;
		for (int i1=i0+1; i1<int(rpoints[0].size()); i1++)
		{
			fvec3 v1 = rpoints[0][i1].P;
			bool			failed =
	!m_graph_engine->search(*m_level_graph,m_level_graph->vertex(u32(-1),v0),m_level_graph->vertex(u32(-1),v1),&xPath,GraphEngineSpace::CBaseParameters());
			...
			for ( ; I != E; ++I) {
				...
				if (I!=E-1)
				{
					fvec3 p0 = m_level_graph->vertex_position(*I);
					fvec3 p1 = m_level_graph->vertex_position(*(I+1));
