...
void CUIStatsPlayerList::Update()
{
	...
	if (GameID() == GAME_ARTEFACTHUNT && !m_bSpectator)
	{
		...
		sprintf_s(teaminfo, "%s: %u, %s: %u, %s: %d", *st.translate("mp_artefacts_upcase"), pl_artefacts,
	...
	else if (GameID() == GAME_TEAMDEATHMATCH && !m_bSpectator)
	{
		...
		sprintf_s(teaminfo, "%s: %d, %s: %u", *st.translate("mp_frags_upcase"), pl_frags, *st.translate("mp_players"),
				  pl_count);
		m_header_text->SetText(teaminfo);
	}
