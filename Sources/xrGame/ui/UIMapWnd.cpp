...
void CUIMapWnd::Init(LPCSTR xml_name, LPCSTR start_from)
{
	...
	m_UIMainScrollH->SetStepSize(std::max(1, iFloor(m_UILevelFrame->GetWidth() / 10)));
	...
	m_UIMainScrollV->Init(r.right + SCROLLBARS_SHIFT, r.top, m_UIMainScrollH->GetWndRect().bottom - r.top, false,
						  "pda");
	m_UIMainScrollV->SetWindowName("scroll_v");
	m_UIMainScrollV->SetStepSize(std::max(1, iFloor(m_UILevelFrame->GetHeight() / 10)));
	...
	if (uiXml.NavigateToNode(pth, 0))
	{
		...
		AddCallback(*m_ToolBar[btnIndex]->WindowName(), BUTTON_CLICKED,
					CUIWndCallback::void_function(this, &CUIMapWnd::OnToolGlobalMapClicked));
	}
	...
	if (uiXml.NavigateToNode(pth, 0))
	{
		...
		Register(m_ToolBar[btnIndex]);
		AddCallback(*m_ToolBar[btnIndex]->WindowName(), BUTTON_CLICKED,
					CUIWndCallback::void_function(this, &CUIMapWnd::OnToolActorClicked));
	}
	...
	if (uiXml.NavigateToNode(pth, 0))
	{
		...
		Register(m_ToolBar[btnIndex]);
		AddCallback(*m_ToolBar[btnIndex]->WindowName(), BUTTON_CLICKED,
					CUIWndCallback::void_function(this, &CUIMapWnd::OnToolZoomInClicked));
	}
	btnIndex = eZoomOut;
	...
	if (uiXml.NavigateToNode(pth, 0))
	{
		...
		Register(m_ToolBar[btnIndex]);
		AddCallback(*m_ToolBar[btnIndex]->WindowName(), BUTTON_CLICKED,
					CUIWndCallback::void_function(this, &CUIMapWnd::OnToolZoomOutClicked));
	}
	/*
		btnIndex		= eAddSpot;
		...
		if(uiXml.NavigateToNode(pth,0) && IsGameTypeSingle() ){
			...
			Register						(m_ToolBar[btnIndex]);
			AddCallback
	   (*m_ToolBar[btnIndex]->WindowName(),BUTTON_CLICKED,CUIWndCallback::void_function(&CUIMapWnd::OnToolAddSpotClicked,this,_1,_2));
		}
		...
		if(uiXml.NavigateToNode(pth,0 ) && IsGameTypeSingle() ){
			...
			Register						(m_ToolBar[btnIndex]);
			AddCallback
	   (*m_ToolBar[btnIndex]->WindowName(),BUTTON_CLICKED,CUIWndCallback::void_function(&CUIMapWnd::OnToolRemoveSpotClicked,this,_1,_2));
		}
		...
		if(uiXml.NavigateToNode(pth,0) && IsGameTypeSingle() ){
			...
			Register						(m_ToolBar[btnIndex]);
			AddCallback
	   (*m_ToolBar[btnIndex]->WindowName(),BUTTON_CLICKED,CUIWndCallback::void_function(&CUIMapWnd::OnToolHighlightSpotClicked,this,_1,_2));
		}
	*/
...
void CUIMapWnd::SetTargetMap(const shared_str& name, const fvec2& pos, bool bZoomIn)
{
...
void CUIMapWnd::SetTargetMap(CUICustomMap* m, const fvec2& pos, bool bZoomIn)
{
	m_tgtMap = m;
...
bool CUIMapWnd::OnMouse(float x, float y, EUIMessages mouse_action)
{
	...
	if (((mouse_action == WINDOW_LBUTTON_DOWN) && (m_flags.is_any(lmZoomIn + lmZoomOut))) ||
		(mouse_action == WINDOW_MOUSE_WHEEL_DOWN) || (mouse_action == WINDOW_MOUSE_WHEEL_UP))
	{
...
{
	fvec2 w_pos = GlobalMap()->GetWndPos();
...
void CUIMapWnd::OnToolActorClicked(CUIWindow*, void*)
{
	...
	fvec3 v = Level().CurrentEntity()->Position();
	fvec2 v2;
	v2.set(v.x, v.z);
...
void CUIMapWnd::AddUserSpot			(CUILevelMap* lm)
{
	...
	fvec2 cursor_pos = GetUICursor()->GetPos();
	fvec2 _p;lm->GetAbsolutePos(_p);
	cursor_pos.sub					(_p);
	fvec2 p =					lm->ConvertLocalToReal(cursor_pos);
	fvec3 pos;
