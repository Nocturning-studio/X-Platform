...
CUITaskRootItem::CUITaskRootItem(CUIEventsWnd* w) : inherited(w)
{
	Init();
}
...
void CUITaskRootItem::Init()
{
	...
	AddCallback("m_switchDescriptionBtn", BUTTON_CLICKED,
				CUIWndCallback::void_function(this, &CUITaskRootItem::OnSwitchDescriptionClicked));
...
void CUITaskRootItem::SetGameTask(CGameTask* gt, u16 obj_idx)
{
	...
	m_captionTime->SetWndPos(m_captionTime->GetWndPos().x,
							 m_captionStatic->GetWndPos().y + m_captionStatic->GetHeight() + 3.0f);

	float h = std::max(m_taskImage->GetWndPos().y + m_taskImage->GetHeight(),
				   m_captionTime->GetWndPos().y + m_captionTime->GetHeight());
...
{
	m_switchDescriptionBtn->SetButtonMode(m_EventsWnd->GetDescriptionMode() ? CUIButton::BUTTON_PUSHED
																			: CUIButton::BUTTON_NORMAL);
...
CUITaskSubItem::CUITaskSubItem(CUIEventsWnd* w) : inherited(w)
{
	Init();
}
...
void CUITaskSubItem::Init()
{
	...
	AddCallback("m_showDescriptionBtn", BUTTON_CLICKED,
				CUIWndCallback::void_function(this, &CUITaskSubItem::OnShowDescriptionClicked));
...
void  CUIUserTaskItem::Init					()
{
	...
	m_image							= xr_new<CUIStatic>();		m_image->SetAutoDelete(true);
AttachChild(m_image);

	m_descriptionStatic				= xr_new<CUIStatic>();		m_descriptionStatic->SetAutoDelete(true);
AttachChild(m_descriptionStatic);

	m_captionStatic					= xr_new<CUIStatic>();		m_captionStatic->SetAutoDelete(true);
AttachChild(m_captionStatic);


	m_showLocationBtn				= xr_new<CUI3tButton>();	m_showLocationBtn->SetAutoDelete(true);
AttachChild(m_showLocationBtn); m_showLocationBtn->				SetWindowName("m_showLocationBtn"); Register
(m_showLocationBtn); AddCallback
(m_showLocationBtn->WindowName(),BUTTON_CLICKED,boost::bind(&CUIUserTaskItem::OnShowLocationClicked,this));

	m_showPointerBtn				= xr_new<CUI3tButton>();	m_showPointerBtn->SetAutoDelete(true);
AttachChild(m_showPointerBtn); m_showPointerBtn->				SetWindowName("m_showPointerBtn"); Register
(m_showPointerBtn); AddCallback
(m_showPointerBtn->WindowName(),BUTTON_CLICKED,boost::bind(&CUIUserTaskItem::OnShowPointerClicked,this));


	m_editTextBtn					= xr_new<CUI3tButton>();	m_editTextBtn->SetAutoDelete(true);
AttachChild(m_editTextBtn); m_editTextBtn->					SetWindowName("m_editTextBtn"); Register
(m_editTextBtn); AddCallback						(m_editTextBtn->WindowName(),
BUTTON_CLICKED,boost::bind(&CUIUserTaskItem::OnEditTextClicked,this));

	m_removeBtn						= xr_new<CUI3tButton>();	m_removeBtn->SetAutoDelete(true);
AttachChild(m_removeBtn); m_removeBtn->					SetWindowName("m_removeBtn"); Register
(m_removeBtn); AddCallback						(m_removeBtn->WindowName(),
BUTTON_CLICKED,boost::bind(&CUIUserTaskItem::OnRemoveClicked,this));
...
void CUIUserTaskItem::SetGameTask				(CGameTask* gt, u16 obj_idx)
{
	...
	float h1 = std::max( m_image->GetWndPos().y+m_image->GetHeight(),
	...
	float h = std::max(	m_image->GetWndPos().y+m_image->GetHeight(),
					m_descriptionStatic->GetWndPos().y+ m_descriptionStatic->GetHeight());
...
void CUIUserTaskEditWnd::Init					()
{
	...
	m_btnOk				= xr_new<CUI3tButton>();	m_btnOk->SetAutoDelete(true);
m_background->AttachChild(m_btnOk); m_btnOk->SetWindowName("m_btnOk"); Register			(m_btnOk); AddCallback
(m_btnOk->WindowName(),BUTTON_CLICKED,boost::bind(&CUIUserTaskEditWnd::OnOk,this));

	m_btnCancel			= xr_new<CUI3tButton>();	m_btnCancel->SetAutoDelete(true);
m_background->AttachChild(m_btnCancel); m_btnCancel->SetWindowName("m_btnCancel"); Register			(m_btnCancel);
	AddCallback			(m_btnCancel->WindowName(),BUTTON_CLICKED,boost::bind(&CUIUserTaskEditWnd::OnCancel,this));

	m_editCaption		= xr_new<CUIEditBox>();			m_editCaption->SetAutoDelete(true);
m_background->AttachChild(m_editCaption); m_editDescription	= xr_new<CUIEditBoxEx>();
m_editDescription->SetAutoDelete(true); m_background->AttachChild(m_editDescription);
