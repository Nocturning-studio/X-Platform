...
void CMapActionZoomControl::init_internal()
{
	...
	if (bZoom && bMove)
		m_endMovingTime += std::max(map_zoom_time, dist / map_resize_speed);
	else if (bZoom)
		m_endMovingTime += map_zoom_time;
	else if (bMove)
		m_endMovingTime += std::max(dist / map_resize_speed, min_move_time);
}
...
void CMapActionZoomControl::execute()
{
	...
	float gt = Engine.TimeManager.GetGlobalTime();
	float time_to = m_endMovingTime - gt;
	float dt = std::min(Engine.TimeManager.GetDeltaTime(), time_to);
...
void CMapActionMinimize::initialize()
{
	...
	inherited::initialize();
	m_endMovingTime = Engine.TimeManager.GetGlobalTime() + map_zoom_time;
}
