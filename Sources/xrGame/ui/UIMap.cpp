...
fvec2 CUICustomMap::ConvertLocalToReal(const fvec2& src)
{
	fvec2 res;
...
fvec2 CUICustomMap::ConvertRealToLocal(const fvec2& src) // meters->pixels (relatively own left-top pos)
{
	fvec2 res;
	if (!Heading())
	{
	...
	{
		fvec2 heading_pivot = GetStaticItem()->GetHeadingPivot();
...
fvec2 CUICustomMap::ConvertRealToLocalNoTransform(
	const fvec2& src) // meters->pixels (relatively own left-top pos)
{
	fvec2 res;
...
bool CUICustomMap::GetPointerTo(const fvec2& src, float item_radius, fvec2& pos, float& heading)
{
...
void CUICustomMap::SetActivePoint(const fvec3& vNewPoint)
{
	fvec2 pos;
	...
	fvec2 pos_on_map = ConvertRealToLocalNoTransform(pos);
	Frect map_abs_rect;
...
fvec2 CUIGlobalMap::ConvertRealToLocal(const fvec2& src) // pixels->pixels (relatively own left-top pos)
{
	fvec2 res;
	res.x = (src.x - m_BoundRect.lt.x) * GetCurrentZoom();
...
void CUIGlobalMap::MoveWndDelta(const fvec2& d)
{
...
float CUIGlobalMap::CalcOpenRect(const fvec2& center_point, Frect& map_desired_rect, float tgt_zoom)
{
	fvec2 new_center_pt;
...
{
	inherited::Init(name, gameLtx, sh_name);
	fvec4 tmp = gameLtx.r_fvector4(MapName(), "global_rect");
	...
	/*
		if(gameLtx.line_exist(MapName(),"anomalies_texture")){
			...
			fvec4 tmp						= gameLtx.r_fvector4(MapName(),"anomalies_texture_rect"); //lt,wh
