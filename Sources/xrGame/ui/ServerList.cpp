...
void CServerList::UpdateSizes()
{
	...
	float2 pos = m_edit_gs_filter.GetWndPos();
...
void CServerList::InitFromXml(CUIXml& xml_doc, LPCSTR path)
{
	...
	CUIXmlInit::InitListWnd(xml_doc, strconcat(sizeof(buf), buf, path, ":list_server_properties"), 0,
							&m_list[LST_SRV_PROP]);
	...
	CUIXmlInit::InitFrameWindow(xml_doc, strconcat(sizeof(buf), buf, path, ":list_players_list:frame"), 0,
								&m_frame[LST_PLAYERS]);
	CUIXmlInit::InitFont(xml_doc, strconcat(sizeof(buf), buf, path, ":list_item:text"), 0, m_itemInfo.color,
						 m_itemInfo.font);
