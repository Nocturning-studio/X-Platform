...
void CUIMainIngameWnd::SetAmmoIcon(const shared_str& sect_name)
{
	...
	UIWeaponIcon.GetUIStaticItem().SetOriginalRect((iXPos * INV_GRID_WIDTH), (iYPos * INV_GRID_HEIGHT),
	...
	if (g_bHudAdjustMode)
	{
		...
		if (1 == g_bHudAdjustMode) // zoom offset
		{
			...
			case DIK_P:
				...
				sprintf_s(tmpStr, "zoom_offset\t\t\t= %f,%f,%f", pWpnHud->ZoomOffset().x, pWpnHud->ZoomOffset().y,
		...
		else if (3 == g_bHudAdjustMode) // MissileOffset
		{
			...
			case DIK_P:
				...
				sprintf_s(tmpStr, "missile_throw_offset\t\t\t= %f,%f,%f", pActor->GetMissileOffset().x,
						  pActor->GetMissileOffset().y, pActor->GetMissileOffset().z);
...
void CUIMainIngameWnd::AnimateContacts(bool b_snd)
{
	...
	if (b_snd)
		HUD_SOUND::PlaySound(m_contactSnd, float3().set(0, 0, 0), 0, true);
}
...
void CUIMainIngameWnd::UpdatePickUpItem()
{
	...
	UIPickUpItemIcon.GetUIStaticItem().SetOriginalRect(
		float(m_iXPos * INV_GRID_WIDTH), float(m_iYPos * INV_GRID_HEIGHT), float(m_iGridWidth * INV_GRID_WIDTH),
...
void CUIMainIngameWnd::draw_adjust_mode()
{
	if (g_bHudAdjustMode && m_pWeapon) // draw firePoint,ShellPoint etc
	{
		...
		if (bCamFirstEye)
		{
			...
			float4x4& fire_mat = V->LL_GetTransform(u16(pWpnHud->FireBone()));
			float4x4& parent = pWpnHud->Transform();

			const float3& fp = pWpnHud->FirePoint();
			const float3& fp2 = pWpnHud->FirePoint2();
			const float3& sp = pWpnHud->ShellPoint();
		...
		{
			float3 FP = m_pWeapon->get_CurrentFirePoint();
			float3 FP2 = m_pWeapon->get_CurrentFirePoint2();
			float3 SP = m_pWeapon->get_LastSP();
			RenderBackend.dbg_DrawAABB(FP, 0.01f, 0.01f, 0.01f, D3DCOLOR_XRGB(255, 0, 0));
			RenderBackend.dbg_DrawAABB(FP2, 0.02f, 0.02f, 0.02f, D3DCOLOR_XRGB(0, 0, 255));
