// exxZERO Time Stamp AddIn. Document modified at : Thursday, March 07, 2002 14:13:00 , by user : Oles , from computer :
// OLES HUDCursor.cpp: implementation of the CHUDTarget class.
//
//////////////////////////////////////////////////////////////////////
			...
			if (IsGameTypeSingle())
			{
				...
				if (E && E->g_Alive() && !E->cast_base_monster())
				{
					...
					if (our_inv_owner && others_inv_owner)
					{
						...
						{
							CStringTable strtbl;
							F->SetColor(color_set_alpha(C, u8(iFloor(255.f * (fuzzyShowInfo - 0.5f) * 2.f))));
				...
				if (E && (E->GetfHealth() > 0))
				{
					if (pCurEnt && GameID() == GAME_SINGLE)
					{
						...
						if (fuzzyShowInfo > 0.5f)
						{
							...
							F->SetColor(color_set_alpha(C, alpha_b));
