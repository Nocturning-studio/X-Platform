...
void CCustomDetector::UpdateCL()
{
	...
	for (it = m_ZoneInfoMap.begin(); m_ZoneInfoMap.end() != it; ++it)
	{
		...
		zone_info.cur_freq =
			zone_type.min_freq + (zone_type.max_freq - zone_type.min_freq) * fRelPow * fRelPow * fRelPow * fRelPow;
		...
		if ((float)zone_info.snd_time > current_snd_time)
		{
			...
			HUD_SOUND::PlaySound(zone_type.detect_snds, float3().set(0, 0, 0), this, true, false);
		}
		else
...
void CCustomDetector::UpdateNightVisionMode()
{
	...
	bool bOn = bNightVision && m_pCurrentActor && m_pCurrentActor == Level().CurrentViewEntity() && IsWorking() &&
