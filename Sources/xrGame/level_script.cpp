...
void CLevel::script_register(lua_State* L)
{
	...
	module(L, "level")[
		...
		def("cover_in_direction", cover_in_direction), def("vertex_in_direction", vertex_in_direction),
		def("rain_factor", rain_factor), def("patrol_path_exists", patrol_path_exists),
		...
		def("physics_world", &physics_world), def("get_snd_volume", &get_snd_volume),
		...
		module(L, "actor_stats")[def("add_points", &add_actor_points), def("add_points_str", &add_actor_points_str),
								 def("get_points", &get_actor_points), def("add_to_ranking", &add_human_to_top_list),
								 def("remove_from_ranking", &remove_human_from_top_list),
								 def("get_actor_ranking", &get_actor_ranking)];

	module(L)[def("command_line", &command_line), def("IsGameTypeSingle", &IsGameTypeSingle)];

	module(L, "relation_registry")[def("community_goodwill", &g_community_goodwill),
