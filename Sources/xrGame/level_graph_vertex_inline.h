...
IC float CLevelGraph::distance(const fvec3& position, const fvec3& point0, const fvec3& point1) const
{
	fvec3 c, V;
...
IC void CLevelGraph::project_point(const Fplane& plane, fvec3& point) const
{
...
IC float CLevelGraph::distance(const fvec3& position, const u32 vertex_id) const
{
	return (distance(position, vertex(vertex_id)));
}

IC float CLevelGraph::distance(const u32 vertex_id, const fvec3& position) const
{
...
IC bool CLevelGraph::similar(const fvec3& tPoint0, const fvec3& tPoint1) const
{
	return ((std::abs(tPoint0.x - tPoint1.x) < EPS_L) && (std::abs(tPoint0.z - tPoint1.z) < EPS_L));
}

IC bool CLevelGraph::inside(const fvec3& tPoint, const CLevelGraph::SContour& tContour) const
{
...
{
	fvec3 vertex_position = this->vertex_position(vertex->p);
...
IC void CLevelGraph::nearest(fvec3& destination, const fvec3& position, const CLevelGraph::SContour& contour) const
{
...
IC bool CLevelGraph::intersect(fvec3& dst, const fvec3& v1, const fvec3& v2, const fvec3& v3,
...
IC u32 CLevelGraph::check_position_in_direction(u32 start_vertex_id, const fvec3& start_position,
												const fvec3& finish_position) const
{
	return (check_position_in_direction(start_vertex_id, fvec2().set(start_position.x, start_position.z),
										fvec2().set(finish_position.x, finish_position.z)));
}

IC bool CLevelGraph::check_vertex_in_direction(u32 start_vertex_id, const fvec3& start_position,
											   u32 finish_vertex_id) const
{
	return (check_vertex_in_direction(start_vertex_id, fvec2().set(start_position.x, start_position.z),
									  finish_vertex_id));
}
