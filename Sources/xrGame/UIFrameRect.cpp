...
void CUIFrameRect::UpdateSize()
{
	...
	fvec2 _bk, _lt, _lb, _rb, _rt, _l, _r, _t, _b;
	...
	tile_x = std::max(tile_x, 0);
	tile_y = iFloor(size_left / ts.y);
	tile_y = std::max(tile_y, 0);
...
void CUIFrameRect::SetWndSize(const fvec2& size)
{
...
void CUIFrameRect::SetWndPos(const fvec2& pos)
{
