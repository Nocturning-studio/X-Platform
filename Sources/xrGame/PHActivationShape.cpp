...
static float max_depth_shape = 0.f;
static float friction_factor_shape = 0.f;
...
void ActivateTestDepthCallback(bool& do_colide, bool bo1, dContact& c, SGameMtl* material_1, SGameMtl* material_2)
{
	...
	if (do_colide && !material_1->Flags.test(SGameMtl::flPassable) && !material_2->Flags.test(SGameMtl::flPassable))
	{
		...
		save_max(max_depth_shape, test_depth);
		c.surface.mu *= friction_factor_shape;

		c.surface.soft_cfm = cfm_shape;
		c.surface.soft_erp = erp_shape;
	}
}
...
void CPHActivationShape::Create(const fvec3 start_pos, const fvec3 start_size, CPhysicsShellHolder* ref_obj,
...
bool CPHActivationShape::Activate(const fvec3 need_size, u16 steps, float max_displacement, float max_rotation,
...
const fvec3& CPHActivationShape::Position()
{
	return cast_fv(dBodyGetPosition(m_body));
}
void CPHActivationShape::Size(fvec3& size)
{
...
void CPHActivationShape::set_rotation(const fmat4x4& sof)
{
	dMatrix3 rot;
