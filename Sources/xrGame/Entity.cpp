...
void CEntity::Load(LPCSTR section)
{
	...
	m_fMorale = READ_IF_EXISTS(pSettings, r_float, section, "morale", 66.f);
...
BOOL CEntity::net_Spawn(CSE_Abstract* DC)
{
	...
	if (!g_Alive())
	{
		m_level_death_time = Engine.TimeManager.GetGlobalTimeMs();
...
void CEntity::shedule_Update(u32 dt)
{
	...
	{
		if (Engine.TimeManager.GetGlobalTimeMs() > m_level_death_time + FORGET_KILLER_TIME)
		{
			m_killer_id = u16(-1);
			NET_Packet P;
