...
bool CMovementManager::actual_all() const
{
	...
	case ePathTypeLevelPath:
		return (level_path().actual() && detail().actual());
	case ePathTypePatrolPath:
		return (patrol().actual() && level_path().actual() && detail().actual());
	case ePathTypeNoPath:
...
void CMovementManager::teleport(u32 game_vertex_id)
{
	...
	float3 position = ai().game_graph().vertex(_game_vertex_id)->level_point();
...
{
	//	Msg								("[%6d][%s][on_restrictions_change]",Engine.TimeManager.GetGlobalTimeMs(),*object().cName());
	m_path_actuality = false;
	level_path_builder().remove();
	detail_path_builder().remove();
...
void CMovementManager::on_frame(CPHMovementControl* movement_control, float3& dest_position)
{
