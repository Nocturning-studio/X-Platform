...
bool CInventory::Slot(PIItem pIItem, bool bNotActivate)
{
	...
	if (((m_iActiveSlot == pIItem->GetSlot()) ||
		 (m_iActiveSlot == NO_ACTIVE_SLOT) && m_iNextActiveSlot == NO_ACTIVE_SLOT) &&
		(!bNotActivate))
		Activate(pIItem->GetSlot());
...
bool CInventory::Activate(u32 slot, EActivationReason reason, bool bForce)
{
	...
	if (m_iActiveSlot == slot || (m_iNextActiveSlot == slot && m_iActiveSlot != NO_ACTIVE_SLOT &&
								  m_slots[m_iActiveSlot].m_pIItem && m_slots[m_iActiveSlot].m_pIItem->IsHiding()))
	{
		res = false;
		goto _finish;
	}
...
bool CInventory::Action(s32 cmd, u32 flags)
{
	...
	case kARTEFACT: {
		if (flags & CMD_START)
		{
			if ((int)m_iActiveSlot == ARTEFACT_SLOT && m_slots[m_iActiveSlot].m_pIItem && IsGameTypeSingle())
			{
				b_send_event = Activate(NO_ACTIVE_SLOT);
			}
			else
...
u32 CInventory::dwfGetGrenadeCount(LPCSTR caSection, bool SearchAll)
{
	...
	{
		PIItem l_pIItem = *l_it;
		if (l_pIItem && l_pIItem->object().CLS_ID == CLSID_GRENADE_F1 ||
			l_pIItem->object().CLS_ID == CLSID_GRENADE_RGD5)
			++l_dwCount;
	}
