...
void CHelicopter::UpdateHeliParticles()
{
	...
	if (m_pParticle)
	{
		...
		fvec3 last_pos = PositionStack.back().vPosition;
	...
	if (m_light_render->get_active())
	{
		...
		{
			int frame;
			u32 clr = m_lanim->CalculateBGR(Engine.TimeManager.GetGlobalTime(), frame); // тючтЁр•рхЄ т ЇюЁьрЄх BGR
			Fcolor fclr;
...
void CHelicopter::SetEnemy(fvec3* pos)
{
...
void SHeliBodyState::LookAtPoint(fvec3 point, bool do_it)
{
