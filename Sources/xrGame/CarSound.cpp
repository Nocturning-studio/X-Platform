...
void CCar::SCarSound::Init()
{
	...
	if (ini->section_exist("car_sound") && ini->line_exist("car_sound", "snd_volume"))
	{
		...
		snd_engine_stop.create(READ_IF_EXISTS(ini, r_string, "car_sound", "engine_stop", "car\\test_car_stop"),
							   st_Effect, sg_SourceType);
		float fengine_start_delay = READ_IF_EXISTS(ini, r_float, "car_sound", "engine_sound_start_dellay", 0.25f);
		// engine_start_delay=iFloor((snd_engine_start._handle() ? snd_engine_start._handle()->length_ms()
		// : 1.f)*fengine_start_delay);
		engine_start_delay =
