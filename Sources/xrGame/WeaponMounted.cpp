...
{
	camera = xr_new<CCameraFirstEye>(this, CCameraBase::flRelativeLink | CCameraBase::flPositionRigid |
...
bool CWeaponMounted::Use(const fvec3& pos, const fvec3& dir, const fvec3& foot_pos)
{
	return !Owner();
}
bool CWeaponMounted::attach_Actor(CGameObject* actor)
{
	...
	const fmat4x4& A = K->LL_GetTransform(actor_bone);
	fvec3 ap;
...
fvec3 CWeaponMounted::ExitPosition()
{
	return Transform().c;
}
...
const fmat4x4& CWeaponMounted::get_ParticlesTransform()
{
