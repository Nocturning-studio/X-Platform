...
bool game_sv_ArtefactHunt::assign_rp_tmp(game_PlayerState* ps_who, xr_vector<RPoint>& rps, xr_vector<u32>& dest,
										 xr_vector<u32>& rpIDEnemy, xr_vector<u32>& EnemyIt, bool force_find)
{
	dest.clear();
...
void game_sv_ArtefactHunt::OnArtefactOnBase(ClientID id_who)
{
	...
	if (pTeam)
	{
		...
		for (u32 it = 0; it < cnt; ++it)
		{
			...
			if (pstate->team == ps->team)
			{
				...
				Player_AddExperience(pstate,
									 READ_IF_EXISTS(pSettings, r_float, "mp_bonus_exp", "target_succeed_all", 0));
			}
			else
			{
	...
	NET_Packet P;
	P.w_begin(M_EVENT);
	P.w_u32(Engine.TimeManager.GetGlobalTimeMs());
	P.w_u16(GE_DESTROY);
...
void game_sv_ArtefactHunt::Update()
{
	...
	case GAME_PHASE_TEAM2_ELIMINATED: {
		if (m_delayedTeamEliminated && m_TeamEliminatedDelay < Engine.TimeManager.TimerAsync())
		{
...
bool game_sv_ArtefactHunt::ArtefactSpawn_Allowed()
{
	...
	for (u32 it = 0; it < cnt; ++it)
	{
		...
		{
			VERIFY2(((ps->team - 1) < 2) && ((ps->team - 1) >= 0),
					make_string("cnt=%d,ps->team = %d,it=%d", cnt, ps->team, it));
			TeamAlived[ps->team - 1]++;
		}
	}

	if (TeamAlived[0] == 0 || TeamAlived[1] == 0)
		return FALSE;
...
void game_sv_ArtefactHunt::Artefact_PrepareForSpawn()
{
	...
	m_dwArtefactSpawnTime = Engine.TimeManager.GetGlobalTimeMs() + Get_ArtefactsRespawnDelta() * 1000;
...
{
	m_dwArtefactRemoveTime = Engine.TimeManager.GetGlobalTimeMs() + Get_ArtefactsStayTime() * 60000;
...
bool game_sv_ArtefactHunt::Artefact_NeedToSpawn()
{
	...
	if (m_dwArtefactSpawnTime < Engine.TimeManager.GetGlobalTimeMs())
	{
...
bool game_sv_ArtefactHunt::Artefact_NeedToRemove()
{
	...
	if (m_dwArtefactRemoveTime < Engine.TimeManager.GetGlobalTimeMs())
	{
...
void game_sv_ArtefactHunt::MoveAllAlivePlayers()
{
	...
	for (u32 it = 0; it < cnt; ++it)
	{
		...
		//-----------------------------------------------
		fvec3 Pos = pA->o_Position;
		fvec3 Angle = pA->o_Angle;
		//		pA->o_Position	= Pos;
		//		pA->o_Angle		= Angle;
		//------------------------------------------------
		pActor->SetfHealth(pActor->GetMaxHealth());
