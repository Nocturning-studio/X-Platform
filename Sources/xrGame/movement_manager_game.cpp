...
void CMovementManager::process_game_path()
{
	...
	case ePathStateBuildGamePath: {
		...
		if (game_path().failed())
		{
			...
			float3 temp = ai().game_graph().vertex(object().ai_location().game_vertex_id())->level_point();
			...
			Msg("! Target point mask [%d][%d][%d][%d]", target_vertex_type[0], target_vertex_type[1],
				target_vertex_type[2], target_vertex_type[3]);
	...
	case ePathStateBuildLevelPath: {
		...
		level_path().build_path(object().ai_location().level_vertex_id(), dest_level_vertex_id);
	...
	case ePathStateBuildDetailPath: {
		...
		detail().set_start_direction(float3().setHP(-m_body.current.yaw, 0));
