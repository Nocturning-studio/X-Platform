...
void CIKLimbsController::Create(CGameObject* O)
{
	...
	VERIFY(K);
	{
		u16 bones[4] = {K->LL_BoneID("bip01_l_upperarm"), K->LL_BoneID("bip01_l_forearm"), K->LL_BoneID("bip01_l_hand"),
						K->LL_BoneID("bip01_l_finger0")};
		LimbSetup(bones);
	}

	{
		u16 bones[4] = {K->LL_BoneID("bip01_r_upperarm"), K->LL_BoneID("bip01_r_forearm"), K->LL_BoneID("bip01_r_hand"),
						K->LL_BoneID("bip01_r_finger0")};
		LimbSetup(bones);
	}

	{
		u16 bones[4] = {K->LL_BoneID("bip01_l_thigh"), K->LL_BoneID("bip01_l_calf"), K->LL_BoneID("bip01_l_foot"),
						K->LL_BoneID("bip01_l_toe0")};
		LimbSetup(bones);
	}

	{
		u16 bones[4] = {K->LL_BoneID("bip01_r_thigh"), K->LL_BoneID("bip01_r_calf"), K->LL_BoneID("bip01_r_foot"),
						K->LL_BoneID("bip01_r_toe0")};
		LimbSetup(bones);
	}
...
void CIKLimbsController::LimbSetup(const u16 bones[4])
{
	...
	_bone_chains.back().Create((u16)_bone_chains.size() - 1, skeleton_animated, bones, toe,
							   true); // Fvector( ).set( 0.13143f, 0, 0.20f )
}

void CIKLimbsController::LimbCalculate(SCalculateData& cd)
{
	cd.do_collide =
		m_legs_blend; // && !cd.m_K->LL_GetMotionDef(m_legs_blend->motionID)->marks.empty();//m_legs_blend->;
