...
class Point
{
  public:
	...
	// }
	//! TO BE DOCUMENTED
	// IC	Point&		Min(const Point& p)								{ x = MIN(x, p.x); y = MIN(y, p.y); z = MIN(z,
	// p.z);	return *this;	}
	//! TO BE DOCUMENTED
	// IC	Point&		Max(const Point& p)								{ x = MAX(x, p.x); y = MAX(y, p.y); z = MAX(z,
	// p.z);	return *this;	}

	//! Computes square magnitude
	IC float SquareMagnitude() const
	{
		return x * x + y * y + z * z;
	}
	//! Computes magnitude
	...
	//! Access as array
	ICF operator const float*() const
	{
		return &x;
	}
	//! Access as array
...
{
	float norm_box_proj = std::abs(maxbox.x * normal.x) + std::abs(maxbox.y * normal.y) + std::abs(maxbox.z * normal.z);
	if (d > -norm_box_proj && d < norm_box_proj)
		return true;
	else
		return false;
}
