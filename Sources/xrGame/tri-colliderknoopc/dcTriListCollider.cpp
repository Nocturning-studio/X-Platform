...
int dcTriListCollider::CollideCylinder(dxGeom* Cylinder, int Flags, dContactGeom* Contacts, int Stride)
{
	...
	AABB.x = REAL(0.5) * dFabs(R[1] * CylinderLength) + (std::sqrt(R[0] * R[0] + R[2] * R[2]) * CylinderRadius);

	AABB.y = REAL(0.5) * dFabs(R[5] * CylinderLength) + (std::sqrt(R[4] * R[4] + R[6] * R[6]) * CylinderRadius);

	AABB.z = REAL(0.5) * dFabs(R[9] * CylinderLength) + (std::sqrt(R[8] * R[8] + R[10] * R[10]) * CylinderRadius);
