...
Flags32 g_mt_config = {mtLevelPath | mtDetailPath | mtObjectHandler | mtSoundPlayer | mtAiVision | mtBullets |
					   mtLUA_GC | mtLevelSounds | mtALife};
#ifdef DEBUG
...
class CCC_MemStats : public IConsole_Command
{
  public:
	...
	virtual void Execute(LPCSTR args)
	{
		...
		Msg("* [x-ray]: crt heap[%d K], process heap[%d K], game lua[%d K], engine lua[%d K], render[%d K]",
...
{
  public:
	CCC_DemoPlay(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = TRUE;
	};
	virtual void Execute(LPCSTR args)
...
{
  public:
	CCC_ALifeLoadFrom(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = true;
	};
	virtual void Execute(LPCSTR args)
...
{
  public:
	CCC_FlushLog(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = true;
	};
	virtual void Execute(LPCSTR /**args/**/)
...
{
  public:
	CCC_ClearLog(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = true;
	};
...
{
  public:
	CCC_Script(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = true;
	};
	virtual void Execute(LPCSTR args)
...
{
	CCC_ShowSmartCastStats(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = true;
	};
...
{
	CCC_ClearSmartCastStats(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = true;
	};
...
{
  public:
	CCC_MainMenu(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = true;
	};
	virtual void Execute(LPCSTR args)
...
{
  public:
	CCC_DumpObjects(LPCSTR N) : IConsole_Command(N)
	{
		bEmptyArgsHandled = true;
	};
	virtual void Execute(LPCSTR)
...
#ifndef MASTER_GOLD
	CMD4(CCC_Vector3, "psp_cam_offset", &CCameraLook2::m_cam_offset, fvec3().set(-1000, -1000, -1000),
		 fvec3().set(1000, 1000, 1000));
#endif // MASTER_GOLD
