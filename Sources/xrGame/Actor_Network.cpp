...
void CActor::net_Export(NET_Packet& P) // export to server
{
	...
	fvec3 v = character_physics_support()->movement()->GetVelocity();
...
static void w_vec_q8(NET_Packet& P, const fvec3& vec, const fvec3& min, const fvec3& max)
{
	P.w_float_q8(vec.x, min.x, max.x);
	P.w_float_q8(vec.y, min.y, max.y);
	P.w_float_q8(vec.z, min.z, max.z);
}
static void r_vec_q8(NET_Packet& P, fvec3& vec, const fvec3& min, const fvec3& max)
{
...
void CActor::CalculateInterpolationParams()
{
	...
	*/
	...
	/////////////////////////////////////////////////////////////////////
	fvec3 SP0, SP1, SP2, SP3;
	fvec3 HP0, HP1, HP2, HP3;
...
void dbg_draw_piramid(fvec3 pos, fvec3 dir, float size, float xdir, u32 color)
{

	fvec3 p0, p1, p2, p3, p4;
...
void CActor::OnRender_Network()
{
	...
	if (g_Alive())
	{
		...
		if (dbg_net_Draw_Flags.test(1 << 0) && V)
		{
			if (this != Level().CurrentViewEntity() || cam_active != eacFirstEye)
			{
				...
				for (u16 i=0; i<BoneCount; i++)
				{
					...
					fmat4x4 BoneMatrix; BoneOBB.transform_get(BoneMatrix);
					fmat4x4 BoneMatrixRes; BoneMatrixRes.mul(V->LL_GetTransform(i), BoneMatrix);
		...
		fmat4x4 MS, MH, ML, *pM = NULL;
		ML.translate(0, 0.2f, 0);
		MS.translate(0, 0.2f, 0);
		MH.translate(0, 0.2f, 0);

		fvec3 point0S, point1S, point0H, point1H, point0L, point1L, *ppoint0 = NULL, *ppoint1 = NULL;
		fvec3 tS, tH;
...
void CActor::Check_for_AutoPickUp()
{
	...
	bc.add(Position(), m_AutoPickUp_AABB_Offset);
	Fbox APU_Box;
	APU_Box.set(fvec3().sub(bc, m_AutoPickUp_AABB), fvec3().add(bc, m_AutoPickUp_AABB));
...
void CActor::SetHitInfo(CObject* who, CObject* weapon, s16 element, fvec3 Pos, fvec3 Dir)
{
