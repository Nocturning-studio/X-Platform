...
float CLevelGraph::distance(const fvec3& position, const CLevelGraph::CVertex* vertex) const
{
	SContour _contour;
...
void CLevelGraph::choose_point(const fvec3& start_point, const fvec3& finish_point, const SContour& _contour,
							   int node_id, fvec3& temp_point, int& saved_index) const
{
	SContour tNextContour;
	SSegment tNextSegment;
	fvec3 tCheckPoint1 = start_point, tCheckPoint2 = start_point, tIntersectPoint;
	contour(tNextContour, node_id);
	intersect(tNextSegment, tNextContour, _contour);
	u32 dwIntersect =
		intersect(start_point.x, start_point.z, finish_point.x, finish_point.z, tNextSegment.v1.x, tNextSegment.v1.z,
				  tNextSegment.v2.x, tNextSegment.v2.z, &tIntersectPoint.x, &tIntersectPoint.z);
	if (!dwIntersect)
		return;
	for (int i = 0; i < 4; ++i)
	{
...
float CLevelGraph::check_position_in_direction(u32 start_vertex_id, const fvec3& start_position,
											   const fvec3& finish_position, const float max_distance) const
{
	SContour _contour;
	const_iterator I, E;
	int saved_index, iPrevIndex = -1, iNextNode;
	fvec3 start_point = start_position, temp_point = start_position, finish_point = finish_position;
	...
	if (inside(vertex(dwCurNode), finish_position) &&
		(std::abs(vertex_plane_y(*vertex(dwCurNode), finish_position.x, finish_position.z) - finish_position.y) < .5f))
		return (start_point.distance_to_xz(finish_position));
	else
		return (max_distance);
}

float CLevelGraph::mark_nodes_in_direction(u32 start_vertex_id, const fvec3& start_position,
										   const fvec3& tDirection, float fDistance, xr_vector<u32>& tpaStack,
										   xr_vector<bool>* tpaMarks) const
{
	fvec3 finish_point, direction = tDirection;
	direction.normalize();
...
float CLevelGraph::mark_nodes_in_direction(u32 start_vertex_id, const fvec3& start_position, u32 finish_vertex_id,
...
float CLevelGraph::mark_nodes_in_direction(u32 start_vertex_id, const fvec3& start_point, const fvec3& finish_point,
										   xr_vector<u32>& tpaStack, xr_vector<bool>* tpaMarks) const
{
	SContour _contour;
	const_iterator I, E;
	int saved_index, iPrevIndex = -1, iNextNode;
	fvec3 temp_point = start_point;
...
float CLevelGraph::farthest_vertex_in_direction(u32 start_vertex_id, const fvec3& start_point,
												const fvec3& finish_point, u32& finish_vertex_id,
...
bool CLevelGraph::create_straight_path(u32 start_vertex_id, const fvec3& start_point, const fvec3& finish_point,
									   xr_vector<fvec3>& tpaOutputPoints, xr_vector<u32>& tpaOutputNodes,
...
u32 CLevelGraph::check_position_in_direction_slow(u32 start_vertex_id, const fvec2& start_position,
...
bool CLevelGraph::check_vertex_in_direction_slow(u32 start_vertex_id, const fvec2& start_position,
												 u32 finish_vertex_id) const
{
	fvec3 finish_position = vertex_position(finish_vertex_id);
	u32 cur_vertex_id = start_vertex_id, prev_vertex_id = u32(-1);
	Fbox2 box;
	fvec2 identity, start, dest, dir;
...
IC fvec3 v3d(const fvec2& vector2d)
{
	return (fvec3().set(vector2d.x, 0.f, vector2d.y));
}

IC fvec2 v2d(const fvec3& vector3d)
{
	return (fvec2().set(vector3d.x, vector3d.z));
}

bool CLevelGraph::create_straight_path(u32 start_vertex_id, const fvec2& start_point, const fvec2& finish_point,
									   xr_vector<fvec3>& tpaOutputPoints, xr_vector<u32>& tpaOutputNodes,
									   bool bAddFirstPoint, bool bClearPath) const
{
...
bool CLevelGraph::neighbour_in_direction(const fvec3& direction, u32 start_vertex_id) const
{
	u32 cur_vertex_id = start_vertex_id, prev_vertex_id = u32(-1);
	Fbox2 box;
	fvec2 identity, start, dest, dir;
