...
CWeaponMagazinedWGrenade::CWeaponMagazinedWGrenade(LPCSTR name, ESoundTypes eSoundType)
	: CWeaponMagazined(name, eSoundType)
{
	m_ammoType2 = 0;
...
bool CWeaponMagazinedWGrenade::CanAttach(PIItem pIItem)
{
	...
	if (pGrenadeLauncher && CSE_ALifeItemWeapon::eAddonAttachable == m_eGrenadeLauncherStatus &&
...
void CWeaponMagazinedWGrenade::UpdateSounds()
{
	...
	if (sndShotG.playing())
		sndShotG.set_position(get_LastFP());
	if (sndReloadG.playing())
		sndReloadG.set_position(get_LastFP());
