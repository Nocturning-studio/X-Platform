...
void SPHCharacterRestrictor::Create(CPHCharacter* ch, dVector3 sizes)
{
	...
	case CPHCharacter::rtStalker:
		static_cast<CPHActorCharacter::stalker_restrictor*>(this)->Create(ch, sizes);
		break;
	case CPHCharacter::rtStalkerSmall:
		static_cast<CPHActorCharacter::stalker_small_restrictor*>(this)->Create(ch, sizes);
		break;
	case CPHCharacter::rtMonsterMedium:
		static_cast<CPHActorCharacter::medium_monster_restrictor*>(this)->Create(ch, sizes);
		break;
	default:
		NODEFAULT;
	}
...
void CPHActorCharacter::SetAcceleration(float3 accel)
{
	float3 cur_a, input_a;
...
void CPHActorCharacter::Jump(const float3& accel)
{
	if (!b_exist)
		return;
