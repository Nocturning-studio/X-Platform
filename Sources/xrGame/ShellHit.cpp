...
void CPHShell::applyHit(const float3& pos, const float3& dir, float val, const u16 id, ALife::EHitType hit_type)
{
	if (id == u16(-1))
		return;
...
void CPHShell::ExplosionHit(const float3& pos, const float3& dir, float val, const u16 id)
{
	if (!isActive())
		return;
	EnableObject(0);
	// float3 local_pos;local_pos.set(0.f,0.f,0.f);
