...
void CLevelGraph::draw_nodes()
{
	...
	RenderBackendLegacy.set_Shader(sh_debug);
	...
	//////////////////////////////////////////////////////////////////////////
	float3 min_position, max_position;
	...
	for (; I != E; ++I)
	{
		...
		if (::Render->ViewBase.testSphere_dirty(PC, sr))
		{
			...
			// render quad
			RenderBackendLegacy.dbg_DrawTRI(Fidentity, v3, v2, v1, CT);
			RenderBackendLegacy.dbg_DrawTRI(Fidentity, v1, v4, v3, CT);
...
void CLevelGraph::draw_restrictions()
{
	...
	for (; I != E; ++I)
	{
		...
		for (; i != e; ++i)
		{
			float3 temp = ai().level_graph().vertex_position(*i);
			temp.y += .1f;
...
void CLevelGraph::draw_objects()
{
	...
	for (; I < E; ++I)
	{
		...
		if (tpCustomMonster)
		{
			...
			{
				float3 temp = tpCustomMonster->movement()
								   .detail()
