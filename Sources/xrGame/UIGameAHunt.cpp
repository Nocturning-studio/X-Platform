...
void CUIGameAHunt::SetClGame(game_cl_GameState* g)
{
	...
	m_pBuySpawnMsgBox->AddCallback("msg_box", MESSAGE_BOX_YES_CLICKED,
								   CUIWndCallback::void_function(clmp_game, &game_cl_mp::OnBuySpawn));
}
...
void CUIGameAHunt::Init()
{
	...
	CUIFrags2* pFragList = xr_new<CUIFrags2>();
	...
	CUIFrags2* pPlayerListT1 = xr_new<CUIFrags2>();
	pPlayerListT1->SetAutoDelete(true);
	...
	GameCaptions()->addCustomMessage(m_todo_caption, 0.0f, -0.8f, 0.02f, HUD().Font().pFontGraffiti19Russian,
									 CGameFont::alCenter, TODO_MSG_COLOR, "");
	GameCaptions()->customizeMessage(m_todo_caption, CUITextBanner::tbsFlicker)->fPeriod = 0.5f;

	m_buy_msg_caption = "ah_buy";
	GameCaptions()->addCustomMessage(m_buy_msg_caption, DI2PX(0.0f), DI2PY(0.9f), SZ(0.02f),
									 HUD().Font().pFontGraffiti19Russian, CGameFont::alCenter, BUY_MSG_COLOR, "");
	//-----------------------------------------------------------
