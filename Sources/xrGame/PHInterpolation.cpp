...
void CPHInterpolation::SetBody(dBodyID body)
{
	if (!body)
		return;
	m_body = body;
	qPositions.fill_in(*((fvec3*)dBodyGetPosition(m_body)));
...
void CPHInterpolation::UpdatePositions()
{
	...
	qPositions.push_back(*((fvec3*)dBodyGetPosition(m_body)));
}
...
void CPHInterpolation::InterpolatePosition(fvec3& pos)
{
...
void CPHInterpolation::InterpolateRotation(fmat4x4& rot)
{
	Fquaternion q;
...
void CPHInterpolation::GetPosition(fvec3& p, u16 num)
{
	if (!m_body)
		return;
	p.set(qPositions[num]);
}
void CPHInterpolation::SetPosition(const fvec3& p, u16 num)
{
	if (!m_body)
		return;
