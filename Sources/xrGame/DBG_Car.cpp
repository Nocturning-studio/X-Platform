...
void CCar::DbgCreatePlots()
{
	...
	m_dbg_torque_rpm.Init(CFunctionGraph::type_function(this, &CCar::TorqueRpmFun), m_min_rpm, m_max_rpm, 0, y_pos, 500,
						  y_w, 1000);
	...
	{
		xr_vector<float3>::iterator i = m_gear_ratious.begin() + 1, e = m_gear_ratious.end();
		for (; i < e; i++)
		{
...
void CCar::DbgUbdateCl()
{
	...
	if (m_pPhysicsShell && OwnerActor() && static_cast<CObject*>(Owner()) == Level().CurrentViewEntity())
	{
		if (ph_dbg_draw_mask.test(phDbgDrawCarDynamics))
		{
			...
			HUD().Font().pFontStat->SetColor(
				D3DCOLOR_XRGB(255, !b_transmission_switching * 255, !b_transmission_switching * 255));
			...
			HUD().Font().pFontStat->OutNext("rpm:      [%3.2f]", m_current_rpm / (1.f / 60.f * 2.f * PI));
