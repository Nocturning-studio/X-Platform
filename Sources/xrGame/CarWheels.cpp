...
void CCar::SWheel::Init()
{
	...
	R_ASSERT2(dJointGetType(joint->GetDJoint()) == dJointTypeHinge2,
			  "No wheel join was set for a wheel, only wheel-joint valid!!!");
...
void CCar::SWheelDrive::Init()
{
	...
	{
	case jtWheel:
		pos_fvd = bone_map.find(pwheel->bone_id)->second.element->mTransform.k.x;
		break;
...
void CCar::SWheelSteer::Init()
{
	...
	case jtWheel:
		pos_right =
			bone_map.find(pwheel->bone_id)->second.element->mTransform.i.y; //.dotproduct(pwheel->car->m_root_transform.j);
		break;
