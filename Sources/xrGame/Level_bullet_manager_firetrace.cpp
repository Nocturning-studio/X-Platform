...
void CBulletManager::FireShotmark(SBullet* bullet, const fvec3& vDir, const fvec3& vEnd, collide::rq_result& R,
								  u16 target_material, const fvec3& vNormal, bool ShowMark)
{
	...
	if ((ps_name && ShowMark) || (bullet->flags.explosive && bStatic))
	{
		...
		fmat4x4 pos{0.f, 0.f, 0.f, 0.f};
		pos.k.normalize(particle_dir);
		fvec3::generate_orthonormal_basis(pos.k, pos.j, pos.i);
		pos.c.set(vEnd);
		if (ps_name && ShowMark)
		{
...
void CBulletManager::DynamicObjectHit(CBulletManager::_event& E)
{
	...
	fvec3 original_dir = E.bullet.dir;
	float power, impulse;
	...
	fvec3 p_in_object_space, position_in_bone_space;
	fmat4x4 m_inv;
	...
	if (V)
	{
		...
		fmat4x4& m_bone = (V->LL_GetBoneInstance(u16(E.R.element))).mTransform;
		fmat4x4 m_inv_bone;
...
extern void random_dir(fvec3& tgt_dir, const fvec3& src_dir, float dispersion);

std::pair<float, float> CBulletManager::ObjectHit(SBullet* bullet, const fvec3& end_point, collide::rq_result& R,
												  u16 target_material, fvec3& hit_normal)
{
	//----------- normal - start
	if (R.O)
	{
	...
	if (((f + shoot_factor) < ricoshet_factor) && bullet->flags.allow_ricochet)
	{
		...
		float scale = 1.f - std::abs(bullet->dir.dotproduct(hit_normal)) * m_fCollisionEnergyMin;
