...
template <typename T> IC T sin_apb(T sina, T cosa, T sinb, T cosb)
{
	return (sina * cosb + cosa * sinb);
}

template <typename T> IC T cos_apb(T sina, T cosa, T sinb, T cosb)
{
...
bool CDetailPathManager::build_circle_trajectory(const STrajectoryPoint& position, xr_vector<STravelPathPoint>* path,
												 u32* vertex_id, const u32 velocity)
{
	...
	{
		int m = std::min(iFloor(std::abs(angle) / position.angular_velocity * 10.f + 1.5f),
					 iFloor(position.radius * std::abs(angle) / min_dist + 1.5f));
#ifdef DEBUG
		if (m >= 10000)
		{
											   ...
											   u32 vertex_id, xr_vector<STravelPathPoint>* path, u32 velocity)
{
	...
	return (path ? ai().level_graph().create_straight_path<false>(vertex_id, start.point, dest.point, *path, t, false,
																  false)
										  ...
										  const u32 velocity2, const u32 velocity3)
{
	...
	float straight_velocity = std::abs(velocity(velocity2).linear_velocity);
	{
		...
		{
			dist[i].index = i;
			dist[i].time = std::abs(tangents[i][0].angle) / start.angular_velocity +
											...
											const EDirectionType direction_type)
{
	...
	for (u32 i = 0; i < 2; ++i)
		for (u32 j = 0; j < 2; ++j)
			if (compute_tangent(start, start_circles[i], dest, dest_circles[j], tangent_points[tangent_count],
								direction_type))
			{
										 ...
										 STrajectoryPoint& start, STrajectoryPoint& dest)
{
	...
	for (int _i = 0, i = 0, n = (int)level_path.size() - 1, j = n, m = j; _i < n;)
	{
		...
		{
			if ((j == n) && !ai().level_graph().valid_vertex_id(ai().level_graph().check_position_in_direction(
...
IC CDetailPathManager::STravelPoint CDetailPathManager::compute_better_key_point(const STravelPoint& point0,
...
void CDetailPathManager::postprocess_key_points(const xr_vector<u32>& level_path, u32 intermediate_index,
												STrajectoryPoint& start, STrajectoryPoint& dest,
												xr_vector<STravelParamsIndex>& finish_params, u32 straight_line_index,
												u32 straight_line_index_negative)
{
	...
	for (int i = 1, n = (int)m_key_points.size() - 1; i < n; ++i)
	{
		...
		{
			u32 vertex_id = ai().level_graph().check_position_in_direction(key_point0.vertex_id, key_point0.position,
																		   m_key_points[i + 1].position);
			if (!ai().level_graph().valid_vertex_id(vertex_id))
			{
				vertex_id = vertex_id;
			}
		...
		{
			u32 vertex_id = ai().level_graph().check_position_in_direction(key_point1.vertex_id, key_point1.position,
																		   m_key_points[i + 1].position);
			if (!ai().level_graph().valid_vertex_id(vertex_id))
			{
				vertex_id = vertex_id;
...
void CDetailPathManager::add_patrol_point()
{
	...
	if ((m_path.size() > 1) && m_state_patrol_path && !fis_zero(extrapolate_length()))
	{
		...
		ai().level_graph().create_straight_path<false>(
			m_path.back().vertex_id, ai().level_graph().v2d(m_path.back().position), ai().level_graph().v2d(t.position),
			m_path, m_path.back(), false, false);
	}
}
