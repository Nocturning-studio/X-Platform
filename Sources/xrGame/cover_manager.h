...
class CCoverManager
{
  ...
  public:
	...
	void clear();
	template <typename _evaluator_type, typename _restrictor_type>
	IC const CCoverPoint* best_cover(const float3& position, float radius, _evaluator_type& evaluator,
									 const _restrictor_type& restrictor) const;
	template <typename _evaluator_type>
	IC const CCoverPoint* best_cover(const float3& position, float radius, _evaluator_type& evaluator) const;
