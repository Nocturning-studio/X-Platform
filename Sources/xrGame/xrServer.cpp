	...
	{ // for each client
		...
		for (u32 p = 0; p <= m_iCurUpdatePacket; p++)
		{
			...
			if (ToSend.B.count > 2)
			{
				...
				{
					Msg("- Server Update[%d] to Client[%s]  : %d", *((u16*)ToSend.B.data), Client->ps->getName(),
						ToSend.B.count);
				}
				//.#endif
...
void xrServer::verify_entity(const CSE_Abstract* entity) const
{
	...
	if (entity->ID_Parent != 0xffff)
	{
		...
		VERIFY3(std::find((*J).second->children.begin(), (*J).second->children.end(), entity->ID) !=
				...
				"SERVER : Parent/Children relationship mismatch - Object has parent, but corresponding parent doesn't "
				"have children",
