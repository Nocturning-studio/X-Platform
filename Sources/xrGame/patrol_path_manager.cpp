...
IC bool CPatrolPathManager::accessible(const fvec3& position) const
{
...
struct CAccessabilityEvaluator
{
	...
	IC bool operator()(const fvec3& position) const
	{
		return (m_manager->accessible(position));
	}
};

void CPatrolPathManager::select_point(const fvec3& position, u32& dest_vertex_id)
{
	...
	if (!actual() || !m_path->vertex(m_curr_point_index))
	{
		...
		R_ASSERT2(vertex || show_restrictions(m_object),
				  make_string("any vertex in patrol path [%s] in inaccessible for object [%s]", *m_path_name,
							  *m_game_object->cName()));
