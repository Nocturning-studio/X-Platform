...
void add_online_impl(CSE_ALifeDynamicObject* object, const bool& update_registries)
{
	...
	ClientID clientID;
	clientID.set(object->alife().server().GetServerClient() ? object->alife().server().GetServerClient()->ID.value()
															: 0);
	...
	for (; I != E; ++I)
	{
		...
		l_tpALifeInventoryItem->base()->s_flags.bit_or(M_SPAWN_UPDATE);
		...
		//		R_ASSERT3
		//(ai().level_graph().valid_vertex_id(l_tpALifeDynamicObject->m_tNodeID),"Invalid vertex for object
		//",l_tpALifeInventoryItem->name_replace());
		...
		l_tpALifeDynamicObject->s_flags.bit_and(u16(-1) ^ M_SPAWN_UPDATE);
