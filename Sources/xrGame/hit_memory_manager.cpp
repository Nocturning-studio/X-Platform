...
struct CHitObjectPredicate
{
	...
	CHitObjectPredicate(const CObject* object) : m_object(object)
	{
	}
...
{
	add(0, fvec3().set(0, 0, 1), entity_alive, 0);
}
...
void CHitMemoryManager::add(float amount, const fvec3& vLocalDir, const CObject* who, s16 element)
{
	...
	if (m_hits->end() == J)
	{
		...
		hit_object.m_first_level_time = Engine.TimeManager.GetGlobalTimeMs();
#endif
