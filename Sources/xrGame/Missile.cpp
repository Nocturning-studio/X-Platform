...
void CMissile::UpdatePosition(const fmat4x4& trans)
{
...
void CMissile::setup_throw_params()
{
	...
	trans.k.set(FireDir);
	fvec3::generate_orthonormal_basis(trans.k, trans.j, trans.i);
	trans.c.set(FirePos);
...
void CMissile::UpdateFireDependencies_internal()
{
	...
	if (Engine.TimeManager.GetFrameCount() != dwFP_Frame)
	{
		...
		{
			// 3rd person
			fmat4x4& parent = H_Parent()->Transform();
