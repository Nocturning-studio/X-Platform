...
IC bool PhOutOfBoundariesCharacter(const float3& v)
{
...
bool test_sides(const float3& center, const float3& side_dir, const float3& fv_dir, const float3& box, int tri_id)
{
	Triangle tri;
	InitTriangle(Level().ObjectSpace.GetStaticTris() + tri_id, tri);
	float3* verts = Level().ObjectSpace.GetStaticVerts();
	{
	...
	const float3& v2 = verts[tri.T->verts[2]];
	const float3& v0 = verts[tri.T->verts[0]];
	crses[2].x = -(v0.z - v2.z);
	crses[2].y = 0.f;
	crses[2].z = v0.x - v2.x;
	for (u8 i = 0; 3 > i; ++i)
	{
		const float3& crs = crses[i];
...
void CPHSimpleCharacter::ApplyImpulse(const float3& dir, dReal P)
{
	...
	// m_ext_imulse.set(float3().mul(dir,P/fixed_step/impulse_time_constant));
	dBodySetForce(m_body, m_ext_imulse.x * P / fixed_step, m_ext_imulse.y * P / fixed_step,
				  m_ext_imulse.z * P / fixed_step);
}

void CPHSimpleCharacter::ApplyForce(const float3& force)
{
...
void CPHSimpleCharacter::ApplyForce(const float3& dir, float force)
{
...
void CPHSimpleCharacter::PhDataUpdate(dReal /**step/**/)
{
	...
	if (PhOutOfBoundariesCharacter(cast_fv(dBodyGetPosition(m_body))))
		Disable();
...
void CPHSimpleCharacter::PhTune(dReal step)
{
	...
	if ((ud->pushing_neg || ud->pushing_b_neg) && !b_death_pos)
	{
		...
		if (!ud->pushing_b_neg)
		{
			// float3 movement;movement.sub(cast_fv(dGeomGetPosition(m_wheel)),cast_fv(ud->last_pos));
	...
	if (is_control)
	{
		...
		if (ph_dbg_draw_mask.test(phDbgCharacterControl))
		{
			DBG_DrawLine(cast_fv(dBodyGetPosition(m_body)),
						 float3().add(cast_fv(dBodyGetPosition(m_body)), float3().mul(cast_fv(sidedir), 1.f)),
						 D3DCOLOR_XRGB(0, 0, 255));
			DBG_DrawLine(
				cast_fv(dBodyGetPosition(m_body)),
				float3().add(cast_fv(dBodyGetPosition(m_body)), float3().mul(cast_fv(m_control_force), 1.f / 19.6f)),
				D3DCOLOR_XRGB(0, 0, 255));
			DBG_DrawLine(cast_fv(dBodyGetPosition(m_body)),
						 float3().add(cast_fv(dBodyGetPosition(m_body)),
									   float3().mul(cast_fv(dBodyGetForce(m_body)), 1.f / 19.6f)),
...
{
	float3 AABB, AABB_forbid, center, center_forbid, accel_add, accel;
	...
	// float3 fv_dir;fv_dir.mul(accel,1.f/mag);
	float3 sd_dir;
	...
	for (CDB::RESULT* Res = R_begin; Res != R_end; ++Res)
	{
		...
		Point vertices[3] = {Point((dReal*)&Res->verts[0]), Point((dReal*)&Res->verts[1]),
							 Point((dReal*)&Res->verts[2])};
...
void CPHSimpleCharacter::SetAcceleration(float3 accel)
{
	if (!b_exist)
		return;
...
void CPHSimpleCharacter::SetCamDir(const float3& cam_dir)
{
...
void CPHSimpleCharacter::ApplyAcceleration()
{
	...
	if (m_elevator_state.Active())
	{
		if (m_elevator_state.NearState())
			m_elevator_state.GetControlDir(*(float3*)accel);
		if (m_elevator_state.ClimbingState())
		{
			// dVectorSet(m_control_force,accel);
			if (m_elevator_state.GetControlDir(*(float3*)m_control_force))
			{
...
void CPHSimpleCharacter::IPosition(float3& pos)
{
...
void CPHSimpleCharacter::SetPosition(float3 pos)
{
	VERIFY_BOUNDARIES(pos, phBoundaries, PhysicsRefObject());
	if (!b_exist)
		return;
...
void CPHSimpleCharacter::GetPosition(float3& vpos)
{
	if (!b_exist)
	{
...
void CPHSimpleCharacter::GetPreviousPosition(float3& pos)
{
	VERIFY(b_exist);
	VERIFY(!ph_world->Processing());
	m_body_interpolation.GetPosition(pos, 0);
}
void CPHSimpleCharacter::GetVelocity(float3& vvel)
{
	if (!b_exist)
	{
...
void CPHSimpleCharacter::SetVelocity(float3 vel)
{
	if (!b_exist)
		return;
...
void CPHSimpleCharacter::OnRender()
{
	...
	float4x4 m;
	m.identity();
	float3 n = *(float3*)m_ground_contact_normal;
	n.mul(100.f);
	float3 pos;
	GetPosition(pos);
	pos.y += m_radius;
	Level().debug_renderer().draw_line(m, pos, *(float3*)m_control_force, color_rgba(256, 0, 0, 1));
	...
	Level().debug_renderer().draw_aabb(m_bcenter_forbid, m_AABB_forbid.x, m_AABB_forbid.y, m_AABB_forbid.z,
									   D3DCOLOR_XRGB(255, 0, 0));
#endif
	/// M.c.set(0.f,1.f,0.f);
	// Level().debug_renderer().draw_ellipse(M, 0xffffffff);
...
void CPHSimpleCharacter::SafeAndLimitVelocity()
{
	...
	if (dV_valid(linear_velocity))
	{
		...
		if (b_external_impulse)
		{
			...
			{
				float3 acc;
				acc.set(float3().mul(m_acceleration, 1.f / _sqrt(sq_mag)));
				float3 vll;
...
void CPHSimpleCharacter::GroundNormal(float3& norm)
{
	if (m_elevator_state.ClimbingState())
	{
	...
	{
		norm.set(*((float3*)m_ground_contact_normal));
	}
}
...
void CPHSimpleCharacter::DeathPosition(float3& deathPos)
{
	if (!b_exist)
		return;
...
void CPHSimpleCharacter::AddControlVel(const float3& vel)
{
...
void CPHSimpleCharacter::SCollisionDamageInfo::HitDir(float3& dir) const
{
...
void CPHSimpleCharacter::GetSmothedVelocity(float3& vvel)
{
	if (!b_exist)
	{
		vvel.set(0, 0, 0);
