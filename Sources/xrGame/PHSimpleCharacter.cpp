...
IC bool PhOutOfBoundariesCharacter(const fvec3& v)
{
...
bool test_sides(const fvec3& center, const fvec3& side_dir, const fvec3& fv_dir, const fvec3& box, int tri_id)
{
	Triangle tri;
	InitTriangle(Level().ObjectSpace.GetStaticTris() + tri_id, tri);
	fvec3* verts = Level().ObjectSpace.GetStaticVerts();
	{
	...
	const fvec3& v2 = verts[tri.T->verts[2]];
	const fvec3& v0 = verts[tri.T->verts[0]];
	crses[2].x = -(v0.z - v2.z);
	crses[2].y = 0.f;
	crses[2].z = v0.x - v2.x;
	for (u8 i = 0; 3 > i; ++i)
	{
		const fvec3& crs = crses[i];
...
void CPHSimpleCharacter::ApplyImpulse(const fvec3& dir, dReal P)
{
	...
	// m_ext_imulse.set(fvec3().mul(dir,P/fixed_step/impulse_time_constant));
	dBodySetForce(m_body, m_ext_imulse.x * P / fixed_step, m_ext_imulse.y * P / fixed_step,
				  m_ext_imulse.z * P / fixed_step);
}

void CPHSimpleCharacter::ApplyForce(const fvec3& force)
{
...
void CPHSimpleCharacter::ApplyForce(const fvec3& dir, float force)
{
...
void CPHSimpleCharacter::PhTune(dReal step)
{
	...
	if ((ud->pushing_neg || ud->pushing_b_neg) && !b_death_pos)
	{
		...
		if (!ud->pushing_b_neg)
		{
			// fvec3 movement;movement.sub(cast_fv(dGeomGetPosition(m_wheel)),cast_fv(ud->last_pos));
	...
	if (is_control)
	{
		...
		if (ph_dbg_draw_mask.test(phDbgCharacterControl))
		{
			DBG_DrawLine(cast_fv(dBodyGetPosition(m_body)),
						 fvec3().add(cast_fv(dBodyGetPosition(m_body)), fvec3().mul(cast_fv(sidedir), 1.f)),
						 D3DCOLOR_XRGB(0, 0, 255));
			DBG_DrawLine(
				cast_fv(dBodyGetPosition(m_body)),
				fvec3().add(cast_fv(dBodyGetPosition(m_body)), fvec3().mul(cast_fv(m_control_force), 1.f / 19.6f)),
				D3DCOLOR_XRGB(0, 0, 255));
			DBG_DrawLine(cast_fv(dBodyGetPosition(m_body)),
						 fvec3().add(cast_fv(dBodyGetPosition(m_body)),
									   fvec3().mul(cast_fv(dBodyGetForce(m_body)), 1.f / 19.6f)),
...
{
	fvec3 AABB, AABB_forbid, center, center_forbid, accel_add, accel;
	...
	// fvec3 fv_dir;fv_dir.mul(accel,1.f/mag);
	fvec3 sd_dir;
...
void CPHSimpleCharacter::SetAcceleration(fvec3 accel)
{
	if (!b_exist)
		return;
...
void CPHSimpleCharacter::SetCamDir(const fvec3& cam_dir)
{
...
void CPHSimpleCharacter::ApplyAcceleration()
{
	...
	if (m_elevator_state.Active())
	{
		if (m_elevator_state.NearState())
			m_elevator_state.GetControlDir(*(fvec3*)accel);
		if (m_elevator_state.ClimbingState())
		{
			// dVectorSet(m_control_force,accel);
			if (m_elevator_state.GetControlDir(*(fvec3*)m_control_force))
			{
...
void CPHSimpleCharacter::IPosition(fvec3& pos)
{
...
void CPHSimpleCharacter::SetPosition(fvec3 pos)
{
	VERIFY_BOUNDARIES(pos, phBoundaries, PhysicsRefObject());
	if (!b_exist)
		return;
...
void CPHSimpleCharacter::GetPosition(fvec3& vpos)
{
	if (!b_exist)
	{
...
void CPHSimpleCharacter::GetPreviousPosition(fvec3& pos)
{
	VERIFY(b_exist);
	VERIFY(!ph_world->Processing());
	m_body_interpolation.GetPosition(pos, 0);
}
void CPHSimpleCharacter::GetVelocity(fvec3& vvel)
{
	if (!b_exist)
	{
...
void CPHSimpleCharacter::SetVelocity(fvec3 vel)
{
	if (!b_exist)
		return;
...
void CPHSimpleCharacter::OnRender()
{
	...
	fmat4x4 m;
	m.identity();
	fvec3 n = *(fvec3*)m_ground_contact_normal;
	n.mul(100.f);
	fvec3 pos;
	GetPosition(pos);
	pos.y += m_radius;
	Level().debug_renderer().draw_line(m, pos, *(fvec3*)m_control_force, color_rgba(256, 0, 0, 1));
...
void CPHSimpleCharacter::GroundNormal(fvec3& norm)
{
	if (m_elevator_state.ClimbingState())
	{
	...
	{
		norm.set(*((fvec3*)m_ground_contact_normal));
	}
}
...
void CPHSimpleCharacter::DeathPosition(fvec3& deathPos)
{
	if (!b_exist)
		return;
...
void CPHSimpleCharacter::AddControlVel(const fvec3& vel)
{
...
void CPHSimpleCharacter::SCollisionDamageInfo::HitDir(fvec3& dir) const
{
...
void CPHSimpleCharacter::GetSmothedVelocity(fvec3& vvel)
{
	if (!b_exist)
	{
		vvel.set(0, 0, 0);
