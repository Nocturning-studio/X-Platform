...
class CSightAction : public CControlAction
{
  ...
  protected:
	float3 m_holder_start_position;
	float3 m_object_start_position;
  ...
  public:
	...
	IC CSightAction(const ESightType& sight_type, const float3& vector3d, bool torso_look = false);
	IC CSightAction(const ESightType& sight_type, const float3* vector3d);
	...
	void initialize();
	void execute();
	IC void set_vector3d(const float3& position);
