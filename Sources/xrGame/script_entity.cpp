...
#ifdef DEBUG
	if (bScriptControl)
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeInfo, "Script %s set object %s under its control",
										*caSciptName, *object().cName());
	else
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeInfo,
										"Script %s freed object %s from its control", *caSciptName, *object().cName());
#endif
	if (!bScriptControl)
...
void CScriptEntity::vfUpdateParticles()
{
	...
	if (xr_strlen(l_tParticleAction.m_caBoneName))
	{
		...
		l_tpParticlesObject->UpdateParent(GetUpdatedMatrix(l_tParticleAction.m_caBoneName,
...
void CScriptEntity::vfUpdateSounds()
{
	...
	if (xr_strlen(l_tSoundAction.m_caBoneName) && m_current_sound && m_current_sound->_feedback())
		m_current_sound->_feedback()->set_position(
			GetUpdatedMatrix(l_tSoundAction.m_caBoneName, l_tSoundAction.m_tSoundPosition, fvec3().set(0, 0, 0)).c);
}
...
//%s",Engine.TimeManager.GetGlobalTimeMs(),*l_tpEntityAction->m_tAnimationAction.m_caAnimationToPlay);
#endif
...
#ifdef DEBUG
		if (empty_queue)
			ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeInfo, "Object %s has an empty script queue!",
											*object().cName());
#endif
		return;
	}
...
const fmat4x4 CScriptEntity::GetUpdatedMatrix(shared_str caBoneName, const fvec3& tPositionOffset,
											  const fvec3& tAngleOffset)
{
	fmat4x4 l_tMatrix;
	...
	if (xr_strlen(caBoneName))
	{
		CBoneInstance& l_tBoneInstance =
			smart_cast<CKinematics*>(object().Visual())
				->LL_GetBoneInstance(smart_cast<CKinematics*>(object().Visual())->LL_BoneID(caBoneName));
		l_tMatrix.mulA_43(l_tBoneInstance.mTransform);
		l_tMatrix.mulA_43(object().Transform());
	}
...
#endif
				const fmat4x4& l_tMatrix = GetUpdatedMatrix(
...
bool CScriptEntity::bfAssignMovement(CScriptEntityAction* tpEntityAction)
{
	...
	case CScriptMovementAction::eGoalTypeObject: {
		...
		//[%f][%f][%f]",Engine.TimeManager.GetGlobalTimeMs(),*l_tpGameObject->cName(),VPUSH(l_tpGameObject->Position()));
...
bool CScriptEntity::bfScriptAnimation()
{
		...
		xr_strlen(GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay))
	{
		...
		if (m_tpScriptAnimation == m_tpNextAnimation)
			...
			//%s",Engine.TimeManager.GetGlobalTimeMs(),*GetCurrentAction()->m_tAnimationAction.m_caAnimationToPlay, *object().cName());
#endif
...
void CScriptEntity::sound_callback(const CObject* object, int sound_type, const fvec3& position, float sound_power)
{
	...
	m_saved_sounds.push_back(
		CSavedSound(smart_cast<const CGameObject*>(object)->lua_game_object(), sound_type, position, sound_power));
}
...
void CScriptEntity::process_sound_callbacks()
{
	...
	for (; I != E; ++I)
	{
		...
		object().callback(GameObject::eSound)(object().lua_game_object(), (*I).m_lua_game_object, (*I).m_sound_type,
											  (*I).m_position, (*I).m_sound_power);
	}
