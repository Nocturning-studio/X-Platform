...
void CCustomMonster::mk_orientation(fvec3& dir, fmat4x4& mR)
{
...
void CCustomMonster::eye_pp_s0()
{
	...
	V->CalculateBones();
	fmat4x4& mEye = V->LL_GetTransform(u16(eye_bone));
	fmat4x4 X;
...
void CCustomMonster::eye_pp_s1()
{
	...
	if (g_Alive())
	{
		...
		update_range_fov(new_range, new_fov,
						 human_being() ? memory().visual().current_state().m_max_view_distance * eye_range : eye_range,
						 eye_fov);
#else
	...
	Engine.Statistic->AI_Vis_Query.End();
}

void CCustomMonster::eye_pp_s2()
{
	// Tracing
	Engine.Statistic->AI_Vis_RayTests.Begin();
	...
	Engine.Statistic->AI_Vis_RayTests.End();
}
...
void CCustomMonster::HitSignal(float /**perc/**/, fvec3& /**vLocalDir/**/, CObject* /**who/**/)
{
}
...
// void CCustomMonster::Hit(float P, fvec3 &dir,CObject* who, s16 element,fvec3 position_in_object_space, float
...
extern void dbg_draw_frustum(float FOV, float _FAR, float A, fvec3& P, fvec3& D, fvec3& U);
