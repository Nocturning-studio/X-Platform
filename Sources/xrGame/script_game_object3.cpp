...
CScriptGameObject* CScriptGameObject::GetCurrentOutfit() const
{
	...
	CGameObject* current_equipment =
		inventoryOwner->GetCurrentOutfit() ? &inventoryOwner->GetCurrentOutfit()->object() : 0;
...
void CScriptGameObject::set_mental_state(EMentalState mental_state)
{
	...
	else
	{
		...
		if (mental_state != eMentalStateDanger)
		{
			...
			{
				ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,
												"CAI_Stalker : set_mental_state is used during universal combat!");
				return;
			}
		}
#endif // DEBUG
...
void CScriptGameObject::sell_condition(float friend_factor, float enemy_factor)
{
	...
	inventory_owner->trade_parameters().default_factors(CTradeParameters::action_sell(0),
														CTradeFactors(friend_factor, enemy_factor));
}
...
void CScriptGameObject::buy_condition(float friend_factor, float enemy_factor)
{
	...
	inventory_owner->trade_parameters().default_factors(CTradeParameters::action_buy(0),
														CTradeFactors(friend_factor, enemy_factor));
}
...
{
	default_trade_parameters().default_factors(CTradeParameters::action_sell(0),
											   CTradeFactors(friend_factor, enemy_factor));
}
...
{
	default_trade_parameters().default_factors(CTradeParameters::action_buy(0),
											   CTradeFactors(friend_factor, enemy_factor));
}
