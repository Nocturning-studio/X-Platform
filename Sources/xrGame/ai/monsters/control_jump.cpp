...
// Start jump
//////////////////////////////////////////////////////////////////////////
void CControlJump::start_jump(const fvec3& point)
{
	...
	else
	{
		...
		if (is_flag(SControlJumpData::ePrepareInMove))
		{
			...
			if (m_man->path_builder().accessible(target_point))
			{
				...
				u32 node = ai().level_graph().check_position_in_direction(m_object->ai_location().level_vertex_id(),
																		  m_object->Position(), target_point);
			...
			if (prepared)
			{
				if (m_man->build_path_line(this, target_point, u32(-1),
										   m_data.state_prepare_in_move.velocity_mask |
											   MonsterMovement::eVelocityParameterStand))
				{
					//---------------------------------------------------------------------------------------------------
					// set path params
...
void CControlJump::select_next_anim_state()
{
	...
	{
	case eStatePrepare:
		ctrl_data->global.motion = m_data.state_prepare.motion;
		break;
	case eStatePrepareInMove:
		ctrl_data->global.motion = m_data.state_prepare_in_move.motion;
		break;
	case eStateGlide:
		ctrl_data->global.motion = m_data.state_glide.motion;
		break;
	case eStateGround:
		ctrl_data->global.motion = m_data.state_ground.motion;
		break;
	default:
		NODEFAULT;
	}
...
fvec3 CControlJump::get_target(CObject* obj)
{
...
bool CControlJump::can_jump(CObject* target)
{
	...
	if ((dist < m_min_distance) || (dist > m_max_distance))
		return false;

	// получить вектор направления и его мир угол
	float dir_yaw = fvec3().sub(target_position, source_position).getH();
...
fvec3 CControlJump::predict_position(CObject* obj, const fvec3& pos)
{
	return pos;
