...
void CControlManagerCustom::on_start_control(ControlCom::EControlType type)
{
	...
	{
	case ControlCom::eControlSequencer:
		m_man->subscribe(this, ControlCom::eventSequenceEnd);
		break;
	case ControlCom::eControlTripleAnimation:
		m_man->subscribe(this, ControlCom::eventTAChange);
		break;
	case ControlCom::eControlJump:
		m_man->subscribe(this, ControlCom::eventJumpEnd);
		break;
	case ControlCom::eControlRotationJump:
		m_man->subscribe(this, ControlCom::eventRotationJumpEnd);
		break;
	case ControlCom::eControlMeleeJump:
		m_man->subscribe(this, ControlCom::eventMeleeJumpEnd);
		break;
	case ControlCom::eControlRunAttack:
		m_man->subscribe(this, ControlCom::eventRunAttackEnd);
		break;
	case ControlCom::eControlThreaten:
		m_man->subscribe(this, ControlCom::eventThreatenEnd);
		break;
	case ControlCom::eComCriticalWound:
		m_man->subscribe(this, ControlCom::eventCriticalWoundEnd);
		break;
	}
}
...
void CControlManagerCustom::on_stop_control(ControlCom::EControlType type)
{
	...
	{
	case ControlCom::eControlSequencer:
		m_man->unsubscribe(this, ControlCom::eventSequenceEnd);
		break;
	case ControlCom::eControlTripleAnimation:
		m_man->unsubscribe(this, ControlCom::eventTAChange);
		break;
	case ControlCom::eControlJump:
		m_man->unsubscribe(this, ControlCom::eventJumpEnd);
		break;
	case ControlCom::eControlRotationJump:
		m_man->unsubscribe(this, ControlCom::eventRotationJumpEnd);
		break;
	case ControlCom::eControlMeleeJump:
		m_man->unsubscribe(this, ControlCom::eventMeleeJumpEnd);
		break;
	case ControlCom::eControlRunAttack:
		m_man->unsubscribe(this, ControlCom::eventRunAttackEnd);
		break;
	case ControlCom::eControlThreaten:
		m_man->unsubscribe(this, ControlCom::eventThreatenEnd);
		break;
	case ControlCom::eComCriticalWound:
		m_man->unsubscribe(this, ControlCom::eventCriticalWoundEnd);
		break;
	}
}
...
void CControlManagerCustom::on_event(ControlCom::EEventType type, ControlCom::IEventData* data)
{
	...
	case ControlCom::eventJumpEnd:
		m_man->release(this, ControlCom::eControlJump);
		break;
	case ControlCom::eventRotationJumpEnd:
		m_man->release(this, ControlCom::eControlRotationJump);
		break;
	case ControlCom::eventMeleeJumpEnd:
		m_man->release(this, ControlCom::eControlMeleeJump);
		break;
	case ControlCom::eventRunAttackEnd:
		m_man->release(this, ControlCom::eControlRunAttack);
		break;
	case ControlCom::eventThreatenEnd:
		m_man->release(this, ControlCom::eControlThreaten);
		break;
	case ControlCom::eventCriticalWoundEnd:
		m_man->release(this, ControlCom::eComCriticalWound);
		break;
	}
}
										   ...
										   u32 vel_mask_ground, u32 flags)
{
	...
	if (s2)
	{
		...
		m_jump->setup_data().flags.bit_or(SControlJumpData::ePrepareInMove);
	}
	else
	...
	else
	{
		...
		m_jump->setup_data().flags.bit_or(SControlJumpData::eGroundSkip);
	}

	if (!s1 && !s2)
	{
		m_jump->setup_data().flags.bit_or(SControlJumpData::ePrepareSkip);
	}

	m_jump->setup_data().flags.bit_or(SControlJumpData::eGlidePlayAnimOnce);
	m_jump->setup_data().flags.bit_or(SControlJumpData::eGlideOnPrepareFailed);
...
void CControlManagerCustom::jump(const fvec3& position)
{
	if (!m_man->check_start_conditions(ControlCom::eControlJump))
		return;
	...
	ctrl_data->flags.bit_or(SControlJumpData::ePrepareSkip);
	ctrl_data->force_factor = -1.f;

	m_man->activate(ControlCom::eControlJump);
}

void CControlManagerCustom::script_jump(const fvec3& position, float factor)
{
	if (!m_man->check_start_conditions(ControlCom::eControlJump))
		return;
...
void CControlManagerCustom::check_jump_over_physics()
{
	...
	fvec3 prev_pos = m_object->Position();
	float dist_sum = 0.f;
		 ...
		 i < m_man->path_builder().detail().path().size(); i++)
	{
		...
		for (u32 k = 0; k < m_nearest.size(); k++)
		{
			...
			fvec3 dir = fvec3().sub(travel_point.position, m_object->Position());
			...
			dir = fvec3().sub(obj->Position(), m_object->Position());
