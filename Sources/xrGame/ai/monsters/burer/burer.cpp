...
void CBurer::reload(LPCSTR section)
{
	...
	sound().add(pSettings->r_string(section, "sound_gravi_attack"), DEFAULT_SAMPLE_COUNT, SOUND_TYPE_MONSTER_ATTACKING,
				MonsterSound::eHighPriority + 2, u32(MonsterSound::eBaseChannel), eMonsterSoundGraviAttack,
				"bip01_head");
	sound().add(pSettings->r_string(section, "sound_tele_attack"), DEFAULT_SAMPLE_COUNT, SOUND_TYPE_MONSTER_ATTACKING,
				MonsterSound::eHighPriority + 3, u32(MonsterSound::eBaseChannel), eMonsterSoundTeleAttack,
				"bip01_head");

	// add triple animations
	com_man().ta_fill_data(anim_triple_gravi, "stand_gravi_0", "stand_gravi_1", "stand_gravi_2", TA_EXECUTE_ONCE,
						   TA_DONT_SKIP_PREPARE, ControlCom::eCapturePath | ControlCom::eCaptureMovement);
	com_man().ta_fill_data(anim_triple_tele, "stand_tele_0", "stand_tele_1", "stand_tele_2", TA_EXECUTE_ONCE,
						   TA_DONT_SKIP_PREPARE, ControlCom::eCapturePath | ControlCom::eCaptureMovement);
}
...
void CBurer::Load(LPCSTR section)
{
	...
	anim().AddAnim(eAnimStandTurnRight, "stand_turn_rs_", -1, &velocity_turn, PS_STAND, "fx_stand_f", "fx_stand_b",
				   "fx_stand_l", "fx_stand_r");
	anim().AddAnim(eAnimStandDamaged, "stand_idle_dmg_", -1, &velocity_none, PS_STAND, "fx_stand_f", "fx_stand_b",
				   "fx_stand_l", "fx_stand_r");
	...
	anim().AddAnim(eAnimDie, "stand_die_", -1, &velocity_none, PS_STAND, "fx_stand_f", "fx_stand_b", "fx_stand_l",
				   "fx_stand_r");
	...
	anim().AddAnim(eAnimSteal, "stand_steal_", -1, &velocity_steal, PS_STAND, "fx_stand_f", "fx_stand_b", "fx_stand_l",
				   "fx_stand_r");
	anim().AddAnim(eAnimEat, "sit_eat_", -1, &velocity_none, PS_SIT, "fx_stand_f", "fx_stand_b", "fx_stand_l",
				   "fx_stand_r");

	anim().AddAnim(eAnimSitIdle, "sit_idle_", -1, &velocity_none, PS_SIT, "fx_stand_f", "fx_stand_b", "fx_stand_l",
				   "fx_stand_r");
	anim().AddAnim(eAnimCheckCorpse, "sit_check_corpse_", -1, &velocity_none, PS_SIT, "fx_stand_f", "fx_stand_b",
				   "fx_stand_l", "fx_stand_r");
	anim().AddAnim(eAnimSitStandUp, "sit_stand_up_", -1, &velocity_none, PS_SIT, "fx_stand_f", "fx_stand_b",
				   "fx_stand_l", "fx_stand_r");
	anim().AddAnim(eAnimStandSitDown, "stand_sit_down_", -1, &velocity_none, PS_STAND, "fx_stand_f", "fx_stand_b",
				   "fx_stand_l", "fx_stand_r");
...
// void CBurer::Hit(float P,Fvector &dir,CObject*who,s16 element,Fvector p_in_object_space,float impulse,
...
void CBurer::Hit(SHit* pHDS)
{
	if (m_shield_active && (pHDS->hit_type == ALife::eHitTypeFireWound) && (Engine.TimeManager.GetFrameCount() != last_hit_frame))
	{
		...
		// вычислить позицию и направленность партикла
		Fmatrix pos;
		// CParticlesPlayer::MakeTransform(this,element,Fvector().set(0.f,0.f,1.f),p_in_object_space,pos);
...
CBaseMonster::SDebugInfo CBurer::show_debug_info()
{
	...
	DBG().text(this).add_item("---------------------------------------", info.x, info.y += info.delta_y,
							  info.delimiter_color);
