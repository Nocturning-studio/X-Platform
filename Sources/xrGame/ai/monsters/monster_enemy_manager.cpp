...
void CMonsterEnemyManager::update()
{
	...
	if (prev_enemy == enemy)
	{
		...
		if (std::abs(dist_now - dist_prev) < 0.2f)
		...
		flags.bit_or(FLAG_ENEMY_STATS_NOT_READY);
...
bool CMonsterEnemyManager::is_faced(const CEntityAlive* object0, const CEntityAlive* object1)
{
	...
	fvec3 tPosition = object0->Position();
	...
	fYawFov =
		angle_normalize_signed((std::abs(fYawFov) + std::abs(atanf(1.f / tPosition.distance_to(object1->Position())))) / 2.f);
