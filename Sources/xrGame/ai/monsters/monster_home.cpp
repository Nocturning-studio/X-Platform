...
{
	VERIFY2(
		ai().game_graph().vertex(path->vertices().begin()->second->data().game_vertex_id())->level_id() ==
...
u32 CMonsterHome::get_place_in_cover()
{
	...
	const CCoverPoint* point =
		m_object->CoverMan->find_cover(vertex->data().position(), vertex->data().position(), m_radius_min,
