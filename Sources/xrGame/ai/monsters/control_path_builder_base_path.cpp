...
void CControlPathBuilderBase::find_target_point_set()
{
	...
	if (m_target_type == eMoveToTarget)
	{
		...
		{
			m_target_found.node = m_man->path_builder().restrictions().accessible_nearest(
				float3().set(m_target_found.position), m_target_found.position);
...
void CControlPathBuilderBase::find_node()
{
	...
	m_target_found.node = ai().level_graph().check_position_in_direction(m_object->ai_location().level_vertex_id(),
																		 m_object->Position(), m_target_found.position);
		...
		m_man->path_builder().accessible(m_target_found.node))
	{
		...
		m_man->path_builder().fix_position(float3().set(m_target_found.position), m_target_found.node,
	...
	m_target_found.node = m_man->path_builder().find_nearest_vertex(m_object->ai_location().level_vertex_id(),
																	m_target_found.position, 30.f);
