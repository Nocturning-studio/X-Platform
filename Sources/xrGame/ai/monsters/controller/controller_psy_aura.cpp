	...
	: inherited(ppi)
{
	...
	m_time_state_started = Engine.TimeManager.GetGlobalTimeMs();

	m_snd_left.clone(snd_left, st_Effect, sg_SourceType);
	m_snd_right.clone(snd_right, st_Effect, sg_SourceType);

	m_snd_left.play_at_pos(Actor(), float3().set(-1.f, 0.f, 1.f), sm_Looped | sm_2D);
	m_snd_right.play_at_pos(Actor(), float3().set(-1.f, 0.f, 1.f), sm_Looped | sm_2D);
}
