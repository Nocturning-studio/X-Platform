...
bool CBaseMonster::bfAssignMovement(CScriptEntityAction* tpEntityAction)
{
	...
	case eMA_WalkFwd:
		...
		break;
	case eMA_WalkBkwd:
		anim().m_tAction = ACT_WALK_BKWD;
		break;
	case eMA_Run:
		anim().m_tAction = ACT_RUN;
		break;
	case eMA_Drag:
		anim().m_tAction = ACT_DRAG;
		break;
	case eMA_Steal:
		anim().m_tAction = ACT_STEAL;
		break;
	}
...
bool CBaseMonster::bfAssignAnimation(CScriptEntityAction* tpEntityAction)
{
	...
	case eAA_StandIdle:
		...
		break;
	case eAA_SitIdle:
		anim().m_tAction = ACT_SIT_IDLE;
		break;
	case eAA_LieIdle:
		anim().m_tAction = ACT_LIE_IDLE;
		break;
	case eAA_Eat:
		anim().m_tAction = ACT_EAT;
		break;
	case eAA_Sleep:
		anim().m_tAction = ACT_SLEEP;
		break;
	case eAA_Rest:
		anim().m_tAction = ACT_REST;
		break;
	case eAA_Attack:
		anim().m_tAction = ACT_ATTACK;
		break;
	case eAA_LookAround:
		anim().m_tAction = ACT_LOOK_AROUND;
		break;
	}
...
bool CBaseMonster::bfAssignSound(CScriptEntityAction* tpEntityAction)
{
	...
	case eMonsterSoundIdle:
		sound().play(eMonsterSoundIdle, 0, 0,
					 (l_tAction.m_monster_sound_delay == int(-1)) ? db().m_dwIdleSndDelay
																  : l_tAction.m_monster_sound_delay);
		break;
	case eMonsterSoundEat:
		sound().play(eMonsterSoundEat, 0, 0,
					 (l_tAction.m_monster_sound_delay == int(-1)) ? db().m_dwEatSndDelay
																  : l_tAction.m_monster_sound_delay);
		break;
