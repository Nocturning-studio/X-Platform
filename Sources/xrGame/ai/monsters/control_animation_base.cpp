...
void CControlAnimationBase::select_animation(bool anim_end)
{
	...
	// установить анимацию
	string128 s1, s2;
	MotionID cur_anim = smart_cast<CKinematicsAnimated*>(m_object->Visual())
	...
	m_cur_anim.time_started = Engine.TimeManager.GetGlobalTimeMs();
	m_cur_anim.speed._set_current(1.f);
	m_cur_anim.speed._set_target(-1.f);
}
...
EAction CControlAnimationBase::VelocityIndex2Action(u32 velocity_index)
{
	...
	case MonsterMovement::eVelocityParameterWalkNormal:
		return ACT_WALK_FWD;
	case MonsterMovement::eVelocityParameterRunNormal:
		return ACT_RUN;
	case MonsterMovement::eVelocityParameterWalkDamaged:
		return ACT_WALK_FWD;
	case MonsterMovement::eVelocityParameterRunDamaged:
		return ACT_RUN;
	case MonsterMovement::eVelocityParameterSteal:
		return ACT_STEAL;
	case MonsterMovement::eVelocityParameterDrag:
		return ACT_DRAG;
	case MonsterMovement::eVelocityParameterInvisible:
		return ACT_RUN;
	}
...
MotionID CControlAnimationBase::get_motion_id(EMotionAnim a, u32 index)
{
	...
	string128 s1, s2;
	return (smart_cast<CKinematicsAnimated*>(m_object->Visual())
				->ID_Cycle_Safe(strconcat(sizeof(s2), s2, *anim_it->target_name, itoa(index, s1, 10))));
}
