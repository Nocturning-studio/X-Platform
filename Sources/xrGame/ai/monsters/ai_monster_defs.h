...
typedef struct
{
	...
	fvec3 trace_from; // направление трассировки (относительно центра)
	fvec3 trace_to;
...
struct SCurrentAnimationInfo
{
	...
	struct
	{
		...
		{
			current = v;
			VERIFY2(std::abs(v) < 1000, "_set_current(). monster speed is too big");
		}
		IC void _set_target(float v)
		{
			target = v;
			VERIFY2(std::abs(v) < 1000, "_set_target(). monster speed is too big");
		}
		IC float _get_current()
		{
			return current;
		}
		IC float _get_target()
		{
			return target;
		}
