...
BOOL CAlienEffectorPP::Process(SPPInfo& pp)
{
	...
	def_lerp(factor, target_factor, PERIOD_SPEED, Engine.TimeManager.GetDeltaTime());
...
CAlienEffector::CAlienEffector(ECamEffectorType type, CAI_Bloodsucker* obj) : inherited(type, flt_max)
{
	...
	fvec3::generate_orthonormal_basis(m_prev_eye_matrix.k, m_prev_eye_matrix.j, m_prev_eye_matrix.i);
	m_inertion = 1.f;
...
BOOL CAlienEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	def_lerp(m_inertion, 1 - rel_dist, rel_dist, Engine.TimeManager.GetDeltaTime());
	...
	fvec3::generate_orthonormal_basis_normalized(m_prev_eye_matrix.k, m_prev_eye_matrix.j, m_prev_eye_matrix.i);
	...
	def_lerp(m_current_fov, m_target_fov, BSA_FOV_SPEED, Engine.TimeManager.GetDeltaTime());
