...
bool CAI_Stalker::bfAssignMovement(CScriptEntityAction* tpEntityAction)
{
	...
	movement().update(Engine.TimeManager.GetDeltaTimeMs());
	sight().update();
	...
	case CScriptWatchAction::eGoalTypeObject: {
		...
		else
		{
			CBoneInstance& l_tBoneInstance =
				smart_cast<CKinematics*>(l_tWatchAction.m_tpObjectToWatch->Visual())
					->LL_GetBoneInstance(smart_cast<CKinematics*>(l_tWatchAction.m_tpObjectToWatch->Visual())
											 ->LL_BoneID(l_tWatchAction.m_bone_to_watch));
			float4x4 l_tMatrix;

			l_tMatrix = l_tBoneInstance.mTransform;
			l_tMatrix.mulA_43(l_tWatchAction.m_tpObjectToWatch->Transform());
...
bool CAI_Stalker::bfAssignObject(CScriptEntityAction* tpEntityAction)
{
	...
	case eObjectActionReload1: {
		...
		//			}
		//			else
		//				ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError,"cannot reload active item
...
//%s",Engine.TimeManager.GetGlobalTimeMs(),*tpEntityAction->m_tAnimationAction.m_caAnimationToPlay);
#endif
