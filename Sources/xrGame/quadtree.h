...
template <typename _object_type> class CQuadTree
{
  ...
  protected:
	IC u32 neighbour_index(const fvec3& position, fvec3& center, float distance) const;
	IC void nearest(const fvec3& position, float radius, xr_vector<_object_type*>& objects, CQuadNode* node,
					fvec3 center, float distance, int depth) const;
	IC _object_type* remove(const _object_type* object, CQuadNode*& node, fvec3 center, float distance, int depth);
  ...
  public:
	...
	IC _object_type* find(const fvec3& position);
	IC void nearest(const fvec3& position, float radius, xr_vector<_object_type*>& objects, bool clear = true) const;
