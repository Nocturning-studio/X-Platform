...
void CMovementManager::move_along_path(CPHMovementControl* movement_control, Fvector& dest_position, float time_delta)
{
	...
	while (detail().m_current_travel_point < detail().path().size() - 2)
	{
		...
		float pos_dist_to_cur_point =
			dest_position.distance_to(detail().path()[detail().m_current_travel_point].position);
		float pos_dist_to_next_point =
			dest_position.distance_to(detail().path()[detail().m_current_travel_point + 1].position);
	...
	precision); movement_control->GetPosition			(dest_position);
