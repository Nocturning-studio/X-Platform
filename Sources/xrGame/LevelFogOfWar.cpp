...
ivec2 CLevelFogOfWar::ConvertRealToLocal(const fvec2& src)
{
	ivec2 res;
...
fvec2 CLevelFogOfWar::ConvertLocalToReal(const ivec2& src)
{
	return fvec2().set(m_levelRect.lt.x + src.x * FOG_CELL_SZ, m_levelRect.lt.y + src.y * FOG_CELL_SZ);
}
...
void CLevelFogOfWar::Draw()
{
	...
	tgt.rb.add(fvec2().set(FOG_CELL_SZ - EPS_L, FOG_CELL_SZ - EPS_L));
	Irect cells = ConvertRealToLocal(tgt);

	fvec2 realCellsPosLT = ConvertLocalToReal(cells.lt);
	...
	const fvec2 uvs[6] = {{0.0f, 0.0f}, {0.5f, 0.0f}, {0.5f, 1.0f}, {0.0f, 0.0f}, {0.5f, 1.0f}, {0.0f, 1.0f}};
