...
bool CTrade::CanTrade()
{
	...
	float Res = rad2deg(std::abs(yaw - yaw2) < PI ? std::abs(yaw - yaw2) : PI_MUL_2 - std::abs(yaw - yaw2));
...
u32 CTrade::GetItemPrice(PIItem pItem, bool b_buying)
{
	...
	clamp(action_factor, std::min(trade_factors.enemy_factor(), trade_factors.friend_factor()),
