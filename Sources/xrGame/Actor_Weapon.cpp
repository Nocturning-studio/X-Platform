	...
	if (!effector)
	{
		if (IsZoomAimingMode())
		{
			effector = (CCameraShotEffector*)Cameras().AddCamEffector(xr_new<CCameraShotEffector>(
				weapon->zoom_camMaxAngle, weapon->zoom_camRelaxSpeed, weapon->zoom_camMaxAngleHorz,
				weapon->zoom_camStepAngleHorz, weapon->zoom_camDispertionFrac));
		}
		else
		{
			effector = (CCameraShotEffector*)Cameras().AddCamEffector(
				xr_new<CCameraShotEffector>(weapon->camMaxAngle, weapon->camRelaxSpeed, weapon->camMaxAngleHorz,
											weapon->camStepAngleHorz, weapon->camDispertionFrac));
		}
	}
	R_ASSERT(effector);
