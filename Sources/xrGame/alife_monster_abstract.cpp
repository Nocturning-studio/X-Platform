...
void CSE_ALifeMonsterAbstract::update()
{
	...
	m_tTimeID)/1000.f/ai().alife().time_manager().normal_time_factor()*m_fCurSpeed; if (m_fDistanceToPoint -
	m_fDistanceFromPoint < EPS_L) { bContinue = true; if ((m_fDistanceFromPoint - m_fDistanceToPoint > EPS_L) &&
					...
					for ( ; i != e; ++i)
						if ((*i).vertex_id() != m_tPrevGraphID) {
							for (int j=0; j<iPointCount; ++j)
								if
	(ai().game_graph().mask(tpaTerrain[j].tMask,ai().game_graph().vertex((*i).vertex_id())->vertex_type()) &&
	((*i).vertex_id() != m_tPrevGraphID)) { if (iBranches == iChosenBranch) { m_tNextGraphID	= (*i).vertex_id();
										m_fDistanceToPoint = (*i).distance();
										bOk = true;
										break;
									}
																 ...
																 int iGroupIndex, bool bMutualDetection)
{
	...
	(l_tpALifeMonsterAbstract,"Inconsistent meet action type"); return						(ALife::eRelationTypeFriend
...
bool CSE_ALifeMonsterAbstract::bfActive()
{
	...
	return (/**/ interactive() && /**/ ((l_tpALifeGroupAbstract && (l_tpALifeGroupAbstract->m_wCount > 0)) ||
										(!l_tpALifeGroupAbstract && (fHealth > EPS_L))));
}
