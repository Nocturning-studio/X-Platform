	...
	case kRIGHT:
		...
		/*
			case kFWD:
				...
				else								m_vCamDeltaHP.y += active_camera->rot_speed.y*Engine.TimeManager.GetDeltaTime();
				break;
			case kBACK:
			...
			case kL_STRAFE:
				...
				else								m_vCamDeltaHP.x -= active_camera->rot_speed.x*Engine.TimeManager.GetDeltaTime();
				break;
			case kR_STRAFE:
...
void CCar::SetParam(int id, float2 val)
{
	if (m_car_weapon)
		m_car_weapon->SetParam(id, val);
}
void CCar::SetParam(int id, float3 val)
{
	if (m_car_weapon)
		m_car_weapon->SetParam(id, val);
}
