...
{
	CHK_DX(HW.GetDevice()->BeginStateBlock());
	for (u32 it = 0; it < States.size(); it++)
	{
		...
		{
		case 0:
			CHK_DX(HW.GetDevice()->SetRenderState((D3DRENDERSTATETYPE)S.v1, S.v2));
			break;
		case 1:
			break;
		case 2: {
			CHK_DX(HW.GetDevice()->SetSamplerState(
	...
	CHK_DX(HW.GetDevice()->EndStateBlock(&SB));
	return SB;
}
