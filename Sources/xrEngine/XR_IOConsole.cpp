...
void CConsole::DrawBackgrounds(bool bGame)
{
	...
	DrawRect(TL_pv, pr, tips_back_color);
	DrawRect(TL_pv, sr, tips_select_color);
	...
	if (tips_sz > VIEW_TIPS_COUNT)
	{
		...
		rb.x2 = rb.x1 + 2 * w1;
		rb.y2 = pr.y2;
		DrawRect(TL_pv, rb, tips_scroll_back_color);
		...
		DrawRect(TL_pv, rs, tips_scroll_pos_color);
	}
