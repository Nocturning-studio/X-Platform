...
public:
    ...
    {
        if (Const) constants.set(Const, A);
    }
    ...
    {
        if (Const) constants.set(Const, x, y, z, w);
    }
    ICF void set_Array_Constant(R_constant* Const, u32 e, const fmat4x4& A)
    {
        if (Const) constants.seta(Const, e, A);
    }
    ...
    {
        if (Const) constants.seta(Const, e, x, y, z, w);
    }
    ...
    {
        if (ctable) set_Constant(&*ctable->get(n), x, y, z, 0);
    }
    ICF void set_Constant(LPCSTR n, float x, float y, float z, float w)
    {
        if (ctable) set_Constant(&*ctable->get(n), x, y, z, w);
    }
    ICF void set_Array_Constant(LPCSTR n, u32 e, const fmat4x4& A)
    {
        if (ctable) set_Array_Constant(&*ctable->get(n), e, A);
    }
    ...
    {
        if (ctable) set_Array_Constant(&*ctable->get(n), e, x, y, z, w);
    }
    ...
    {
        CHK_DX(m_pDevice->SetRenderState(p1, p2));
    }
    IC void dbg_SetSS(u32 sampler, D3DSAMPLERSTATETYPE type, u32 value)
    {
        CHK_DX(m_pDevice->SetSamplerState(sampler, type, value));
    }
    ...
    {
        fvec3 half_dim; half_dim.set(sx, sy, sz);
        fmat4x4 TM; TM.translate(Translation);
    ...
    BOOL get_BlendState() const { return bBlend; }
    D3DBLEND get_SrcBlend() const { return srcBlend; }
    D3DBLEND get_DstBlend() const { return dstBlend; }
