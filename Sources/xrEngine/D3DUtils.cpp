...
#define DU_DRAW_SH_C(sh, c)                                                                                            \
	{                                                                                                                  \
		...
		RenderBackendLegacy.set_Constant("tfactor", float(color_get_R(c)) / 255.f, float(color_get_G(c)) / 255.f,                          \
									  ...
									  u16* indices, u32 _i_cnt)
{
	...
	R_CHK(HW.GetDevice()->CreateVertexBuffer(v_cnt * stride, D3DUSAGE_WRITEONLY, 0, D3DPOOL_DEFAULT, &pVB, 0));
	u8* bytes;
	...
	if (i_cnt)
	{
		R_CHK(HW.GetDevice()->CreateIndexBuffer(i_cnt * sizeof(u16), D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_DEFAULT,
											&pIB, NULL));
...
void CDrawUtilities::DrawOBB(const fmat4x4& parent, const Fobb& box, u32 clr_s, u32 clr_w)
{
	...
	X.mul_43(R, S);
	R.mul_43(parent, X);
	RenderBackendLegacy.set_transform_world(R);
								...
								BOOL bSolid, BOOL bWire)
{
	...
	B.mulA_43(parent);
	RenderBackendLegacy.set_transform_world(B);
