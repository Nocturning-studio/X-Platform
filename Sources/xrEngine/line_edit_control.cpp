...
void line_edit_control::on_frame()
{
	...
	u32 fr_time = Engine.TimeManager.GetContinualTimeMs();
	...
	if (m_last_changed_frame + 1 < Engine.TimeManager.GetFrameCount())
	{
		m_need_update = false;
	}

	/*if ( Engine.TimeManager.GetFrameCount() % 100 == 0 )
	{
...
void line_edit_control::update_bufs()
{
	...
	m_need_update = true;
	m_last_changed_frame = Engine.TimeManager.GetFrameCount();
	//	if ( m_cursor_view )	{
...
void line_edit_control::add_inserted_text()
{
	...
	strncpy_s(buf + m_p1, m_buffer_size - m_p1, m_inserted, std::min(new_size, m_buffer_size - m_p1)); // part 2
