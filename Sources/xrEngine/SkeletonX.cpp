...
void CSkeletonX::_Render(ref_geom& hGeom, u32 vCount, u32 iOffset, u32 pCount)
{
	...
	case RM_SINGLE: {
		...
		RenderBackendLegacy.set_transform_world(W);
	...
	case RM_SKINNING_2B: {
		// transfer matrices
		ref_constant array = RenderBackendLegacy.get_Constant(s_bones_array_const);
		u32 count = RMS_bonecount;
		for (u32 mid = 0; mid < count; mid++)
		{
			fmat4x4& M = Parent->LL_GetTransform_R(u16(mid));
			u32 id = mid * 3;
			RenderBackendLegacy.set_Array_Constant(&*array, id + 0, M._11, M._21, M._31, M._41);
			RenderBackendLegacy.set_Array_Constant(&*array, id + 1, M._12, M._22, M._32, M._42);
			RenderBackendLegacy.set_Array_Constant(&*array, id + 2, M._13, M._23, M._33, M._43);
		}
...
BOOL CSkeletonX::_PickBoneSoft1W(fvec3& normal, float& dist, const fvec3& S, const fvec3& D, u16* indices,
								 CBoneData::FacesVec& faces)
{
	...
	for (CBoneData::FacesVecIt it = faces.begin(); it != faces.end(); it++)
	{
		...
		for (u32 k = 0; k < 3; k++)
		{
			...
			const fmat4x4& transform = Parent->LL_GetBoneInstance((u16)vert.matrix).mRenderTransform;
...
BOOL CSkeletonX::_PickBoneSoft2W(fvec3& normal, float& dist, const fvec3& S, const fvec3& D, u16* indices,
								 CBoneData::FacesVec& faces)
{
	VERIFY(*Vertices2W);
	bool intersect = FALSE;
	for (CBoneData::FacesVecIt it = faces.begin(); it != faces.end(); it++)
	{
		...
		for (u32 k = 0; k < 3; k++)
		{
			...
			fmat4x4& transform0 = Parent->LL_GetBoneInstance(vert.matrix0).mRenderTransform;
			fmat4x4& transform1 = Parent->LL_GetBoneInstance(vert.matrix1).mRenderTransform;
...
// Fill Vertices
void CSkeletonX::_FillVerticesSoft1W(const fmat4x4& view, CSkeletonWallmark& wm, const fvec3& normal, float size,
...
void CSkeletonX::_FillVerticesSoft2W(const fmat4x4& view, CSkeletonWallmark& wm, const fvec3& normal, float size,
									 u16* indices, CBoneData::FacesVec& faces)
{
	VERIFY(*Vertices2W);
	for (CBoneData::FacesVecIt it = faces.begin(); it != faces.end(); it++)
