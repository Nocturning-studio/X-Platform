...
class ENGINE_API COMotion : public CCustomMotion
{
  ...
  public:
	...
	void _Evaluate(float t, fvec3& T, fvec3& R);
	virtual void Save(IWriter& F);
	virtual bool Load(IReader& F);
	...
	void CreateKey(float t, const fvec3& P, const fvec3& R);
...
class ENGINE_API CSMotion : public CCustomMotion
{
  ...
  public:
	...
	void _Evaluate(int bone_idx, float t, fvec3& T, fvec3& R);
...
class ENGINE_API CClip
{
  public:
	struct AnimItem
	{
		...
		bool valid()
		{
			return !!(name.size() && (slot != u16(-1)));
		}
		bool equal(const AnimItem& d) const
		{
			return name.equal(d.name) && (slot == d.slot);
		}
	};
	shared_str name;
	AnimItem cycles[4];
