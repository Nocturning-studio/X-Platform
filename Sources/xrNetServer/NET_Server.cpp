...
xr_string IBannedClient::BannedTimeTo() const
{
	...
	sprintf_s(res, sizeof(res), "%02d.%02d.%d_%02d:%02d:%02d", _tm_banned->tm_mday, _tm_banned->tm_mon + 1,
...
IPureServer::EConnect IPureServer::Connect(LPCSTR options)
{
	...
	if (!psNET_direct_connect)
	{
	   ...
	   // Create the IDirectPlay8Client object.
		...
		net_Address_device = NULL;
		CHK_DX(CoCreateInstance(CLSID_DirectPlay8Address, NULL, CLSCTX_INPROC_SERVER, IID_IDirectPlay8Address,
		...
		CHK_DX(net_Address_device->AddComponent(DPNA_KEY_TRAVERSALMODE, &dwTraversalMode, sizeof(dwTraversalMode),
												DPNA_DATATYPE_DWORD));
