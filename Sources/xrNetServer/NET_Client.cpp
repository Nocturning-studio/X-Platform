...
BOOL IPureClient::Connect(LPCSTR options)
{
	...
	if (!psNET_direct_connect)
	{
		...
		R_CHK(net_Address_server->AddComponent(DPNA_KEY_HOSTNAME, ServerNameUNICODE,
											   2 * u32(wcslen(ServerNameUNICODE) + 1), DPNA_DATATYPE_STRING));
		...
		if (xr_stricmp(server_name, "localhost") == 0)
		{
			...
			if (dpServerDesc->pwszSessionName)
			{
				...
				R_CHK(WideCharToMultiByte(CP_ACP, 0, dpServerDesc->pwszSessionName, -1, dpSessionName,
		...
		else
		{
			...
			while (res != S_OK && c_port <= END_PORT)
			{
				...
				res = NET->EnumHosts(&dpAppDesc,		 // pApplicationDesc
...
HRESULT IPureClient::net_Handler(u32 dwMessageType, PVOID pMessage)
{
	...
	case DPN_MSGID_ENUM_HOSTS_RESPONSE: {
		...
		if (!bHostRegistered)
		{
			...
			{
				string4096 dpSessionName;
				R_CHK(WideCharToMultiByte(CP_ACP, 0, pDesc->pwszSessionName, -1, dpSessionName, sizeof(dpSessionName),
										  0, 0));
				NODE.dpSessionName = (char*)(&dpSessionName[0]);
			}
