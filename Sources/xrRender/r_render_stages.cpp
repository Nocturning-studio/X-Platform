...
void CRender::render_forward_lights(xr_vector<light*>& lights, int phase)
{
	...
	for (size_t i = 0; i < count; ++i)
	{
		...
		if (!L->VisibilityData.visible || !L->LightFlags.bActive)
			continue;
		...
		u32 mask_id = (L->LightFlags.type == IRender_Light::OMNIPART) ? SE_MASK_POINT : SE_MASK_SPOT;
		...
		if (phase == PHASE_SPOT_LIGHTING || L->LightFlags.type != IRender_Light::OMNIPART)
		{
			...
			m_Shadow.mul(L->TransformContext.ShadowContext.view, xf_inv_view);
			...
			m_Lmap.mul(L->TransformContext.ShadowContext.view, xf_inv_view);
