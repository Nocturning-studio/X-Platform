...
void CRender::accumulate_sun(u32 sub_phase, fmat4x4& transform, fmat4x4& transform_prev)
{
	...
	if (SE_SUN_NEAR == sub_phase)
	{
		...
		u32 Offset = 0;
		RenderBackendLegacy.set_viewport_geometry(Offset);

		// Setup shader
		RenderBackendLegacy.set_Element(RenderTarget->s_accum_mask->E[SE_MASK_DIRECT]);
		RenderBackendLegacy.set_Constant("Ldynamic_dir", L_dir.x, L_dir.y, L_dir.z, 0);

		// Stencil masking
		RenderBackendLegacy.set_ColorWriteEnable(FALSE);
		RenderBackendLegacy.set_Stencil(TRUE, D3DCMP_LESSEQUAL, dwLightMarkerID, 0x01, 0xff, D3DSTENCILOP_KEEP, D3DSTENCILOP_REPLACE, D3DSTENCILOP_KEEP);
	...
	fmat4x4 m_Texgen;
	m_Texgen.identity();
	RenderBackendLegacy.transforms.set_World(m_Texgen);
	RenderBackendLegacy.transforms.set_View(Engine.RenderView.View);
	RenderBackendLegacy.transforms.set_Project(Engine.RenderView.Project);
	...
	{
		// Lock indices
		u16* pib = RenderBackendLegacy.Index.Lock(sizeof(facetable) / sizeof(u16), i_offset);
		CopyMemory(pib, facetable, sizeof(facetable));
		RenderBackendLegacy.Index.Unlock(sizeof(facetable) / sizeof(u16));

		// Lock vertices
		u32 ver_count = sizeof(corners) / sizeof(fvec3);
		FVF::L* pv = (FVF::L*)RenderBackendLegacy.Vertex.Lock(ver_count, RenderTarget->g_cuboid.stride(), v_offset);
		...
		RenderBackendLegacy.Vertex.Unlock(ver_count, RenderTarget->g_cuboid.stride());
	}

	RenderBackendLegacy.set_Geometry(RenderTarget->g_cuboid);

	// Setup shader and constants
	RenderBackendLegacy.set_Element(RenderTarget->s_accum_direct_cascade->E[sub_phase]);
	RenderBackendLegacy.set_Constant("m_bias", NormalBias, DirectionalBias);
	...
	if ((SE_SUN_NEAR == sub_phase || SE_SUN_MIDDLE == sub_phase))
		RenderBackendLegacy.SetRenderState(D3DRS_ZFUNC, D3DCMP_GREATEREQUAL);
	else if (!ps_r_lighting_flags.is(RFLAGEXT_SUN_ZCULLING))
		RenderBackendLegacy.SetRenderState(D3DRS_ZFUNC, D3DCMP_ALWAYS);
	else
		RenderBackendLegacy.SetRenderState(D3DRS_ZFUNC, D3DCMP_LESS);

	// Setup stencil
	if (SE_SUN_NEAR == sub_phase || sub_phase == SE_SUN_MIDDLE)
		RenderBackendLegacy.set_Stencil(TRUE, D3DCMP_LESSEQUAL, dwLightMarkerID, 0xff, 0xFE, D3DSTENCILOP_KEEP, D3DSTENCILOP_ZERO, D3DSTENCILOP_KEEP);
	else
		RenderBackendLegacy.set_Stencil(TRUE, D3DCMP_LESSEQUAL, dwLightMarkerID, 0xff, 0x00);

	// Render
	RenderBackendLegacy.Render(D3DPT_TRIANGLELIST, v_offset, 0, 8, i_offset, 16);
...
void CRender::accumulate_volumetric_sun(u32 sub_phase, fmat4x4 m_shadow, fvec3 L_dir)
{
	...
	if (!(g_pGamePersistent->Environment().CurrentEnv->m_fSunShaftsIntensity > 0.05f) || !ps_r_lighting_flags.test(RFLAG_SUN_SHAFTS))
	{
		...
		{
			RenderBackendLegacy.ClearTexture(RenderTarget->rt_Volumetric_Sun, color_rgba(0, 0, 0, 0));
	...
	RenderBackendLegacy.set_Constant("image_resolution", Weight, Height, 1.0f / Weight, 1.0f / Height);
	RenderBackendLegacy.set_Constant("sun_shafts_intensity", sun_shafts_intensity, 0, 0, 0);
	RenderBackendLegacy.set_Constant("Ldynamic_dir", L_dir.x, L_dir.y, L_dir.z, 0);
	RenderBackendLegacy.set_Constant("m_shadow", m_shadow);

	RenderBackendLegacy.RenderViewportSurface(Weight, Height, RenderTarget->rt_Volumetric_Sun);
}
