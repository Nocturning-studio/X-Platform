					  ...
					  const char* file, int line, const char* function, bool& ignore_always)
{
	...
	buffer += sprintf(buffer, "Press CONTINUE to continue execution and ignore all the errors of this type%s%s",
					  endline, endline);
#endif // USE_OWN_ERROR_MESSAGE_WINDOW
...
#else
#ifdef USE_OWN_ERROR_MESSAGE_WINDOW
	int result = MessageBox(NULL, assertion_info, "Fatal Error", MB_CANCELTRYCONTINUE | MB_ICONERROR | MB_SYSTEMMODAL);
...
void _terminate()
{
	...
	gather_info("<no expression>", "Unexpected application termination", 0, 0,
	...
	MessageBox(GetTopWindow(NULL), assertion_info, "Fatal Error", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
